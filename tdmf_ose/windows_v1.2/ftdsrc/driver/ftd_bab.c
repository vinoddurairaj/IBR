...
ftd_bab_init(int size, int num, ULONG maxmem)
{
    ...
    ALLOC_LOCK(bab_buffer_lock, QNM " bab specific lock");
    ...
    for (i = 0; i < num; i++) 
    {
                ...
                ||  (NULL == buf)          ) 
        {
            ...
            //
            // Give back 32Megs of memory. Since this is 1.2.x it works
            //
            giveback = (32 * 1024 * 1024) / (chunk_size * sizeof(ftd_uint64_t)); 

            //
            // This is ok, you won't see less than 64MB being allocated 
            // anyhow
            //
            if ( giveback > i )
                giveback = 2;

            bablen -= (chunk_size * giveback * sizeof(ftd_uint64_t));
            for (j = 0; j < giveback; j++) 
            {
                buf = bab_buffer_alloc();
                kmem_free(buf->start, chunk_size * sizeof(ftd_uint64_t));
                kmem_free(buf, sizeof(bab_buffer_t));
            }
            break;
        }
...
ftd_bab_fini(ULONG maxmem)
{
    ...
    while (bab_mgrs != NULL) 
    {
        if (ftd_bab_free_mgr(bab_mgrs) != 0) 
        {
            ...
            // PROBLEM!
            // This is kinda scary..
            // if we break out here, it means that we could 
            // not actually free the manger, meaning that 
            // memory is still in use...
            // but we still go on? Whoa!
            // 
            break;
        }
    }
...
ftd_bab_alloc_mgr(ULONG maxmem)
{
    ...
    //
    // This is the same mechanism as in the chunks, they reverse allocate the list...
    //
...
ftd_bab_free_mgr(bab_mgr_t *mgr)
{
    ...
    //
    // set temp var to begining of manager list
    //
    temp = &bab_mgrs;
    while ((*temp != NULL) && (*temp != mgr)) 
    {
        //
        // can't find it? go to the next one
        //
        temp = &(*temp)->next;
    }

    //
    // If you did not find the manager, there is a problem!
    //
    if (*temp == NULL) 
    {
        OUT_FCT(ftd_bab_free_mgr)
    ...
    //
    // PROBLEM!
    // Err, we didn't actually free this memory
    //
    *temp = mgr->next;
    
/* FIXME: adjust the bab_mgr limits now that we have one less to deal with */
