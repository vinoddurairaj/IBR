...
static void printpstore(ps_group_info_t *ginfo);
...
printgroup(ftd_lg_t *lgp, int Other)
{
	...
	//if ((group_info.state == FTD_MODE_BACKFRESH) || (group_info.shutdown == 1)) {
	if ((group_info.shutdown == 1)) {
		printshutdownstate(&group_info);
	}
...
*/
	...
	ForEachLLElement(lgp->cfgp->devlist, devpp) {
...
#if !defined(_WINDOWS)
		stat(devp->devname, &statbuf);

		dev_num = statbuf.st_rdev;
#else
		found = FALSE;
		...
		for (i = 0; i < ndevs; i++) {
			if (!strcmp(devp->devname, devstat[i].devname + strlen(devstat[i].devname) - 2)) {
				// found it
				found = TRUE;
				break;
			}
...
{
	char	buf[MAXPATHLEN];
	unsigned int actual, num_chunks, chunk_size, size;
    ...
    cfg_get_driver_key_value("num_chunks", buf, CFG_IS_NOT_STRINGVAL);
	num_chunks = strtol(buf, NULL, 0);

	memset(buf, 0, sizeof(buf));
	cfg_get_driver_key_value("chunk_size", buf, CFG_IS_NOT_STRINGVAL);
...
static void
printpstore(ps_group_info_t *ginfo)
{
