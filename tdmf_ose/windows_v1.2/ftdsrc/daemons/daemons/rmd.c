...
cmd_handshake(int fd, headpack_t *header)
{
    ...
    if (LICKEY_OK != (response = get_license (RMDFLAG, &lickey))) {
        ...
        case LICKEY_FILEOPENERR:
            reporterr (ERRFAC, M_LICFILMIS, ERRCRIT, "FTD");
            break;
        case LICKEY_BADKEYWORD:
            reporterr (ERRFAC, M_LICBADWORD, ERRCRIT, "FTD");
            break;
        default:
            reporterr (ERRFAC, M_LICFILE, ERRCRIT, "FTD");
        }
        exit (1);
    }
    if (lickey == NULL) {
        reporterr (ERRFAC, M_LICFILFMT, ERRCRIT, "FTD");
        exit (1);
    }
...
cmd_bfddevs(int fd, headpack_t *header)
{
    ...
    error_tracef( TRACEINF, "cmd_bfddevs():BFD device: %s:%d restart sector offset = %d", sd->devname, sd->mirfd, header->offset );
...
cmd_rfddevs(int fd, headpack_t *header)
{
    ...
    /* Now, seek to the appropriate offset */
    error_tracef( TRACEINF, "cmd_rfddevs():RFD device: %s restart sector offset = %d", sd->devname, header->offset );
    offset = (((u_longlong_t)header->offset) << DEV_BSHIFT);
    if (llseek(sd->mirfd, offset, SEEK_SET) == -1) {
...
readitem (int fd)
{
    ...
    error_tracef( TRACEINFO, "readitem():header.magicvalue = %08x", header.magicvalue );
    error_tracef( TRACEINFO, "readitem():header.cmd = %d", header.cmd );
...
main (int argc, char** argv, char** environ)
{
    ...
    if (_rmd_apply) {
        ...
        if (_rmd_jrn_mode == MIR_ONLY) {
            error_tracef( TRACEINFO, " %s opening mirrors EXCL", argv0 );
            closedevs(-1);
