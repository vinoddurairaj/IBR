...
#-----------------------------------------------------------------------------
#
# %Q%configtool -- %PRODUCTNAME% Configuration File Editor
#
# Copyright (c) 1997, 1998 %COMPANYNAME%
# All Rights Reserved.
#
#-----------------------------------------------------------------------------

# -- define necessary enviroment variables, load Tix and BLT shared libraries
global env
...
# --FXM--
set SYS(rebootcmd) "/etc/shutdown -r -y 0"
...
if {$SYS(platform) == "HP-UX"} {
    ...
    set SYS(defjournal) "/%FTDVAROPTDIR%"
    ...
    set SYS(hostname) "/bin/hostname"    
    set SYS(services) "/etc/services"
    set SYS(defjournal) "/%FTDVAROPTDIR%/journal"
}
...
proc checkvarschange {cfgname} {
    ...
    foreach g [info globals "%Q%*"] {
        ...
        foreach element [array names %Q%] { 
            if { [info exists oldSYS($element) ]} {
                if { $SYS($element) != $oldSYS($element) } {
                    set changed 1
                }
            } else {
                set changed 1
...
proc makeMainMenu {} {
    ...
    $w.view.m add command -label "About %PRODUCTNAME%..." \
    ...
    wm title . "%Q% Configuration Editor"
...
proc init_bab_and_ps {} {
    ...
    if {($SYS(platform) == "HP-UX") || ($SYS(platform) == "AIX")} { 
        toplevel .top	
        wm title .top "%PRODUCTNAME% License Agreement"
    ...
    toplevel .top
    wm title .top "%PRODUCTNAME% Configuration"
    wm transient .top [winfo toplevel .]
    set t .top
    frame $t.topF
    frame $t.botF -relief groove -bd 2
	...
	if {[catch "exec /usr/sbin/lsdev -P | grep %Q%" retstr]} {
	    set ret [%Q%_dialog .errorT "Error" "Cannot initialize BAB; %CAPQ% driver is not included in the Predefined Devices object class.  This may be a result of an incomplete or failed installation.  You should remove and re-install %PRODUCTNAME% to resolve this problem." 4i warning 0 Exit]
	    exit
	}
	#
	...
	if {[catch "exec /usr/sbin/lsdev -C | grep %Q%0" retstr]} {
	    set ret [%Q%_dialog .errorT "Error" "Cannot initialize BAB; %CAPQ% driver is not included in the Customized Devices object class.  This may be a result of an incomplete or failed installation.  You should remove and re-install %PRODUCTNAME% Data to resolve this problem." 4i warning 0 Exit]
	    exit
    ...
    set failed 0
    if {[catch "exec /%FTDBINDIR%/%Q%init -b $SYS(bab) >& /dev/null"]} {
        displayInfo ""  off 
        set failed 1
        if {$failed} {
    ...
    if { $SYS(platform) == "HP-UX" } {
        set ret [%Q%_dialog .workedT "Kernel Build Successful" "The kernel was rebuilt sucessfully with the %CAPQ% driver included.  The following command will be used for the reboot:\n      $SYS(rebootcmd)\nIs it ok to reboot the system now?" 4i question 0 Yes "No, Exit" ]
        if {$ret == 1} {
            displayInfo "You will need to reboot the system manually before configuration can continue."
            exit
        } else {
	    cd /
	    if {[catch "exec /bin/nohup $SYS(rebootcmd)" ] } {
		displayError\
		    "Reboot failed.  You will have to reboot the system manually."
		exit
	    }
	}
    } elseif { $SYS(platform) == "AIX" } {
    ...
    if {$failed} {
	displayError "An error occurred during an attempt to load the driver.  You may try a manual configuration of the BAB using the method documented in the %PRODUCTNAME% Data Operations Guide."
	exit
    } else {
...
proc init_bab {} {
	...
	#
	if {[catch "exec /usr/sbin/lsdev -P | grep %Q%" retstr]} {
	    set ret [%Q%_dialog .errorT "Error" "Cannot initialize BAB; %CAPQ% driver is not included in the Predefined Devices object class.  This may be a result of an incomplete or failed installation.  You should remove and re-install %PRODUCTNAME% Data to resolve this problem." 4i warning 0 Exit]
	    exit
	}
	#
	# Make sure that def%Q% has happened
...
proc Introduction {{aboutFlag 0}} {
    ...
    label $w.title -text "%PRODUCTNAME% Configuration Tool" -fg black -bg white
    pack $w.title -side top
    set versionmsg "Version %VERSION%"
    set msg "Copyright (c) 1999 %COMPANYNAME%\nAll Rights Reserved."
    message $w.msg2 -text $msg -aspect 10000 -justify center -font -Adobe-Helvetica-Bold-R-Normal--*-100-*-*-*-*-*-* -bg white
   
    
    message $w.vermsg -text "Version %VERSION%" -aspect 10000 -justify center -font -Adobe-Helvetica-Bold-R-Normal--*-100-*-*-*-*-*-* -bg white
...
proc reportconfigerrors {} {
    ...
    wm title .configerrT "%CAPQ% Configuration Errors"
...
proc editLogGroup {{lgnum ""}} {
    ...
    set w .f1
    set SYS(candidatecfg) [format "p%03d" $SYS(candidatelg)]
    wm title . "%CAPQ% Configuration Editor -- Logical Group $SYS(candidatelg)"
    topNotebook $SYS(candidatecfg)
    set SYS(cur%Q%) ""
    update%Q%devs [$w.nb subwidget %Q%_Devices] $SYS(candidatecfg)
    # - save off cfg variables so we know whether we need to save changes
...
proc saveLogGroup {} {
    ...
    if {$SYS(candidatelg) == "" || ![checkvarschange $SYS(candidatecfg)] } {
	displayInfo "No changes need to be saved"
	menu_on
	return
    }
   ...
   # cleanSlate
    set w .f1
    wm title . "%CAPQ% Configuration Editor -- Saving Changes"
...
proc resetLogGroup {} {
    ...
    wm title . "%CAPQ% Configuration Editor"
    topNotebook $SYS(candidatecfg)
...
proc finalSaveQuery {} {
   ...
   # message $w.msg -aspect 10000 -justify center -text \
    ...
    button $w.save.saveB -text "Yes" -command "if {$SYS(onprimary)} {writetcpinfo}; writeconfig $SYS(candidatelg); exit"
...
proc buildThrot2 {} {
    ...
    set ttestlist "NETKBPS PCTCPU PCTBABINUSE NETCONNECTFLAG PID CHUNKSIZE STATINTERVAL MAXSTATFILESIZE STATTOFILEFLAG TRACETHROTTLE SYNCMODE SYNCMODEDEPTH SYNCMODETIMEOUT COMPRESSION NETMAXKBPS CHUNKDELAY IODELAY DRIVERMODE ACTUALKBPS EFFECTKBPS PERCENTDONE ENTRIES READKBPS WRITEKBPS"
...
proc buildThrot3 {} {
    ...
    set tvarlist { {} CHUNKSIZE STATINTERVAL MAXSTATFILESIZE STATTOFILEFLAG TRACETHROTTLE SYNCMODE SYNCMODEDEPTH SYNCMODETIMEOUT COMPRESSION NETMAXKBPS CHUNKDELAY IODELAY }
...
proc configTunableMenu {w cfgname} {
    ...
    set f [ $w.delF subwidget frame ]
    set f $f.padF
    $f.netE configure -textvariable ${cfgname}(IODELAY:)
}
# -- tunable parameters
proc makeTunableMenu {w cfgname} {
    ...
    grid $f.statonRB - $f.intervalL $f.intervalE $f.secsL -sticky w
    grid $f.statoffRB - $f.maxsizeL $f.maxsizeE  $f.kbL -sticky w
    ...
    tixLabelFrame $w.delF -label "Write Delay" -labelside acrosstop
    pack $w.delF -anchor nw -expand 1 -fill x
    set f [ $w.delF subwidget frame ]

    frame $f.padF
    pack $f.padF -pady .25c -padx .5c -side left
    
    set f $f.padF

    label $f.delL -text "Delay writes for"
    entry $f.netE -width 9 -textvariable ${cfgname}(IODELAY:)
    label $f.microL -text "microseconds"
    pack $f.delL -side left -padx .25c
    pack $f.netE -side left
    pack $f.microL -side left -padx .25c
...
##
## History of changes at end of file
#
##----------------------------------------------------------------------
## DESCRIPTION
##	Tetris with multiplayer mode (Netris!)
##	Works either in Tk plugin or stand-alone.
##	See below for multiplayer specific information.
##
## OPTIONS
##	For plugin, options are specified in the <embed> tag, like:
##		<embed src="tetris.tcl" width=440 height=440 blocksize=14>
##	For standalone, use the regular '-' notation, like:
##		tetris.tcl -blocksize 14
##
## autopause TCL_BOOLEAN	Defaults to 0
##	Whether to use the auto-pause feature (pauses when game loses focus).
##
## blocksize #			Defaults to 15 (pixels)
##	Size in pixels of one block.  A tetris grid is 30 high X 10 wide.
##
## color[0-6] colorname		Defaults to major colors
##	Color of each of the 7 different tetris pieces.
##
## drop binding			Defaults to <Key-space>
##	Key binding to drop the piece all the way down.
##
## left binding			Defaults to <Key-Left> (left arrow)
##	Key binding to move the piece left.
##
## level #			Defaults to 0
##	Initial level to start at.  Don't go higher than 20.
##
## maxinterval #		Defaults to 500 (microsecs)
##	Initial interval in microseconds between slides.  This is
##	decremented by maxint/20 every 10 rows (unless you start at a
##	higher level), and should never go below 10 microsecs.
##
## right binding		Defaults to <Key-Right> (right arrow)
##	Key binding to move the piece right.
##
## rotleft binding		Defaults to <Key-Up> (up arrow)
##	Key binding to rotate the piece left (counterclockwise).
##
## rotright binding		Defaults to <Key-Down> (down arrow)
##	Key binding to rotate the piece right (clockwise).
##
## shadow TCL_BOOLEAN		Defaults to 1
##	Whether to use the shadow-piece feature (shadow at bottom)
##
## shownext TCL_BOOLEAN		Defaults to 1
##	Whether to show the next piece
##
## slide binding		Defaults to <Key-Return>
...
## MULTIPLAYER OPTIONS
##
## multiplayer TCL_BOOLEAN	Defaults to 1
##	Whether to allow multiplayer mode (requires sockets)
##
## highbandwidth TCL_BOOLEAN	Defaults to 1
##	Wether you have a high bandwidth connection to your opponent(s)
##	More info will be shared if true.
##	I recommend turning this off on Win/Mac because of slow game play.
##
## hostname string		Defaults to [info hostname]
##	Hostname to use for yourself
##
## port #			Defaults to 15141
##	TkTetris server port (that other TkTetris players
##	would connect to).
##
## MULTIPLAYER USAGE
##	In order to get the multiplayer option, sockets must be available
##	(thus this is currently disabled for Tk plugin use).  The play is
##	as synchronized as possible between players.  Any player can start
##	or pause a game.  See MULTIPLAYER RULES for more info.
##
## CONNECTING TO ANOTHER PLAYER
##	To connect to another player, you must know their hostname/IP and
##	TkTetris port (defaults to 15141).  You can find your own hostname/IP
##	and port by opening the multiplayer dialog.  When you have the other
##	players vitals, ONLY ONE PLAYER should type in the info of the person
##	to connect to in the proper entry fields on the multiplayer dialog
##	and press 'Connect'.  The other player will receive a connection
##	message.  If the player accepts then you will see their name remain
##	in your list of connected players.  If they refuse then they will
##	disappear from the listbox.  'Disconnect' disconnects from all
##	connected players.
##
## RULES AND SCORING
##	Each dropped piece is 5 points.
##	Each row dropped rows is:
##		( <NUMBER ROWS DROPPED>^^2 ) * ( <LEVEL>+1 )
##	That means dropping multiple rows at a time is best.
##	The level increases every 10 rows dropped.  At this time
##	the slide interval decreases (the game gets faster).
##	The game is over when a new piece can't be placed on the board.
##
## MULTIPLAYER RULES
##	Scoring occurs the same, but game play is slightly different.
##	When a player drops more than one row, an equal number of rows
##	is added to his opponents' board(s).  When the game would
##	normally be over for a player, his screen is instead reset and
##	the game continues.  That player also incurs a 'death', which
##	accumulates.  You can see other players' 'death' count below the
##	scale bar which pops up for that player.  What's the scale bar?
##	That indicates the height of the pieces on the other player's
##	board.  This is updated after each cemented piece in high
##	bandwidth mode, but only after a row drops in low bandwidth mode.
##	Reset also registers as a death and the game continues for that
##	player.  There is no 'Game Over', the game continues on with only
##	the ability to pause.
##
## NOTES
##
##	This is a rough first run at multiplayer tetris.  Future versions
##	will allowing 'targeting' players instead of blasting everybody
##	with added rows, along with other nasty tricks.  If you have any
##	suggestions or bug reports, please email me.
##
##----------------------------------------------------------------------

set TkTetris(WWW) [info exists embed_args]
if $TkTetris(WWW) { 
  proc bell args {}
  proc TkTetris:Usage {} {}
} else {
#  wm withdraw .
  ...
  proc TkTetris:Usage {} {
    ...
    puts stderr "Usage: [file tail $argv0] ?-info? ?-blocksize #pixels?\
      ?-autopause 0/1? ?-color\[0-6\] rgbcolor? ?-maxinterval #usecs?\
      ?-shownext 0/1? ?-(left|right|rotleft|rotright|drop|slide) binding?"
#    exit
  }

  proc TkTetris:About {} {
    global TkTetris
    tk_dialog $TkTetris(base).about "About TkTetris v$TkTetris(version)" \
	"Programmed by Jeffrey Hobbs,\nAugust 1995.\
	\njhobbs@cs.uoregon.edu\nhttp://www.cs.uoregon.edu/~jhobbs/" \
	questhead 0 OK
  }
}

proc TkTetris:ParseArgs {} {
  global TkTetris
  if $TkTetris(WWW) {
    global embed_args
  ...
  for {set i 0; set argc [llength $args]} {$i<$argc} {incr i} {
    ...
    switch -glob $key {
      ...
      a*	{ set TkTetris(autoPause)	[regexp -nocase $truth $val] }
      b*	{ set TkTetris(blocksize)	$val }
      color[0-6] {
	if [regexp {([0-6])} $key junk num] { set TkTetris(Color$num) $val }
      }
      dr*	{ set TkTetris(KeyDrop)		$val }
      ma*	{ set TkTetris(maxInterval)	$val }
      lef*	{ set TkTetris(KeyMoveLeft)	$val }
      lev*	{ set TkTetris(initLevel)	$val }
      ri*	{ set TkTetris(KeyMoveRight)	$val }
      rotl*	{ set TkTetris(KeyRotLeft)	$val }
      rotr*	{ set TkTetris(KeyRotRight)	$val }
      sha*	{ set TkTetris(shadow)		[regexp -nocase $truth $val] }
      sho*	{ set TkTetris(showNext)	[regexp -nocase $truth $val] }
      sl*	{ set TkTetris(KeySlide)	$val }

      mu*	{ set TkTetris(multi)		[regexp -nocase $truth $val] }
      hi*	{ set TkTetris(highband)	[regexp -nocase $truth $val] }
      ho*	{ array set TkTetris [list host	$val con,host $val] }
      p*	{ array set TkTetris [list port	$val con,port $val] }

      help - info - default TkTetris:Usage
    }
  }
}

proc TkTetris:Init {{base {}}} {
  global TkTetris
  ...
  array set TkTetris [list root $base base $base host [info hostname] \
      con,host	[info hostname] myip 127.0.0.1 \
      width	[expr 10*$TkTetris(blocksize)] \
      height	[expr 30*$TkTetris(blocksize)]]
  if [string match {} $base] { set TkTetris(root) . }

  array set TkTetris [list w,board	$base.r.c \
      w,stats	$base.stats	w,shadow	$base.r.l \
      w,next	$base.l.next	w,socket	$base.l.sock \
      w,multi	$base.multi	w,player	$base.multi.pla \
      w,scale	$base.s		]
  pack [frame $base.l] -side left -fill both -padx 2 -pady 2
  pack [frame $base.r] -side left -fill both -exp 1
  pack [frame $TkTetris(w,scale)] -side left -fill both
  pack [canvas $TkTetris(w,board) -highlightt 0 -width $TkTetris(width) \
	    -height $TkTetris(height) -relief ridge -bd 1 -bg gray]
  pack [canvas $TkTetris(w,shadow) -highlightt 0 -width $TkTetris(width) \
	    -height $TkTetris(blocksize) -relief ridge -bd 1 -bg gray]

  set opts {-takefocus 0 -pady 2 -highlightthickness 1}

  label $base.l.title -text "TkTetris v$TkTetris(version)" -relief ridge -bd 2
  label $base.l.obj -text "Next Object" -anchor c
  canvas $TkTetris(w,next) -width [expr $TkTetris(blocksize)*4] \
      -height [expr $TkTetris(blocksize)*4] -highlightthickness 0
  frame $base.l.score
  frame $base.l.level
  frame $base.l.rows
  eval button $base.l.start -textvar TkTetris(start) \
      -command TkTetris:ToggleState $opts
  eval button $base.l.new -text Reset -un 0 $opts -command [list {
    TkTetris:Reset
    if $TkTetris(numplayers) {
      incr TkTetris(deaths)
      TkTetris:TellPlayers update 0 $TkTetris(maxbrick) $TkTetris(deaths)
    }
  } ]

  if !$TkTetris(WWW) {
    eval button $TkTetris(w,socket) -text MultiPlayer $opts \
	-command TkTetris:MultiPlayer -state disabled
      eval button $base.l.quit  -text Quit  -un 0 -command [list {
	  catch "wm withdraw .tetris"
      }]
    eval button $base.l.about -text About -un 0 -command TkTetris:About $opts
    eval button $base.l.stats -text Stats -un 1 -command TkTetris:Stats $opts
    eval checkbutton $base.l.pause -text {{Auto Pause}} -anchor w \
	-var TkTetris(autoPause) -command TkTetris:AutoPause $opts
  }
  eval checkbutton $base.l.show  -text {{Show Next}} \
      -var TkTetris(showNext) -anchor w -takefocus 0 -command [list {
    if !$TkTetris(showNext) {
      $TkTetris(w,next) delete all
    } elseif {[string comp $TkTetris(start) Start]} { TkTetris:ShowNext }
  }] $opts
  eval checkbutton $base.l.shadow -text {{Shadow Piece}} \
      -var TkTetris(shadow) -anchor w -takefocus 0 -command [list {
    if $TkTetris(shadow) {
      if [string comp {} [$TkTetris(w,board) bbox piece]] TkTetris:Shadow
    } else {$TkTetris(w,shadow) itemconfig all -fill gray}
  }] $opts

  bind $TkTetris(root) <Control-r> { $TkTetris(base).l.new invoke }
  bind $TkTetris(root) <Control-s> { $TkTetris(base).l.start invoke }
  if !$TkTetris(WWW) {
    bind $TkTetris(root) <Control-q> { $TkTetris(base).l.quit invoke }
    bind $TkTetris(root) <Control-t> { $TkTetris(base).l.stats invoke }
    bind $TkTetris(root) <Control-a> { $TkTetris(base).l.about invoke }
  }

  pack $base.l.title $base.l.obj $TkTetris(w,next) $base.l.score \
      $base.l.level $base.l.rows $base.l.start $base.l.new -fill both
  if !$TkTetris(WWW) {
    pack $base.l.about $base.l.stats $TkTetris(w,socket) $base.l.quit \
	$base.l.pause -fill both
  }
  pack $base.l.show $base.l.shadow -fill both

  label $base.l.score.l -text Score: -anchor w
  label $base.l.level.l -text Level: -anchor w
  label $base.l.rows.l -text Rows:   -anchor w
  label $base.l.score.r -textvar TkTetris(score) -anchor e
  label $base.l.level.r -textvar TkTetris(level) -anchor e
  label $base.l.rows.r -textvar TkTetris(rows) -anchor e
  button $base.l.level.u -text ^ -padx 0 -pady 0 -takefocus 0 -command {
    set TkTetris(interval) [TkTetris:GetInterval [incr TkTetris(level)]]
  } -highlightthickness 1

  pack $base.l.score.l $base.l.level.l $base.l.rows.l -side left -fill x
  pack $base.l.level.u -side left -fill x
  pack $base.l.score.r $base.l.level.r $base.l.rows.r -side right -fill x

  set half [expr round($TkTetris(blocksize)/2)]
  for {set j 0; set jb 0} {$j < 30} {incr j; incr jb $TkTetris(blocksize)} {
    for {set i 0; set ib 0} {$i < 10} {incr ib $TkTetris(blocksize); incr i} {
      set x [expr $ib+$half]
      set y [expr $jb+$half]
      $TkTetris(w,board) create line $x $y [incr x] [incr y] \
	  -tags back -fill $TkTetris(Color0)
      if {$j == 0} {
	$TkTetris(w,shadow) create rect $ib 0 \
	    [expr $ib+$TkTetris(blocksize)] $TkTetris(blocksize) -outline {}
      }
    }
  }
  focus $TkTetris(w,board)

  TkTetris:Reset
  TkTetris:InitPieces $TkTetris(blocksize)
  if $TkTetris(WWW) { TkTetris:Stats }
  TkTetris:AutoPause
  if {!$TkTetris(WWW) && $TkTetris(multi)} {
    if [catch TkTetris:SocketInit msg] { bgerror $msg }
  }
}

proc TkTetris:SocketInit {} {
  global TkTetris

  set port $TkTetris(port)
  ## Tries to connect to the specified port or any of the 50 above it
  while {[catch {socket -server TkTetris:SocketAccept $port} msg]} {
    if [string match {*address already in use} $msg] {
      if {[incr port]>[expr $TkTetris(port)+50]} {
	set TkTetris(multi) 0
  ...
  ## Only allow multiplayer mode if we succeeded in getting a port
  $TkTetris(w,socket) config -state normal
  set TkTetris(port) $port

  ## Capture our ip/port by sending ourselves a message
  set TkTetris(sockettest) 1
  set sock [socket -myaddr $TkTetris(host) $TkTetris(host) $TkTetris(port)]
  catch {close $sock}
}

proc TkTetris:SocketAccept {sock ipaddr port} {
  global TkTetris TkTetrisPlayers

  if [info exists TkTetris(sockettest)] {
    unset TkTetris(sockettest)
    set TkTetris(myip) $ipaddr
    catch {close $sock}
    return
  }
  set newhost [lindex [fconfigure $sock -peer] 1]
  if {[tk_dialog $TkTetris(base).socket "Socket Connection" \
      "$TkTetris(port): Accept new connection from $newhost ($ipaddr)?" \
      questhead 0 "You bet" "Kick 'em off"]} {
    puts $sock "TkTetris2 $TkTetris(host) $TkTetris(port) goodbye"
    flush $sock
    catch {close $sock}
    return
  } else {
    set id "(127\.0\.0\.1|$TkTetris(host)|localhost)"
    if [regexp $id $ipaddr] { set $ipaddr $TkTetris(myip) }
    TkTetris:NewPlayer $sock $newhost $ipaddr $port
  }
}

proc TkTetris:SocketRead {sock} {
  if {[eof $sock] || ([gets $sock line] == -1)} {
    TkTetris:SocketShutdown $sock
    return
  }
  global TkTetris TkTetrisPlayers
  set peer [fconfigure $sock -peer]
  set id "[lindex $peer 0] [lindex $peer 2]"
  #puts "read sock from $id: $line"
  ## Line should be of the form:
  ## TkTetris<VERSION> <PLAYERHOST> <PLAYERPORT> <SWITCH> args
  switch -exact -- [lindex $line 3] {
    pause {
      TkTetris:Pause
      set TkTetris(message) $line
    }
    start {
      if $TkTetris(break) {
	TkTetris:Resume
	set TkTetris(message) [lreplace $line 0 0]
      }
    }
    brick { set TkTetrisPlayers($id,maxbrick) [lindex $line 4] }
    update {
      if {[scan [lrange $line 4 end] "%d %d %d" addrows height deaths]==3} {
	TkTetris:AddRows $addrows
	set TkTetrisPlayers($id,maxbrick) $height
	set TkTetrisPlayers($id,deaths) $deaths
      }
    }
    says { set TkTetris(message) [lreplace $line 0 0] }
    goodbye {
      TkTetris:SocketShutdown $sock
      tk_dialog $TkTetris(base).goodbye "Player left" \
	  [lreplace $line 0 0] warning 0 OK
    }
    default {}
  }
}

proc TkTetris:SocketConnect {host port} {
  global TkTetris TkTetrisPlayers

  set id "(127\.0\.0\.1|$TkTetris(myip)|$TkTetris(host)|localhost)"
  if [regexp "$id $TkTetris(port)" "$host $port"] {
    return -code error "don't touch yourself"
  }
  if [catch {socket -myaddr $TkTetris(host) $host $port} sock] {
    return -code error $sock
  } else {
    set peer [fconfigure $sock -peer]
    TkTetris:NewPlayer $sock [lindex $peer 1] [lindex $peer 0] $port
  }
}

proc TkTetris:SocketShutdown sock {
  global TkTetris TkTetrisPlayers

  catch {close $sock}
  #puts stderr "$TkTetris(port) shutdown socket $sock"

  ## Delete multi-player info
  catch {
    set id $TkTetrisPlayers(sock,$sock)
    unset TkTetrisPlayers(sock,$sock)
    catch {
      unset TkTetrisPlayers($id,sock) \
	  TkTetrisPlayers($id,name) TkTetris($id,maxbrick)
    }
    destroy $TkTetris(w,scale).$sock
  }
  set TkTetris(numplayers) [llength [array names TkTetrisPlayers *,sock]]

  TkTetris:UpdatePlayers
}

proc TkTetris:NewPlayer {sock host ip port} {
  global TkTetris TkTetrisPlayers

  set id [list $ip $port]
  set TkTetrisPlayers(sock,$sock) $id
  set TkTetrisPlayers($id,sock) $sock
  set TkTetrisPlayers($id,name) [list $host $ip $port]
  fconfigure $sock -blocking 0 -translation {auto crlf}
  fileevent $sock readable [list TkTetris:SocketRead $sock]

  set TkTetrisPlayers($id,maxbrick) 0
  set TkTetrisPlayers($id,deaths) 0
  
  pack [set w [frame $TkTetris(w,scale).$sock]] -fill both -exp 1
  pack [scale $w.s -orient v -from 30 -to 0 -state disabled \
      -var TkTetrisPlayers($id,maxbrick) -showvalue yes] -fill y -exp 1
  pack [label $w.l -textvar TkTetrisPlayers($id,deaths)] -fill both
...
proc TkTetris:TellPlayers {args} {
  global TkTetris TkTetrisPlayers

  foreach p [array names TkTetrisPlayers *,sock] {
    if [catch {
      set msg "TkTetris2 $TkTetris(host) $TkTetris(port) $args"
      puts $TkTetrisPlayers($p) $msg
      flush $TkTetrisPlayers($p)
    } err] {
      TkTetris:SocketShutdown $TkTetrisPlayers($p)
    }
  }
}

proc TkTetris:UpdatePlayers {args} {
  global TkTetris TkTetrisPlayers

  if [winfo exists $TkTetris(w,player)] {
    $TkTetris(w,player) delete 0 end
    foreach p [array names TkTetrisPlayers *,name] {
      $TkTetris(w,player) insert end $TkTetrisPlayers($p)
    }
  }
}

proc TkTetris:MultiPlayer {} {
  global TkTetris

  set w $TkTetris(w,multi)
  if ![winfo exists $w] {
    toplevel $w
    wm withdraw $w
    wm title $w "TkTetris v$TkTetris(version) MultiPlayer"

    label $w.hostl -text "My Host:" -anchor w -width 8
    label $w.host  -textvar TkTetris(host) -anchor e
    label $w.ip    -textvar TkTetris(myip) -anchor w
    label $w.portl -text "My Port:" -anchor w -width 8
    label $w.port  -text $TkTetris(port) -anchor e -width 8

    label $w.conhl -text "New Host:" -anchor w -width 8
    entry $w.conhe -width 12 -textvar TkTetris(con,host)
    label $w.conpl -text "Port:" -anchor w -width 8
    entry $w.conpe -width 8 -textvar TkTetris(con,port)

    set TkTetris(w,player) [listbox $w.pla -height 3]
    label $w.lsay -text Message:
    entry $w.say
    bind $w.say <Return> { TkTetris:TellPlayers says [%W get] }
    label $w.msg -textvar TkTetris(message) -relief ridge -bd 2

    button $w.con -text Connect -command {
      TkTetris:SocketConnect $TkTetris(con,host) $TkTetris(con,port)
    }
    button $w.gby -text Disconnect -command {
      TkTetris:TellPlayers goodbye
    }
    button $w.dis -text Dismiss -command "wm withdraw $w"

    grid $w.hostl $w.host $w.ip $w.portl $w.port -sticky news
    grid $w.conhl $w.conhe - $w.conpl $w.conpe -sticky news
    grid $w.con - - - - -sticky news
	...
	[winfo rooty $a]+([winfo height $a]-[winfo reqheight $w])/2]
  }
  TkTetris:UpdatePlayers
...
proc TkTetris:GetInterval {n} {
  global TkTetris

  set i [expr round($TkTetris(maxInterval)-($TkTetris(maxInterval)/20*$n))]
  if {$i<8} { return 8 }
  return $i
}

proc TkTetris:ToggleState {} {
  global TkTetris

  if [string match $TkTetris(start) Pause] {
    TkTetris:Pause
    if $TkTetris(numplayers) { TkTetris:TellPlayers pause }
  } elseif [string match $TkTetris(start) {Game Over}] {
    TkTetris:Reset
  } else {
    TkTetris:Resume
    if $TkTetris(numplayers) { TkTetris:TellPlayers start }
  }
}

proc TkTetris:AutoPause {} {
  global TkTetris

  if {$TkTetris(autoPause) && !$TkTetris(WWW)} {
    ## Not available for WWW play
    bind $TkTetris(root) <Unmap> {
      if [string match $TkTetris(start) Pause] TkTetris:Pause
    }
    bind $TkTetris(root) <Map>   {
      if [string match $TkTetris(start) Resume] TkTetris:Resume
    }
    ## These are not available for during multiplayer mode
    ## (would potentially require way too much communication)
    bind $TkTetris(root) <FocusOut> {
      if {[string match %d NotifyAncestor] && \
	  [string match $TkTetris(start) Pause]} {
	if !$TkTetris(numplayers) { TkTetris:Pause }
      }
    }
    bind $TkTetris(root) <FocusIn>  {
      if {[string match %d NotifyAncestor] && \
	  [string match $TkTetris(start) Resume]} {
	if $TkTetris(numplayers) { TkTetris:Resume }
      }
    }
  } else {
    foreach i {Unmap Map FocusOut FocusIn} { bind $TkTetris(root) <$i> {} }
  }
}
...
proc TkTetris:Pause {} {
  ...
  set TkTetris(break) 1
  foreach i [after info] { after cancel $i }
  set TkTetris(start) Resume
  foreach i [array names TkTetris Key*] {
    bind $TkTetris(root) $TkTetris($i) {}
  }
}

proc TkTetris:Resume {} {
  global TkTetris

  set TkTetris(break) 0
  set TkTetris(start) Pause
  bind $TkTetris(root) $TkTetris(KeyMoveLeft)	TkTetris:Left
  bind $TkTetris(root) $TkTetris(KeyMoveRight)	TkTetris:Right
  bind $TkTetris(root) $TkTetris(KeyRotLeft)	{TkTetris:Rotate left}
  bind $TkTetris(root) $TkTetris(KeyRotRight)	{TkTetris:Rotate right}
  bind $TkTetris(root) $TkTetris(KeySlide)	TkTetris:Slide
  bind $TkTetris(root) $TkTetris(KeyDrop)	TkTetris:Drop
  TkTetris:Fall
}

proc TkTetris:GameOver {} {
  global TkTetris

  if $TkTetris(numplayers) {
    set tmp $TkTetris(score)
    TkTetris:Reset
    set TkTetris(score) $tmp
    incr TkTetris(deaths)
    TkTetris:Resume
    TkTetris:TellPlayers update 0 $TkTetris(maxbrick) $TkTetris(deaths)
  } else {
    set TkTetris(break) 1
    set TkTetris(start) "Game Over"
    $TkTetris(w,board) delete piece
  }
}

proc TkTetris:Reset {} {
  global TkTetris TkTetrisBlock TkTetrisStats

  TkTetris:Pause
  array set TkTetris [list start Start score 0 level $TkTetris(initLevel) \
			  rows 0 next [random 7] maxbrick 0]
  set TkTetris(interval) [TkTetris:GetInterval $TkTetris(level)]
  $TkTetris(w,board) delete piece struc
  $TkTetris(w,next) delete all
  $TkTetris(w,shadow) dtag shadow
  $TkTetris(w,shadow) itemconfig all -fill gray
  for {set i -10} {$i < 300} {incr i} { set TkTetrisBlock($i) 0 }
  for {}          {$i < 310} {incr i} { set TkTetrisBlock($i) 1 }
  for {set i 0}   {$i < 7}   {incr i} { set TkTetrisStats($i) 0 }
}

proc TkTetris:Stats {} {
  global TkTetrisStats TkTetris TkTetrisPmap

  set w $TkTetris(w,stats)
  if ![winfo exists $w] {
    if $TkTetris(WWW) {
      pack [frame $w] -fill both -side left
    } else {
      toplevel $w
      wm withdraw $w
      wm title $w "TkTetris v$TkTetris(version) Stats"
    }
    pack [label $w.l -text "Piece Statistics"] -fill x
    pack [canvas $w.c -relief ridge -bd 2 \
	-width [expr $TkTetris(blocksize)*9.5] \
	-height [expr $TkTetris(blocksize)*22.5]]
    label $w.c.s -text "Session"
    label $w.c.g -text "Game"
    $w.c create window 5 5 \
	-window $w.c.s -anchor nw
    $w.c create window [expr 7*$TkTetris(blocksize)] 5 \
	-window $w.c.g -anchor nw
    for {set i 0} {$i < 7} {incr i} {
      foreach p $TkTetrisPmap($i) {
	$w.c create rectangle [lindex $p 0] [lindex $p 1] \
	    [lindex $p 2] [lindex $p 3] -tags "p$i"
      }
      set y [expr (3*$i+2)*$TkTetris(blocksize)]
      $w.c move p$i 0 $y
      $w.c itemconfig p$i -fill $TkTetris(Color$i)
      label $w.c.g$i -textvar TkTetrisStats(g$i) -anchor w
      $w.c create window $TkTetris(blocksize) $y \
	  -window $w.c.g$i -anchor nw
      label $w.c.$i -textvar TkTetrisStats($i) -anchor w
      $w.c create window [expr 8*$TkTetris(blocksize)] $y \
	  -window $w.c.$i -anchor nw
    }
    if $TkTetris(WWW) {
      label $w.about -font fixed \
	  -text "Programmed by Jeffrey Hobbs\njhobbs@cs.uoregon.edu\
		\nhttp://www.cs.uoregon.edu/~jhobbs/"
      pack $w.about -fill both -expand 1
    } else {
      pack [button $w.b -text "Dismiss" \
		-com "wm withdraw $w"] -fill both
...
proc TkTetris:InitPieces size {
  global TkTetrisStats TkTetrisPmap
  ## Block
  set TkTetrisPmap(0) "{[expr 4*$size] 0 [expr 5*$size] $size 4} \
      {[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 4*$size] $size [expr 5*$size] [expr 2*$size] 14} \
      {[expr 5*$size] $size [expr 6*$size] [expr 2*$size] 15}"
  ## L
  set TkTetrisPmap(1) "{[expr 3*$size] 0 [expr 4*$size] $size 3} \
      {[expr 4*$size] 0 [expr 5*$size] $size 4} \
      {[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 5*$size] $size [expr 6*$size] [expr 2*$size] 15}"
  ## Mirror L
  set TkTetrisPmap(2) "{[expr 3*$size] 0 [expr 4*$size] $size 3} \
      {[expr 4*$size] 0 [expr 5*$size] $size 4} \
      {[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 3*$size] $size [expr 4*$size] [expr 2*$size] 13}"
  ## Shift One
  set TkTetrisPmap(3) "{[expr 4*$size] 0 [expr 5*$size] $size 4} \
      {[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 5*$size] $size [expr 6*$size] [expr 2*$size] 15} \
      {[expr 6*$size] $size [expr 7*$size] [expr 2*$size] 16}"
  ## Shift Two
  set TkTetrisPmap(4) "{[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 6*$size] 0 [expr 7*$size] $size 6} \
      {[expr 4*$size] $size [expr 5*$size] [expr 2*$size] 14} \
      {[expr 5*$size] $size [expr 6*$size] [expr 2*$size] 15}"
  ## Bar
  set TkTetrisPmap(5) "{[expr 3*$size] 0 [expr 4*$size] $size 3} \
      {[expr 4*$size] 0 [expr 5*$size] $size 4} \
      {[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 6*$size] 0 [expr 7*$size] $size 6}"
  ## T
  set TkTetrisPmap(6) "{[expr 4*$size] 0 [expr 5*$size] $size 4} \
      {[expr 5*$size] 0 [expr 6*$size] $size 5} \
      {[expr 6*$size] 0 [expr 7*$size] $size 6} \
      {[expr 5*$size] $size [expr 6*$size] [expr 2*$size] 15}"

  for {set i 0} {$i < 7} {incr i} { array set TkTetrisStats "$i 0 g$i 0" }
}

proc TkTetris:ShowNext {} {
  global TkTetris TkTetrisPmap

  $TkTetris(w,next) delete all
  foreach i $TkTetrisPmap($TkTetris(next)) {
    $TkTetris(w,next) create rectangle [lindex $i 0] [lindex $i 1] \
	[lindex $i 2] [lindex $i 3]
  }
  $TkTetris(w,next) move all -$TkTetris(blocksize) 0
  $TkTetris(w,next) move all -$TkTetris(blocksize) 0
  $TkTetris(w,next) itemconfig all -fill $TkTetris(Color$TkTetris(next))
}

proc TkTetris:CreatePiece {} {
  global TkTetrisPiece TkTetrisPmap TkTetris TkTetrisStats TkTetrisBlock

  set p $TkTetris(next)
  set j 0
  foreach i $TkTetrisPmap($p) {
    if $TkTetrisBlock([set TkTetrisPiece($j) [lindex $i 4]]) {
      TkTetris:GameOver
      return
    }
    set TkTetrisPiece(_$j) [$TkTetris(w,board) create rectangle \
	[lindex $i 0] [lindex $i 1] [lindex $i 2] [lindex $i 3] \
	-tags "p$p piece"]
    incr j
  }
  incr TkTetrisStats($p)
  incr TkTetrisStats(g$p)
  $TkTetris(w,board) itemconfig p$p -fill $TkTetris(Color$p)
  set TkTetris(next) [random 7]

  if $TkTetris(showNext) TkTetris:ShowNext
  TkTetris:Shadow
}

proc TkTetris:Fall {{a {}}} {
  global TkTetris

  if !$TkTetris(break) {
    after $TkTetris(interval) TkTetris:Fall
    TkTetris:Slide
  }
}

proc TkTetris:Shadow {} {
  global TkTetrisPiece TkTetris

  if !$TkTetris(shadow) return
  $TkTetris(w,shadow) dtag shadow
  $TkTetris(w,shadow) itemconfig all -fill gray
  $TkTetris(w,shadow) addtag shadow with [expr $TkTetrisPiece(0)%10+1]
  $TkTetris(w,shadow) addtag shadow with [expr $TkTetrisPiece(1)%10+1]
  $TkTetris(w,shadow) addtag shadow with [expr $TkTetrisPiece(2)%10+1]
  $TkTetris(w,shadow) addtag shadow with [expr $TkTetrisPiece(3)%10+1]
  $TkTetris(w,shadow) itemconfig shadow -fill black
}

proc TkTetris:CementPiece {} {
  global TkTetrisBlock TkTetrisPiece TkTetris

  foreach i {0 1 2 3} {
    set TkTetrisBlock($TkTetrisPiece($i)) 1
    set row [expr $TkTetrisPiece($i)/10]
    $TkTetris(w,board) addtag row$row with $TkTetrisPiece(_$i)
    if {(30-$row)>$TkTetris(maxbrick)} {
      set TkTetris(maxbrick) [expr 30-$row]
    }
  }
  $TkTetris(w,board) addtag struc with piece
  $TkTetris(w,board) itemconfig struc -stipple gray50
  $TkTetris(w,board) dtag piece
  incr TkTetris(score) 5
  TkTetris:DropRows
}

proc TkTetris:Slide {} {
  global TkTetris TkTetrisPiece TkTetrisBlock
  
  if [string match [set ix [$TkTetris(w,board) bbox piece]] {}] {
    TkTetris:CreatePiece
  } else {
    if {$TkTetrisBlock([expr $TkTetrisPiece(0)+10]) || \
	$TkTetrisBlock([expr $TkTetrisPiece(1)+10]) || \
	$TkTetrisBlock([expr $TkTetrisPiece(2)+10]) || \
	$TkTetrisBlock([expr $TkTetrisPiece(3)+10])} {
      TkTetris:CementPiece
      update idletasks
    } else {
      incr TkTetrisPiece(0) 10
      incr TkTetrisPiece(1) 10
      incr TkTetrisPiece(2) 10
      incr TkTetrisPiece(3) 10
      $TkTetris(w,board) move piece 0 $TkTetris(blocksize)
    }
  }
}
...
proc TkTetris:Drop {} {
  ...
  if [string match [set ix [$TkTetris(w,board) bbox piece]] {}] return
  while {1} {
    if {$TkTetrisBlock([expr $TkTetrisPiece(0)+10]) || \
	$TkTetrisBlock([expr $TkTetrisPiece(1)+10]) || \
	$TkTetrisBlock([expr $TkTetrisPiece(2)+10]) || \
...
proc TkTetris:Left {} {
  ...
  if {$TkTetrisBlock([expr $TkTetrisPiece(0)-1]) || \
      $TkTetrisBlock([expr $TkTetrisPiece(1)-1]) || \
      $TkTetrisBlock([expr $TkTetrisPiece(2)-1]) || \
      $TkTetrisBlock([expr $TkTetrisPiece(3)-1])} {
    return
  } else {
    incr TkTetrisPiece(0) -1
    incr TkTetrisPiece(1) -1
    incr TkTetrisPiece(2) -1
    incr TkTetrisPiece(3) -1
    $TkTetris(w,board) move piece -$TkTetris(blocksize) 0
    TkTetris:Shadow
    update idletasks
  }
}
...
proc TkTetris:Right {} {
  ...
  if {[string match {} [set ix [$TkTetris(w,board) bbox piece]]] || \
      [lindex $ix 2] >= $TkTetris(width)} return
  if {$TkTetrisBlock([expr $TkTetrisPiece(0)+1]) || \
      $TkTetrisBlock([expr $TkTetrisPiece(1)+1]) || \
      $TkTetrisBlock([expr $TkTetrisPiece(2)+1]) || \
      $TkTetrisBlock([expr $TkTetrisPiece(3)+1])} {
    ...
    incr TkTetrisPiece(0)
    incr TkTetrisPiece(1)
    incr TkTetrisPiece(2)
    incr TkTetrisPiece(3)
    $TkTetris(w,board) move piece $TkTetris(blocksize) 0
    TkTetris:Shadow
    update idletasks
  }
...
proc TkTetris:Rotate dir {
  global TkTetris TkTetrisPiece TkTetrisBlock

  if [string match {} [set ix [$TkTetris(w,board) find with piece]]] return
  set m [$TkTetris(w,board) bbox piece]
  set x [file root [$TkTetris(w,board) canvasx \
			[expr ([lindex $m 2]+[lindex $m 0])/2] \
			$TkTetris(blocksize)]]
  set y [file root [$TkTetris(w,board) canvasy \
			[expr ([lindex $m 3]+[lindex $m 1])/2] \
			$TkTetris(blocksize)]]
  set flag 1
  foreach i $ix {
    set p [$TkTetris(w,board) coords $i]
    if [string comp left $dir] {
      set coord "[expr -[lindex $p 1]+$x+$y] [expr [lindex $p 0]-$x+$y] \
	  [expr -[lindex $p 3]+$x+$y] [expr [lindex $p 2]-$x+$y]"
    } else {
      set coord "[expr [lindex $p 1]+$x-$y] [expr -[lindex $p 0]+$x+$y] \
	  [expr [lindex $p 3]+$x-$y] [expr -[lindex $p 2]+$x+$y]"
    }
    if {[string match {} \
	[set n [eval $TkTetris(w,board) find enclosed $coord]]] || \
	$TkTetrisBlock([incr n -1])} {
      set flag 0
      break
    }
    set m [eval $TkTetris(w,board) find enclosed $p]
    incr m -1
    array set coords "$m {$i $coord} _$m $n"
  }
  if $flag {
    foreach i {0 1 2 3} {
      eval $TkTetris(w,board) coords $coords($TkTetrisPiece($i))
      set TkTetrisPiece($i) $coords(_$TkTetrisPiece($i))
    }
    TkTetris:Shadow
    update idletasks
  }
...
proc TkTetris:DropRows {} {
  global TkTetris TkTetrisBlock TkTetrisPiece
  set full {}
  foreach {i j} [array get TkTetrisPiece {[0-3]}] {
    if [set j [expr $j/10]] { set tmp($j) {} }
  }
  foreach i [array names tmp] {
    if {$TkTetrisBlock(${i}0) && $TkTetrisBlock(${i}1) &&
	$TkTetrisBlock(${i}2) && $TkTetrisBlock(${i}3) &&
	$TkTetrisBlock(${i}4) && $TkTetrisBlock(${i}5) &&
	$TkTetrisBlock(${i}6) && $TkTetrisBlock(${i}7) &&
	$TkTetrisBlock(${i}8) && $TkTetrisBlock(${i}9)} {
      lappend full $i
      array set TkTetrisBlock "${i}0 0 ${i}1 0 ${i}2 0 ${i}3 0 \
	${i}4 0 ${i}5 0 ${i}6 0 ${i}7 0 ${i}8 0 ${i}9 0"
    }
  }
  if [set i [llength $full]] {
    global TkTetris
    incr TkTetris(score) [expr round(pow($i,2))*($TkTetris(level)+1)]
    incr TkTetris(rows)  $i
    incr TkTetris(maxbrick) -$i
    if {[expr $TkTetris(rows)/10] > $TkTetris(level)} {
      incr TkTetris(level)
      $TkTetris(w,board) itemconfig back \
	  -fill $TkTetris(Color[expr $TkTetris(level)%7])
      bell
      set TkTetris(interval) [TkTetris:GetInterval $TkTetris(level)]
    }
    TkTetris:TellPlayers update $i $TkTetris(maxbrick) $TkTetris(deaths)

    foreach row [lsort -integer $full] {
      $TkTetris(w,board) delete row$row
      for {set i $row; incr i -1} {$i > 0} {incr i -1} {
	$TkTetris(w,board) move row$i 0 $TkTetris(blocksize)
	$TkTetris(w,board) addtag row[expr $i+1] with row$i
	$TkTetris(w,board) dtag row$i
      }
      update idletasks
      for {set i ${row}0} {$i > 0} {incr i -1} {
	if $TkTetrisBlock($i) {
	  set TkTetrisBlock([expr $i+10]) 1
	  set TkTetrisBlock($i) 0
	}
      }
    }
    update
  } elseif {$TkTetris(highband) && $TkTetris(numplayers)} {
    TkTetris:TellPlayers brick $TkTetris(maxbrick)
  }
}

proc TkTetris:AddRows {num} {
  global TkTetris TkTetrisBlock

  if {$num>4 || $num<2} return
  set w $TkTetris(w,board)
  $w delete piece

  ## Move pieces
  $w move struc 0 [expr $num*-$TkTetris(blocksize)]
  for {set i 0} {$i < 31} {incr i} {
    $w addtag row[expr $i-$num] with row$i
    $w dtag row$i
  }
  TkTetris:TellPlayers brick [incr TkTetris(maxbrick) $num]
  if {$TkTetris(maxbrick)>29} {
    TkTetris:GameOver
    return
  }
  ## Reassign TkTetrisBlock vars
  set move ${num}0
  for {set i 0} {$i<300} {incr i} {
    if $TkTetrisBlock($i) {
      set TkTetrisBlock([expr $i-$move]) 1
      set TkTetrisBlock($i) 0
    }
  }
  ...
  for {set i [expr 300-$move]} {$i < 300} {incr i} {
    if {[clock click]%2} {
      set TkTetrisBlock($i) 1
      set row [expr $i/10]
      $w create rectangle [expr ($i%10)*$TkTetris(blocksize)] \
	  [expr $row*$TkTetris(blocksize)] \
	  [expr ($i%10+1)*$TkTetris(blocksize)] \
	  [expr ($row+1)*$TkTetris(blocksize)] \
...
bind all <Alt-t> {
    catch "destroy .tetris"
    toplevel .tetris
    wm geometry .tetris +200+200
    global TkTetris
    TkTetris:Init .tetris
    if !$TkTetris(WWW) {
	wm deiconify .
