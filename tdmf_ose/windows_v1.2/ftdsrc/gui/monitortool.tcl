...
#
# Copyright (c) 1999 Legato Systems, Inc.
# All Rights Reserved.
#
#-----------------------------------------------------------------------------
...
proc monitor_check_info {{autoreschedule 1}} {
	...
	lappend monitor(lgfound) $lgname
        ...
        if {![catch "open $prfpath r" prffile]} {
            set size [file size $prfpath]
            if {$size > 0} {
                set devprfbuf [read $prffile]
                if {$devprfbuf == ""} {
                    set cfg(newprf) 1
                    close $prffile
                    continue
...
proc monitor_create_form {{top "."}} {
    ...
    wm iconwindow $top .a
    wm deiconify $top
    wm title $top "%Q%monitortool - %PRODUCTNAME% Monitor"
    if {$top == "."} {set top ""}
    # -- status line
...
proc monitor_readconfig {lgname} {
    ...
    if  {![info exists cfg(LOCRMDPORT:)]} { set cfg(LOCRMDPORT:) 575 }
	...
	switch -glob -- [lindex $line 0] {
	    ...
	    "PROFILE:" {set remark ""}
            ...
            "%PRODUCTNAME_TOKEN%-DEVICE:" { 
                regexp {ftd[0-9][0-9]*} [lindex $line 1] devbasename xxx
