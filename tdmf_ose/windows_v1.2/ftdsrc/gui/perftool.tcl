...
#-----------------------------------------------------------------------------
#
# ftdperftool -- FullTime Data Performance Monitor
#
# Copyright (c) 1997, 1998 FullTime Software, Inc.
# All Rights Reserved.
#
#-----------------------------------------------------------------------------
global env
...
proc ftdpt_build_form {} {
    ...
    toplevel .qf
    bind .qf <Destroy> {exit}
    toplevel .qfi
    bind .qfi <Destroy> {exit}
    ...
    wm deiconify .qf
    wm withdraw .qf
    wm title .qf "%PRODUCTNAME% Performance Monitor"
    ...
    set ftd(loggrpw) [tixComboBox .qf.f.f4.cb1 -label "Logical Group:  " \
...
proc ftdpt_create_chart {} {
    ...
    if {![info exists chart(inuseflag)]} {
	set chart(inuseflag) 0
    }
    if {![info exists chart(inuseflag)]} {set chart(inuseflag) 0}
    while {$chart(inuseflag)} {
	incr ftd(curchart) 
	if {$ftd(curchart) >= $ftd(chartcnt)} {incr ftd(chartcnt)}
	set chartname ""
	append chartname "chart" $ftd(curchart)
	global $chartname
	upvar #0 $chartname chart
...
#            set chart(TON,$vname) $name
	...
	if {$ftdpt_samples < $chart(samples)} {
	    ...
	    uplevel #0 "vector destroy tempvector"
	    uplevel #0 "vector destory tempvector2" 
	}
	...
	if {$ftdpt_charttype == "Bar" && $btype == "Stacked Bar"} {
	    ...
	    set destroygraph [bind $chart(gw) <Destroy>]
	    ...
	    bind $chart(gw) <Destroy> $destroygraph
...
proc ftdpt_print_file_chart {chartname} {
    global psfilename ftd $chartname
    upvar #0 $chartname chart
    set types {
	{{Postscript Files} {.ps}}
	{{All Files} *}
...
proc ftdpt_watch_for_data_updates {} {
	...
	foreach chartname $ftd(charts) {
	    global $chartname 
	    upvar #0 $chartname chart
	    if {![info exists chart(updateflag)]} {set chart(updateflag) 0}
	    if {$chart(updateflag)} {
		set chart(updateflag) 0
		foreach v $chart(vectors) {
