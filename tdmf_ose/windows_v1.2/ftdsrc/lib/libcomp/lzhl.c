...
int lzhl_compress_init(lzhl_t *lzhlp)
{
    ...
    if (!lzhlp->coder->stat->stat) {
        lzhlp->coder->stat->stat = calloc(NHUFFSYMBOLS, sizeof(HUFFINT));
    } else {
    ...
    if (!lzhlp->decoder->stat) {
        lzhlp->decoder->stat = calloc(1, sizeof(lzhl_decoderstat_t));
    } else {
    ...
    if (!lzhlp->decoder->stat->stat) {
        lzhlp->decoder->stat->stat = calloc(NHUFFSYMBOLS, sizeof(HUFFINT));
    } else {
        memset(lzhlp->decoder->stat->stat, 0, NHUFFSYMBOLS*sizeof(HUFFINT));
    }
    /* init buffer */
    if (!lzhlp->lzbuf->buf) {
        lzhlp->lzbuf->buf = calloc(1, LZBUFSIZE);
    } else {
...
{
    if (lzhlp->lzbuf->buf)
		free(lzhlp->lzbuf->buf);

	if (lzhlp->coder->stat->symbolTable)
		free(lzhlp->coder->stat->symbolTable);

	if (lzhlp->coder->stat->stat)
		free(lzhlp->coder->stat->stat);

    if (lzhlp->coder->stat)
		free(lzhlp->coder->stat);

	if (lzhlp->decoder->stat->symbolTable)
		free(lzhlp->decoder->stat->symbolTable);

	if (lzhlp->decoder->stat->stat)
		free(lzhlp->decoder->stat->stat);

    if (lzhlp->decoder->stat)
		free(lzhlp->decoder->stat);
}
    ...
    for (;;) {
        ...
        if (symbol < 256) {
            ...
            *lzhlp->target = (BYTE)symbol;
            lzhlp->target++;
            LZBUF_TOBUF_BYTE(lzhlp->lzbuf, symbol);
            continue; /* forever */
