...
long fdisksize(HANDLE fd)
{
    ...
    {
        char dbgbuf[256];
        sprintf(dbgbuf, "fdisksize(0x%x): %ld sectors hidden %ld\n", fd, (ppi.PartitionLength.QuadPart >> ftd_dev_bsize), ppi.HiddenSectors);
        OutputDebugString(dbgbuf);
    }
#endif
...
extern long fdisksize(char * szDir, HANDLE fd)
{
	...
	return (uliTotalBytes.QuadPart >> ftd_dev_bsize);
}
#endif
