	...
	while (TRUE) 
		...
		if (ftd_sock_check_send(fsockp, 1000) == 0) 
			...
			if (wrc < 0) 
				...
				UNSET_SOCK_WRITABLE(sockp->flags);
				errnum = sock_errno();
				error_tracef(TRACEERR, "ftd_sock_send - NET BROKEN3 - ERROR: %d %s , tid=%x",errnum,sock_strerror(errnum),GetCurrentThreadId()); 
				return FTD_LG_NET_BROKEN;
				} 
			else if (wrc < wlen) 
				{ 
	...
	while (TRUE) {
		...
		if (ftd_sock_check_send(fsockp, timeo_send) == 0) {
			...
			if (wrc < 0) {
				...
				// connection lost
				UNSET_SOCK_WRITABLE(sockp->flags);
           	    error_tracef(TRACEERR, "ftd_sock_send_lg() FTD_LG_NET_BROKEN3 !:<%x><%s><%s><%d> , tid=%x", 
                    sockp, sockp->lhostname, sockp->rhostname, sockp->port, GetCurrentThreadId() );
				return FTD_LG_NET_BROKEN;
			} else if (wrc < wlen) { 
				UNSET_SOCK_WRITABLE(sockp->flags);
	...
	struct iovec iov[], int iovcnt)
{
	...
	int				cnt, rc, wrc, wlen, len, len1, i, j;
	int				timeo_recv, timeo_send;
	char			*buf;
	...
	while (TRUE) {
		...
		if (ftd_sock_check_send(fsockp, timeo_send) == 0) {
			...
			if (wrc < 0) {
				...
				// connection lost
				UNSET_SOCK_WRITABLE(sockp->flags);
           	        error_tracef( TRACEERR, "ftd_sock_send_lg_vector() FTD_LG_NET_BROKEN3 !:<%x><%s><%s><%d>,tid=%x", 
                        sockp, sockp->lhostname, sockp->rhostname, sockp->port, GetCurrentThreadId() );
				return FTD_LG_NET_BROKEN;
			} else if (wrc < wlen) { 
				// partial write complete
...
ftd_sock_recv(ftd_sock_t *fsockp, char *buf, int len)
{
	...
	while (len > 0) 
			...
			if (errnum == ECONNRESET
				...
				if (fsockp->type == FTD_SOCK_INET) 
					...
					if (ftd_sock_test_link(sockp->lhostname,sockp->rhostname,sockp->lip,sockp->rip,
						...
						{
						error_tracef(TRACEERR, "ftd_sock_recv():NET BROKEN: %d %s, tid=%x", errnum,sock_strerror(errnum), GetCurrentThreadId());
						return FTD_LG_NET_BROKEN;
						}
					} 
				else 
					{
					error_tracef(TRACEERR, "ftd_sock_recv():NET BROKEN2 %d %s, tid=%x ", errnum,sock_strerror(errnum), GetCurrentThreadId());
					return FTD_LG_NET_BROKEN;
					}
				}
		...
		{
			errnum = sock_errno();
			error_tracef(TRACEERR, "ftd_sock_recv():NET BROKEN4 %d %s<%x><%x>, tid=%x", 
...
int ftd_sock_send_header(char tracing,char *mod,char *funct,ftd_sock_t *fsockp, ftd_header_t *header)
{
	...
	switch(header->msgtype) 
		...
		case FTDCRFBLK:
				...
				break;
		default:
				error_tracef(TRACEINF5, "ftd_sock_send_header(): %s.%s Tx hdr: %s tid=%x",(mod[0] == 'C' ? mod:who),funct,tdmf_socket_msg[header->msgtype-1].cmd_txt,GetCurrentThreadId());
				break;
		}
...
int ftd_sock_recv_header(char *mod,char *funct,ftd_sock_t *fsockp, ftd_header_t *header)
{
	...
	switch(header->msgtype) 
		...
		case FTDCCHKSUM:
		case FTDCRFBLK:
			error_tracef(TRACEINF5, "ftd_sock_recv_header():OK %s.%s Rx hdr: %s rc: %d",(mod[0] == 'C' ? mod:who),funct,
							(rc == sizeof(ftd_header_t) ? tdmf_socket_msg[header->msgtype-1].cmd_txt: "??"),
							(rc == sizeof(ftd_header_t) ? 0: rc));

				break;
		default:
			error_tracef(TRACEINF5, "ftd_sock_recv_header():%s.%s Rx hdr: %s rc: %d , tid=%x",(mod[0] == 'C' ? mod:who),funct,
							(rc == sizeof(ftd_header_t) ? tdmf_socket_msg[header->msgtype-1].cmd_txt: "??"),
	...
	ftd_lg_t *lgp) 
{
	...
	if ((devp = ftd_lg_devid_to_dev(lgp, rdev.devid)) == NULL) {
		// cant't find this dev id in secondary config
		reporterr(ERRFAC, M_MIRDEVID, ERRCRIT, devp->devid);
		return -1;
	}
	...
	if (0 == strcmp(rdev.path, devcfgp->sdevname)) {
		...
		size = devp->devsize;
		if (size == (u_long)-1) {
			reporterr(ERRFAC, M_MIRSIZ, ERRCRIT,
				devcfgp->sdevname, ftd_strerror());
			return -1;
		}
	} else {
	...
	ftd_lg_t *lgp) 
{
	...
	if (GET_JRN_MODE(lgp->jrnp->flags) == FTDMIRONLY) {
		...
		if (ftd_dev_write(devp, (char*)header->msg.lg.data,
			offset, len, devcfgp->sdevname) < 0) {
			return -1;
		}
	} else {
 ...
 */
int
ftd_sock_recv_rsync_end(int msgtype) 
{
	return FTD_CNORMAL;
}
...
ftd_sock_recv_lg_msg(ftd_sock_t *fsockp, ftd_lg_t *lgp, int timeo) 
{
	...
	case FTDCRFFEND:
	case FTDCBFEND:
		rc = ftd_sock_recv_rsync_end(header.msgtype);
		break;
	case FTDCRSYNCDEVS:
