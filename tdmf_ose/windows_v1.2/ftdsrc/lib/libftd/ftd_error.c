...
errfac_t *ERRFAC;
#else
__declspec( thread ) errfac_t *ERRFAC;
#endif
...
char *ftd_strerror(void)
{
	...
	GetLastErrorText( str_error, sizeof(str_error) );
	...
	if (strlen(str_error) == 0) {
		{	
			...
			if (!s) {
				sprintf(str_error, "Error %d", errnum);
			} else {
				strcpy(str_error, s);
			}
		}
...
int ftd_lockmsgbox(char *drive)
{
	...
	if (MessageBox ( 
		...
		str,
		PRODUCTNAME,
		MB_RETRYCANCEL | MB_ICONERROR | /* MB_SERVICE_NOTIFICATION */0x00200000L) == IDRETRY)
		return 0;
