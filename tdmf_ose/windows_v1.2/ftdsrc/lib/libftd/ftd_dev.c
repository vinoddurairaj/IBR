...
#if !defined(_WINDOWS)
	if (stat(rdevname, &statbuf) == 0) {
		devp->num = statbuf.st_rdev;
	}
	if (stat(devname, &statbuf) == 0) {
		devp->ftdnum = statbuf.st_rdev;
	}
#else
	devp->num = num;
	devp->ftdnum = num;
...
#if defined(_WINDOWS)
		if (modebits & O_EXCL) { /* flag to tell me dev_lock and share */
...
int ftd_dev_ftd_close(ftd_dev_t *devp)
{
	...
	{
#if defined(_WINDOWS)
		if (ftd_dev_unlock(devp->ftddevfd) == 0)
			ftd_close(devp->ftddevfd);
#else
		ftd_close(devp->ftddevfd);
#endif
		devp->ftddevfd = INVALID_HANDLE_VALUE;
	...
	ftd_dev_info_t *devp, dev_t cdev, int *lrdb, int *hrdb)
{
	...
	num32 = ((devp[i].lrdb_numbits / 32) + 1) < ((FTD_PS_HRDB_SIZE / 4) / expansion) ?
		((devp[i].lrdb_numbits / 32) + 1): ((FTD_PS_HRDB_SIZE / 4) / expansion);
	...
	int regen_hrdb, int *devidx, unsigned int size, ftd_dev_info_t *devp)
{
	...
	*devidx = index;
	...
	if (ftd_ioctl_new_device(ctlfd, &info, 1) != 0) {
		...
		if (!(grpstarted && errnum == EADDRINUSE)) {
			...
			sprintf(buf, "partition:%s, volume device:%s - %s",
				devname, info.vdevname, sock_strerror(errnum));
	...
	printf("\n$$$ start: hrdb = ");
	{
		unsigned char hrdbc[1024];
		int i;

		memcpy(hrdbc, hrdb, 1024);
		for(i=0;i<200;i++) {
			printf("%02x",hrdbc[i]);
		}
		printf("\n");
