...
static int parse_profile(ftd_lg_cfg_t *cfg, LineState *ls);
 ...
 */
int
ftd_config_read(ftd_lg_cfg_t *cfgp, int lgnum, int role, int startflag)
{
    LineState		lstate;
    int				rc = 0;
    ...
    while (1) {
		...
		if (strcmp("SYSTEM-TAG:", lstate.key) == 0) {
            ...
            if ((rc = parse_profile (cfgp, &lstate)) < 0) {
 ...
 */
int
ftd_config_read_2(ftd_lg_cfg_t *cfgp, int lgnum, int role)
{
    LineState		lstate;
    int				rc = 0;
    ...
    while (1) {
		...
		if (strcmp("SYSTEM-TAG:", lstate.key) == 0) {
            ...
            strcpy( cfgp->notes, lstate.readline+strlen("NOTES:") );
            continue;
        } else {
            /*reporterr(ERRFAC, M_CFGBDKEY, ERRCRIT, cfgp->cfgpath, 
                lstate.lineno, lstate.key);*/
...
config_get(char *cfgpath, int role, int startflag, LList *cfglist) 
{
	...
	case ROLEPRIMARY:
		if (startflag) {
			strcpy(cfg_path_suffix, PATH_STARTED_CFG_SUFFIX);
		} else {
	...
	if (startflag) {
		/* open ctl device */
		if ((ctlfd = ftd_open(FTD_CTLDEV, O_RDWR, 0)) ==
    ...
    while (bFound) {
		if (role == -1 
		|| 0 == strncmp(data.cFileName, cfg_path_prefix, strlen(cfg_path_prefix))) {
			...
			if (startflag) {
				{
					...
					if (ftd_ioctl_get_lg_state_buffer(ctlfd,
						cfgp->lgnum, buf, 1) < 0)
					{
						// delete the .cur file
						unlink(cfgp->cfgpath);
...
config_get(char *cfgpath, int role, int startflag, LList *cfglist) 
{
	DIR*		dfd;
	struct dirent*	dent;
	char		line[512], lgnumstr[4], cfg_path_suffix[4], *cfg_path_prefix;
	int		rc, i;
	ftd_lg_cfg_t	*cfgp = NULL;
	...
	if (dfd == (DIR*)NULL) {
		if (ftd_errno() == EMFILE) {
			reporterr(ERRFAC, M_FILE, ERRCRIT,
				PATH_CONFIG, ftd_strerror());
		}
		sprintf(line, "[error opening directory %s]", cfgpath);
			reporterr (ERRFAC, M_CFGFILE, ERRCRIT,
				cfgpath, ftd_strerror());
		return -1;
	}
	...
	while (NULL != (dent = readdir(dfd))) {
		...
		if (0 == (strncmp(&(dent->d_name[5]), cfg_path_suffix, 3))) { 
			if (role == -1 
			|| 0 == strncmp(dent->d_name,
				cfg_path_prefix, strlen(cfg_path_prefix))) {

				if ((cfgp = (ftd_lg_cfg_t*)
					calloc(1, sizeof(ftd_lg_cfg_t))) == NULL) {
					goto errret;
				}

				sprintf(cfgp->cfgpath, "%s/%s", cfgpath, dent->d_name);
				strncpy(lgnumstr, dent->d_name + strlen(cfg_path_prefix), 3);
				lgnumstr[3] = '\0';
...
parse_system(ftd_lg_cfg_t *cfgp, LineState *ls)
{
    ...
    while (1) {
        ...
        if (strcmp("HOST:", ls->key) == 0) {
            if (role == ROLEPRIMARY) {
				strcpy(cfgp->phostname, ls->p1);
			} else {
				strcpy(cfgp->shostname, ls->p1);
			}
		} else if (strcmp("PSTORE:", ls->key) == 0) {
            strcpy(cfgp->pstore, ls->p1);
        } else if (strcmp("JOURNAL:", ls->key) == 0) {
            strcpy(cfgp->jrnpath, ls->p1);
 ...
 */
static int
parse_profile(ftd_lg_cfg_t *cfgp, LineState *ls)
{
    ftd_dev_cfg_t	*devcfgp;
    char			*rest;
    ...
    while (moreprofiles) {
        ...
        while (1)  {
          ...
          if (0 == strcmp("REMARK:", ls->key)) {
            ...
            /* do nothing */
#if defined(_WINDOWS)
          } else if (0 == strcmp(CAPPRODUCTNAME "-DEVICE:", ls->key)) {
#else
          } else if (0 == strcmp(CAPPRODUCTNAME_TOKEN "-DEVICE:", ls->key)) {
#endif
            strcpy(devcfgp->devname, ls->p1);
            devcfgp->devid = parse_devnum(devcfgp->devname);
#if defined(_WINDOWS)
			devcfgp->devid = atoi(strchr(devcfgp->devname, ':') + 1);
			*(strchr(devcfgp->devname, ':') + 1) = '\0'; // get rid of the devid
#endif
          } else if (0 == strcmp("DATA-DISK:", ls->key)) {
			if (   ls->p1 == NULL || ls->p2 == NULL 
				|| ls->p3 == NULL || ls->p4 == NULL )
			{
			  return -1;
			}
            strcpy(devcfgp->pdevname, ls->p1); 
            strcpy(devcfgp->pdriverid, ls->p2);
            strcpy(devcfgp->ppartstartoffset, ls->p3);
            strcpy(devcfgp->ppartlength, ls->p4);
            ...
            strcpy(devcfgp->sdevname, ls->p1);
            strcpy(devcfgp->sdriverid, ls->p2);
            strcpy(devcfgp->spartstartoffset, ls->p3);
            strcpy(devcfgp->spartlength, ls->p4);
...
verify_primary_entries(ftd_lg_cfg_t *cfgp)
{
    ...
    ForEachLLElement(cfgp->devlist, devpp) {
        /* stat the data device and make sure it is a char device. */
        if (stat((*devpp)->pdevname, &statbuf) != 0) {
            reporterr(ERRFAC, M_STAT, ERRCRIT,
				(*devpp)->pdevname, ftd_strerror());
            return -1;
        }
        if (!S_ISCHR(statbuf.st_mode)) {
            reporterr(ERRFAC, M_DEVTYPERR, ERRCRIT,
				(*devpp)->pdevname, cfgp->cfgpath);
            return (-1);
        }
    }
...
#else
    ForEachLLElement(cfgp->devlist, devpp) {
		if (!QueryDosDevice((*devpp)->pdevname, (*devpp)->vdevname, MAXPATHLEN) ) {
            reporterr(ERRFAC, M_MIRSTAT, ERRCRIT,
				(*devpp)->pdevname, ftd_strerror());
            return -1;
        }
...
verify_secondary_entries(ftd_lg_cfg_t *cfgp)
{
    ...
    ForEachLLElement(cfgp->devlist, devpp) {
        ...
        if (stat((*devpp)->sdevname, &statbuf) != 0) {
			reporterr(ERRFAC, M_MIRSTAT, ERRCRIT,
				(*devpp)->sdevname, ftd_strerror());
            //return -1;
			return FTD_CFG_MIR_STAT;
		}
        if (!S_ISCHR(statbuf.st_mode)) {
            reporterr(ERRFAC, M_MIRTYPERR, ERRCRIT,
				(*devpp)->sdevname, cfgp->cfgpath);
			return FTD_CFG_MIR_TYPE;
            //return (-1);
        }
    }
#else
    ForEachLLElement(cfgp->devlist, devpp) {
		if (!QueryDosDevice((*devpp)->sdevname, (*devpp)->vdevname, MAXPATHLEN) ) {
