...
*/
    ...
    if (hndFile == INVALID_HANDLE_VALUE)
    {
		...
		error_tracef(TRACEINF5,"ftd_open(): ERROR=%d handle=%x ",err, hndFile);
    }
...
{                
	...
	int IoctlResult = DeviceIoControl(
	...
	{
		error_tracef(TRACEERR,"ftd_ioctl(): handle=%x  ioctl=%x", hndFile, ioctl);
		return -1;
    }
...
#ifdef _DEBUG
    sprintf(dbgbuf, "ftd_writev(): size =%ld\n", size);
    OutputDebugString(dbgbuf);
#endif
...
int ftd_close(HANDLE hndFile)
{
	...
	{
        DWORD err = GetLastError();
		error_tracef(TRACEERR,"ftd_close(): ERROR=%d handle=%x ", err, hndFile);
	}
