...
errfac_init(errfac_t *errfac) 
{
	...
	while (!feof(emsgfd)) {
...
#if defined(_WINDOWS)
		{
			...
			str = emsg.msg;
			{
				int qnmlen = strlen(QNM);

				while ( (str = strstr(str, "QNM")) != NULL) {
					memcpy(str, QNM, qnmlen);
					if (qnmlen < 3) {
						memmove(str + qnmlen, str + 3, strlen(str) - 3);
					}
				}
			}
		}
...
errfac_get_errmsg(errfac_t *errfac, char *name, char *msg)
{
	...
	sprintf(msg,
		"UNKNOWN " PRODUCTNAME " ERROR MESSAGE - FACILITY= %s NAME=%s",
