...
dumpstats (machine_t *sys)
{
    ...
    if (sys->tunables.stattofileflag == 0
    || sys->tunables.statinterval < 1) {
        return;
    }
...
flushstats(int force)
{
    ...
    || (mysys->tunables.statinterval > 0 && 
        ...
        if (mysys->tunables.stattofileflag != 0) {
...
initstats (machine_t *mysys)
{
    ...
    if (mysys->tunables.stattofileflag == 0 || mysys->tunables.statinterval < 1) {
        return;
    }
    ...
    DPRINTF((ERRFAC,LOG_INFO,"opened stats performance file %s\n", fil));
    (void)time(&statts);
#if defined(HPUX)
...
savestats(int force)
{
    ...
    if (ISPRIMARY(mysys)) {
        for (sd = group->headsddisk; sd; sd = sd->n) {
            ...
            sb.len = lgp->statsize;
            sb.addr = devbuf;
            rc = ioctl(mysys->ctlfd, FTD_GET_DEV_STATE_BUFFER, &sb);
            if (rc != 0) {
#ifdef DEBUG_THROTTLE
