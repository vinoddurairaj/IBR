...
ftd_stop_group(char *ps_name, int group, ps_version_1_attr_t *attr, int autostart)
{
    ...
    if (ioctl(lgfd, FTD_INIT_STOP, &sb) < 0) {
        if (errno == EBUSY) {
    ...
    if((err = ioctl(fd, FTD_GET_GROUP_STATS, &sb)) < 0) {
        exit(1);
    }
    ...
    sb.addr = (char *)&lg_info;
    sb.lg_num = group;
    if (ioctl(fd, FTD_GET_GROUPS_INFO, &sb) < 0) {
    ...
    if (gstat.state == FTD_MODE_REFRESH) {
        ioctl(fd, FTD_UPDATE_LRDBS, group);
        ioctl(fd, FTD_UPDATE_HRDBS, group);
    ...
    /* if we're in NORMAL mode, see if we have journal entries. 
       If so, update dirtybits and set mode to TRACKING */
        ...
        sb.lg_num = group;
        sb.addr = (caddr_t)&gstat;
        ioctl(fd, FTD_GET_GROUP_STATS, &sb);
        if (gstat.bab_used > 0) {
            ioctl(fd, FTD_CLEAR_LRDBS, group);
            ioctl(fd, FTD_CLEAR_HRDBS, group);
            ioctl(fd, FTD_UPDATE_LRDBS, group);
            ioctl(fd, FTD_UPDATE_HRDBS, group);
            new_state = FTD_MODE_TRACKING;
        } else {
    ...
    for (temp = mysys->group->headsddisk, i=0; temp != NULL; temp = temp->n, i++) {
        ...
        if (gstat.state != FTD_MODE_PASSTHRU) {
            ...
            db.devs = &statbuf.st_rdev;
            db.state_change = 0;
            if ((db.dbbuf = (int *)malloc(db.dblen32*4)) == NULL) {
            ...
            if (ioctl(lgfd, FTD_GET_LRDBS, &db) < 0) {
                free(db.dbbuf);
            ...
            if (ioctl(lgfd, FTD_GET_HRDBS, &db) < 0) {
                free(db.dbbuf);
        ...
        if ((sb.addr = (char *)malloc(sb.len)) == NULL) {
        ...
        /* get the attributes */
        if (ioctl(fd, FTD_GET_DEV_STATE_BUFFER, &sb) < 0) {
        ...
        if (ps_set_device_attr(ps_name, rawname, sb.addr, sb.len) != PS_OK) {
            free(sb.addr);
        ...
        if (ioctl(fd, FTD_DEL_DEVICE, &statbuf.st_rdev) < 0) {
    ...
    /* delete the group */
    /* if (ioctl(fd, FTD_DEL_LG, &lg_info.lgdev) < 0) { */
    if (ioctl(fd, FTD_DEL_LG, &group) < 0) {
