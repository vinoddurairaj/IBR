...
get_hires_dirtybits(group_t *group, int lgnum, ftd_stat_t *lgp) 
{
    ...
    sb.lg_num = lgnum;
    sb.addr = (caddr_t)dev;
    rc = ioctl(mysys->ctlfd, FTD_GET_DEVICES_INFO, &sb);
    if (rc != 0) {
        free(dev);
    ...
    if (db->dbbuf == NULL) {
        db->dbbuf = (int*)ftdmalloc(len*sizeof(int));
    }
    db->numdevs = lgp->ndevs;
    db->state_change = 0;

    rc = ioctl(group->devfd, FTD_GET_HRDBS, db);
    if (rc != 0) {
        free(dev);
        free(db->devs);
        free(db->dbbuf);
...
set_dirtybits(group_t *group, int lgnum, ftd_stat_t *lgp) 
{
    ...
    ldb->dblen32 = len;
    ldb->dbbuf = (int*)ftdmalloc(len*sizeof(int));
    memset((unsigned char*)ldb->dbbuf, 0xff, len*sizeof(int));
    rc = ioctl(group->devfd, FTD_SET_LRDBS, ldb);
    if (rc != 0) {
        free(dev);
        free(ldb->devs);
        free(ldb->dbbuf);
    ...
    rc = ioctl(group->devfd, FTD_SET_HRDBS, ldb);
    if (rc != 0) {
        free(dev);
        free(ldb->devs);
        free(ldb->dbbuf);
...
#endif
    if (ioctl(rsync->devfd, DKIOCINFO, &dkinfo) < 0) {
        return -1;
    }
    if (ioctl(rsync->devfd, DKIOCGVTOC, &dkvtoc) < 0) {
        return -1;
    }
...
rsyncinit(group_t *group) 
{
    ...
    if (_pmd_state == FTDRFD) {
        ...
        case FTD_MODE_PASSTHRU:
            ...
            rc = ioctl(mysys->ctlfd, FTD_SET_GROUP_STATE, &stb);
            if (rc != 0) {
...
rsynccleanup(int mode) 
{
    ...
    sb.addr = (char*)&lginfo;
    rc = ioctl(mysys->ctlfd, FTD_GET_GROUPS_INFO, &sb);
    if (rc != 0) {
    ...
    for (sd = group->headsddisk; sd; sd = sd->n) {
        ...
        sb.addr = devbuf;
        rc = ioctl(mysys->ctlfd, FTD_SET_DEV_STATE_BUFFER, &sb);
        if (rc != 0) {
...
refresh_full (void *args)
{
    ...
    while (!done) {
        ...
        for (sd = group->headsddisk; sd; sd = sd->n) {
            ...
            DPRINTF((ERRFAC,LOG_INFO,
            "\n*** refresh_f: offset, ackoff, delta, size = "\
                "%d %d, %d, %d\n",
                rsync->offset, rsync->ackoff,
                rsync->delta, rsync->size));
