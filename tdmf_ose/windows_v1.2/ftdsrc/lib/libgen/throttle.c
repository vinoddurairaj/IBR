...
update_throttle_measure_values ()
{
    ...
    measure_value[TOK_STATTOFILEFLAG].value = mysys->tunables.stattofileflag;
    ...
    measure_value[TOK_IODELAY].value = mysys->tunables.iodelay;
...
action_console_msg (char* message)
{
    ...
    logerrmsg (ERRFAC, 100000, "DATASTAR_THROTTLE_NOTICE", line);
    return 0;
}
...
action_update_tunable (int verbtok, int whattok, int value)
{
    ...
    case TOK_MAXSTATFILESIZE:
        if (val < 0) val = 0;
        mysys->tunables.maxstatfilesize = (int) (val/1024);
        break;
    case TOK_STATTOFILEFLAG:
        if (val != 0) val = 1;
        mysys->tunables.stattofileflag = val;
        break;
    ...
    fflush (throtfd);
#endif /* DEBUG_THROTTLE */
    (void) settunable (lgnum, key, valstr);
    return (0);
}
                    ...
                    if (0 != strncmp("%%", p1, 2)) {
                        ...
                        *pk = '\0';
                        ...
                        if (0 == strcmp(keyword, "PID")) {
                            ...
                            sprintf (tempstr, "%d", mysys->tunables.iodelay);
