...
update_lrdb(int lgnum)
{
    ...
    rc = ioctl(mysys->ctlfd, FTD_UPDATE_LRDBS, &lgnum);
    if (rc != 0) {
        reporterr(ERRFAC, "DRVERR", ERRWARN, "UPDATE_LRT", 
            strerror(errno));
        return -1;
    }
    return 0;
...
get_driver_mode(int lgnum)
{
    ...
    rc = ioctl(ctlfd, FTD_GET_GROUP_STATE, &lgstate);
    if (rc != 0) {
...
clearmode(int lgnum)
{
    ...
    if (chg_drv_state) {
        ...
        rc = ioctl(ctlfd, FTD_SET_GROUP_STATE, &lgstate);
        if (rc != 0) {
            if (errno != EINVAL) {
...
setmode(int lgnum, int mode) 
{
    ...
    /* get current driver mode */
    lgstate.lg_num = lgnum;
    rc = ioctl(mysys->ctlfd, FTD_GET_GROUP_STATE, &lgstate);
    if (rc != 0) {
        reporterr(ERRFAC, M_DRVERR, ERRWARN, "GET_GROUP_STATE", strerror(errno));
        return -1;
    }
...
*/
    ...
    if (chg_drv_state) {
        ...
        lgstate.lg_num = lgnum;
        lgstate.state = target_drv_state;
        rc = ioctl(mysys->ctlfd, FTD_SET_GROUP_STATE, &lgstate);
        if (rc != 0) {
            if (errno != EINVAL) {
                reporterr(ERRFAC, M_DRVERR, ERRWARN, "SET_GROUP_STATE", 
                    strerror(errno));
