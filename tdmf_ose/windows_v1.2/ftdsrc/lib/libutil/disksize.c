...
long fdisksize(HANDLE fd)
{
    ...
    if ( !GetVolumePartitionInfo(fd, &ppi) )
        return -1;

	if ( (ftd_dev_bsize = ftd_get_dev_bsize(fd)) == -1)
        return -1;

	sectors = (long)(ppi.PartitionLength.QuadPart >> ftd_dev_bsize);
/*
    do {
	    DWORD dwNumberOfBytesToRead = (1 << ftd_dev_bsize);
        char    buffer[2048];
        
        ppi.PartitionLength.LowPart = SetFilePointer(fd, ppi.PartitionLength.LowPart, &ppi.PartitionLength.HighPart, FILE_BEGIN);
        if ( (ppi.PartitionLength.LowPart == 0xFFFFFFFF) && (GetLastError() != NO_ERROR) )
            return -1;

	    if (!ReadFile(fd, 
		    buffer, 
            dwNumberOfBytesToRead,  // number of bytes to write ! not 512
        ...
        if (dwNumberOfBytesToRead == (1 << ftd_dev_bsize))
            return (long)( (ppi.PartitionLength.QuadPart >> ftd_dev_bsize) + 1);

        ppi.PartitionLength.QuadPart -= (1 << ftd_dev_bsize);
    } while (ppi.PartitionLength.QuadPart > 0);
*/
    return sectors;
}
#else
...
long disksize(char *fn)
{
	...
	sprintf(szVolumeName, TEXT("\\\\.\\%c:"), fn[0]);
...
//
    ...
    size = fdisksize(fd);
#else
    size = fdisksize(szVolumeName,fd);
#endif
