/* #ident "@(#)$Id: ftd_mngt_send_lg_state.c,v 1.21 2003/11/13 02:48:22 FJjapan Exp $" */
/*
	...
	if ((f = popen(cmdline, "r")) != NULL) {
		while (fgets(line, 384, f) != NULL) {
			...
			sscanf(line, "%s %s %s %s %s %s %s %s", 
			    parmition, link, owner, fsize, month, day, time, filename);
#else
...
getReplGrpSourceIP(int lgnum)
{
	...
	if ((f = popen(cmdline, "r")) != NULL) {
		while (NULL != fgets(line, MMP_MNGT_MAX_MACHINE_NAME_SZ, f)) {
			...
			ipstring_to_ip(line, (unsigned long *)&rgsip);
			memset(line, 0, sizeof(line));
		}
		pclose(f);
		return rgsip;
