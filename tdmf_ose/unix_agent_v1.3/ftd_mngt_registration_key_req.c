/* #ident "@(#)$Id: ftd_mngt_registration_key_req.c,v 1.14 2003/11/13 02:48:22 FJjapan Exp $" */
/* 
...
int ftd_mngt_registration_key_req(sock_t *sockID)
{
	...
	if ((Licfile = fopen(LIC_FILE,"r")) == NULL)
	{
		/* license file open error */
        	logout(4,F_ftd_mngt_registration_key_req,"unable to retreive current Replicator Agent registration key.\n");
		...
		while(!feof(Licfile) && NULL != fgets(licbuf, 256, Licfile)) {	
			...
			if (0 < strlen(licbuf)) {
				...
				if ('#' != licbuf[i] &&
				   ...
				   '\t' != licbuf[i] &&
				   '\n' != licbuf[i]) {		
				   if (0 == strncmp(licbuf, "DTC_LICENSE:", 12)) 
				   {
					    i=12;
				   }
...
void ftd_mngt_send_registration_key()
{
    ...
    sprintf(msg.keydata.szServerUID, "%x", my_gethostid() ); /* mtldev */
    strcpy(msg.keydata.szServerUID, gszServerUID);
