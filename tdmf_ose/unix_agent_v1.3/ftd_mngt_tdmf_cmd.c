/* #ident "@(#)$Id: ftd_mngt_tdmf_cmd.c,v 1.19 2003/11/13 02:48:22 FJjapan Exp $" */
/* 
...
int ftd_mngt_tdmf_cmd(sock_t *sockID)
{
    ...
    strcpy(szShortApplicationName, TDMF_TOOLS_PREFIX);
	...
	logout(9,F_ftd_mngt_tdmf_cmd,"command = cset\n");
 ...
 */
    ...
    /*******************************************************/
    if ( (pid = fork()) < 0 ) {
	logoutx(4, F_ftd_mngt_tdmf_cmd, "fork failed", "errno", errno);
        close(fd[0]);
        close(fd[1]);
	free (pCmdLine);
	pCmdLine = NULL;
	...
	execl("/bin/sh", "sh", "-c", pCmdLine, (char *)0);
	_exit(127);
    }
    else {
	...
	logoutx(19, F_ftd_mngt_tdmf_cmd, "command end", "exitcode", exitcode);
...
#else
    response.iStatus            = MMP_MNGT_TDMF_CMD_STATUS_OK;
#endif
...
void send_event(char *orgdata,int orglen)
{
    ...
    *(orgdata+i) = 0x00;
    i--;
    while ((!isprint(*(orgdata+i))) || *orgdata+i == '\n') {
        *(orgdata+i) = ' ';
        i--;
    }
    ...
    sprintf(stddata,
            "[%s] [proc: FTD_MNGT] [pid,ppid: %d, %d] [src,line: ftd_mngt_tdmf_cmd.c, 225] DTC: [INFO / COMMAND]: [%s]\n",
