...
#                   %VERSION%.
#
# Copyright (c) 2001 %COMPANYNAME% - All Rights Reserved.
#
#-----------------------------------------------------------------------------
...
proc migratecfgs {} {
    ...
    global MIG_INFO
    if {![chkmigrate%PKGNM%]} {
       # No longer migrating from Softek TDMF brand
       # chkmigrateFTSWftd
    }
...
for Softek %PRODUCTNAME% has been detected on this system.
Would you like to migrate them into the current 
Softek %PRODUCTNAME% environment?"
    } elseif {$MIG_INFO(FTSWftdflag)} {
        set rap "A previous set of configuration files
for Softek TDMF has been detected on this system.  
Would you like to have them converted and migrated into 
the current Softek %PRODUCTNAME% environment?"
    } else {
        destroy $w
        update
        return 0
...
Softek TDMF configuration files to Softek %PRODUCTNAME% format
...
#-----------------------------------------------------------------------------
# chkmigrateFTSWftd -- returns 1 if we need to migrate a Softek TDMF
#                      set of configuration files
#-----------------------------------------------------------------------------
proc chkmigrateFTSWftd {} {
    ...
    set newestdir ""
    set tname ""
    # -- see if there is an environment saved for Softek TDMF
...
#-----------------------------------------------------------------------------
# domigrateFTSWftd -- migrate Softek TDMF .cfg files
#-----------------------------------------------------------------------------
    ...
    foreach fdirname "[glob -nocomplain $MIG_INFO(fromdir)/*.cfg]" {
        ...
        if {![catch {open $fdirname r} fd]} {
            ...
            while {![eof $fd]} {
                ...
                puts $ofd "$line"
                if {$linecount == 2} {
                    puts $ofd "#  Softek %PRODUCTNAME% Version %VERSION%"
                }
