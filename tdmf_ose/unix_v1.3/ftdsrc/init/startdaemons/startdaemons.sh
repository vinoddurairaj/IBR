...
#
# This script will start the daemons for the Softek %PRODUCTNAME%
...
case "$1" in
'start_msg')
    echo "Start Softek %PRODUCTNAME% daemons"
    ;;
'stop_msg')
    echo "Stop Softek %PRODUCTNAME% daemons"
    ;;
'start')		
    # Start the PMDs	
    echo "Starting Softek %PRODUCTNAME% daemons"
    /usr/bin/nohup /%OPTDIR%/%PKGNM%/bin/in.%Q% >/tmp/in.%Q%.log 2>&1 &
    /bin/sleep 5
    /usr/bin/nohup /%OPTDIR%/%PKGNM%/bin/launchpmds >/tmp/in.pmd.log 2>&1 &
    ;;
'stop')
    echo "Stopping Softek %PRODUCTNAME% daemons"
