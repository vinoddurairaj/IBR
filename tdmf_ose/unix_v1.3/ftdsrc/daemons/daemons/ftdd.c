...
extern char** pmdenviron = (char**)NULL;
extern int pmdenvcnt = 0;
...
readfirstitem (int fd, int pmdrequest, unsigned char *cmd)
{
    ...
    if (header->cmd == CMDVERSION) {
        ...
        pmdenviron[pmdenvcnt] = (char*)ftdmalloc(sizeof(char)*(strlen(tbuf)));
        strcpy(pmdenviron[pmdenvcnt++], tbuf);
...
installsigaction(void)
{
    ...
    sigset_t block_mask;
    static int msignal[] = { SIGALRM, SIGHUP, SIGCHLD, SIGTERM };
...
main (int argc, char** argv, char **envp)
{
    ...
    i = 0;
    while (envp[i]) {
        pmdenviron[i] = (char*)ftdmalloc((1+strlen(envp[i]))*sizeof(char));
        strcpy(pmdenviron[i], envp[i]);
        i++;	
    }
