...
int setdrvrtunables (int lgnum, tunable_t *tunables) 
{
    ...
    statbuf.addr = (ftd_uint64ptr_t)tunables;
...
int verify_and_set_tunable(char *key, char *value, tunable_t *tunables) {
    ...
    if (strcmp("CHUNKSIZE:", key) == 0) {
        ...
        intval = atoi(value);
        if ( intval == 0 ) {
            reporterr(ERRFAC, M_SYNCDEPTH, ERRCRIT, intval, UINT_MAX, 1);
            return -1;
        }
        ...
        intval = atoi(value);
        if ( intval < 1 ) {
            reporterr(ERRFAC, M_SYNCTIMEOUT, ERRCRIT, intval, INT_MAX, 1);
            return -1;
        }
...
getdrvrtunables(int lgnum, tunable_t *tunables)
{
    ...
    statbuf.addr = (ftd_uint64ptr_t)lgbuf;
...
*/
    ...
    memcpy(tunables, (const void *)statbuf.addr, sizeof(tunable_t));
    free(lgbuf);
    close(master);
...
#ifdef TDMF_TRACE
	DPRINTF("DTC %s.%s() getconfigs(): w:%d start:%d ", mod,funct,who,startflag);	
#endif
    i = 0;
    ...
    while (curdev != NULL) {
        ...
        if (openflag) {
            ...
            do {
                ...
                if (FTD_IOCTL_CALL(curdev->mirfd, DKIOCGVTOC, &dkvtoc) < 0) {
                    break;
                }
