...
*/
        ...
        rc = unlock_journal(jrn_offset+lock_len, (~(lock_len)+1));
        if (rc == -1) {
            reporterr(ERRFAC, "JRNUNLOCK", ERRCRIT,
...
getchunk (group_t *group) 
{
    ...
    oe->addr = (ftd_uint64ptr_t)group->chunk;
