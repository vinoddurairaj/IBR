...
#elif defined(SOLARIS) || defined(_AIX) || defined(HPUX)
      bablen = 0;
...
#else
	  buf = (bab_buffer_t *) kmem_alloc (chunk_size * sizeof (ftd_uint64_t), KM_NOSLEEP);
#endif
	  if (buf == NULL)
	    {
	      if (i < 2)
		{
	      ...
	      /* Free the last two chunks, since we ran the system of memory. */
	      giveback = i >> 2;
	      if (giveback < 2)
		giveback = i == 2 ? 1 : 2;
	      ...
	      for (j = 0; j < giveback; j++)
		{
		  ...
		  kmem_free (buf, chunk_size * sizeof (ftd_uint64_t));
		}
...
#if defined(SOLARIS) || defined(_AIX) || (defined(HPUX) && !defined(STATIC_BAB_MALLOC))
...
ftd_bab_free_mgr (bab_mgr_t * mgr)
{
  ...
  *temp = mgr->next;
  ...
  /* XXX: fixme. adjust the bab_mgr limits now that we have one less to deal with */
...
/* save the pending pointers, just in case we return error after allocating a buffer */
  ...
  tag=1;
    ...
    if(total > ((sizeof_64bit(stat_buffer_t))+(sizeof_64bit(wlheader_t)))) 
   {
      len64 = len64 - total;
 ...
 */
FTD_PUBLIC ftd_int32_t
ftd_bab_copy_out (bab_mgr_t * mgr, ftd_int32_t offset64, ftd_uint64_t * addr, ftd_int32_t len64)
{
  ftd_uint64_t templen;
  bab_buffer_t *temp;
  ftd_uint64_t *start;
#if defined(HPUX)
  ftd_context_t context;
