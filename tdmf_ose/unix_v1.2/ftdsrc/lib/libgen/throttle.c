...
********************************************************* {COPYRIGHT-END} **/
/***************************************************************************
 * throttle.c - FullTime Data throttle implementation file
 *
 * (c) Copyright 1997 FullTime Software, Inc. All Rights Reserved
 *
 * History:
 *   3/1/97 - Steve Wahl - original code
 *
 ***************************************************************************
 */
...
int exec_action (action_t* action);
...
parse_dowdom (char* token, throttle_t* throt)
{
	...
	if (token[i] >= '0' && token[i] <= '9') {
            throt->day_of_week_mask = 0;/*WR15789*/	
		...
		c1 = c;
	        ...
	        throt->day_of_month_mask = 0;/*WR15789*/ 
                wday = -1;
                if (c1 == 'e' && c == 'm') {
 ...
 **************************************************************************/
int 
eval_dom (throttle_t* throttle, int dom)
{
    if (throttle->end_of_month_flag == 1 && mysys->lastdayofmon == mysys->day) {
    ...
    if (throttle->day_of_month_mask == -1 ||
        (0 != (throttle->day_of_month_mask & (1 << (mysys->day-1))))) {
#ifdef DEBUG_THROTTLE
...
eval_throttles ()
{
    ...
    if (mysys->throttles == (throttle_t*) NULL) return;
    /* -- test for new day rollover */
    timst1 = localtime (&mysys->group->throtstats.statts);
    mysys->day = timst1->tm_mday;
    mysys->wday = timst1->tm_wday;
    mysys->lastdayofmon = ldom[timst1->tm_mon];
    if (timst1->tm_mon == 1 && ((1900+timst1->tm_year)%4) == 0) 
        mysys->lastdayofmon = 29;
    memcpy ((void*)&timst2, (void*)timst1, sizeof(timst2));
    timst2.tm_sec = 0;
    timst2.tm_min = 0;
    timst2.tm_hour = 0;
    tim = mktime (&timst2);
    if (tim != mysys->midnight) mysys->midnight = tim;
    ...
    update_throttle_measure_values ();
    ...
    throttle = mysys->throttles;
    while (throttle) {
        /*===== day-of-week, day-of-month, and time period processing ===== */
        if (0 == eval_dow(throttle, mysys->wday) &&
            0 == eval_dom(throttle, mysys->day)) {
#ifdef DEBUG_THROTTLE
	...
	tim = mysys->group->throtstats.statts - mysys->midnight;
	...
	if (throttle->to != (time_t) -1) {
        ...
        if (1 == prevtest) {
            for (i=0; i<throttle->num_actions; i++) {
                ...
                if (mysys->tunables.tracethrottle) 
                    printthrottletrace (throttle, action);
                (void) exec_action (action);
            }
        }
        throttle = throttle->n;
    }
...
update_throttle_measure_values ()
{
    ...
    /* -- get current values -- */
    measure_value[TOK_NETKBPS].value = (int) (.5 + group->throtstats.effectkbps);
    measure_value[TOK_PCTCPU].value = group->throtstats.percentcpu;
    measure_value[TOK_PCTWL].value = group->throtstats.pctbab;
    measure_value[TOK_NETCONNECTFLAG].value = group->throtstats.pmdup;
    measure_value[TOK_PID].value = group->throtstats.pid;
    measure_value[TOK_CHUNKSIZE].value = mysys->tunables.chunksize;
    measure_value[TOK_STATINTERVAL].value = mysys->tunables.statinterval;
    measure_value[TOK_MAXSTATFILESIZE].value = mysys->tunables.maxstatfilesize;
    measure_value[TOK_TRACETHROTTLE].value = mysys->tunables.tracethrottle;
    measure_value[TOK_SYNCMODE].value = mysys->tunables.syncmode;
    measure_value[TOK_SYNCMODEDEPTH].value = mysys->tunables.syncmodedepth;
    measure_value[TOK_SYNCMODETIMEOUT].value = mysys->tunables.syncmodetimeout;
    measure_value[TOK_COMPRESSION].value = mysys->tunables.compression;
    measure_value[TOK_TCPWINDOW].value = mysys->tunables.tcpwindowsize;
    measure_value[TOK_NETMAXKBPS].value = mysys->tunables.netmaxkbps;
    measure_value[TOK_CHUNKDELAY].value = mysys->tunables.chunkdelay;
    measure_value[TOK_DRVMODE].value = group->throtstats.drvmode;
    measure_value[TOK_ACTUALKBPS].value = (int) (.5 + group->throtstats.actualkbps);
    measure_value[TOK_EFFECTKBPS].value = (int) (.5 + group->throtstats.effectkbps);
    measure_value[TOK_PCTDONE].value = (int) (.5 + group->throtstats.pctdone);
    measure_value[TOK_ENTRIES].value = group->throtstats.entries;
    measure_value[TOK_PCTBAB].value = group->throtstats.pctbab;
    measure_value[TOK_READKBPS].value = (int) (.5 + group->throtstats.local_kbps_read);
    measure_value[TOK_WRITEKBPS].value = (int) (.5 + group->throtstats.local_kbps_written);
#ifdef DEBUG_THROTTLE 
    fprintf (throtfd, "  (in update_throttle_measure_values)\n");
    fprintf (throtfd, "     NETKBPS=%d PCTCPU=%d PCTWL=%d NETCONNECTFLAG=%d PID=%d CHUNKSIZE=%d\n     STATINTERVAL=%d MAXSTATFILESIZE=%d TRACETHROTTLE=%d \n     SYNCMODE=%d SYNCMODEDEPTH=%d SYNCMODETIMEOUT=%d COMPRESSION=%d TCPWINDOW=%d\n     NETMAXKBPS=%d CHUNKDELAY=%d IODELAY=%d DRVMODE=%d ACTUALKBPS=%d EFFECTKBPS=%d PCTDONE=%d ENTRIES=%d PCTBAB=%d READKBPS=%d WRITEKBPS=%d\n",
...
action_console_msg (char* message)
{
    ...
    sprintf (name, "%s_THROTTLE_NOTICE", PRODUCTNAME);
    logerrmsg (ERRFAC, 100000, name, line);
    return 0;
}
...
#endif /* DEBUG_THROTTLE */
#if defined(linux)
    sprintf (cmd, "/bin/mail -s \"%s PMD THROTTLE NOTICE\" %s", PRODUCTNAME, whoto);
#else
    sprintf (cmd, "/bin/mailx -s \"%s PMD THROTTLE NOTICE\" %s", PRODUCTNAME, whoto);
#endif /* defined(linux) */
#if defined(SOLARIS)
    for(i=0; i<4; i++)
        newfd[i] = fcntl(dummyfd_throtd[i], F_DUPFD, 0);
    for(i=0; i<4; i++)
        close(dummyfd_throtd[i]);
#endif
    if ((fd = popen (cmd, "w")) == NULL) {
	if (errno == EMFILE)
	    reporterr(ERRFAC, M_FILE, ERRCRIT, "pipe", strerror(errno));
    }
    if (message != NULL) {
...
/***************************************************************************
 * wait_exec -- The signal handler called
 *                                after a child process is completed
 **************************************************************************/
void
wait_exec(int sig)
{
    while( waitpid(-1, NULL, WNOHANG) > 0 );
    signal(SIGCHLD, wait_exec);
}
 ...
 **************************************************************************/
int
action_update_tunable (int verbtok, int whattok, int value)
{
    int lgnum;
    char key[256];
    char valstr[256];
    ...
    fflush (throtfd);
#endif /* DEBUG_THROTTLE */
    longlongval = measure_value[whattok].value;
    switch (verbtok) {
    case VERB_SET:
	val = value;
	break;
    ...
    case TOK_CHUNKSIZE:
        ...
        measure_value[whattok].value = val;
        val = (int)(val/1024);
        mysys->tunables.chunksize = (int)val;
        break;
    case TOK_STATINTERVAL:
        if (val < 1) val = 1;			/* WR15349 */
        if (val > 86400) val = 86400;
        measure_value[whattok].value = val;
        mysys->tunables.statinterval = val;
        break;
    case TOK_MAXSTATFILESIZE:
        if (val < 1024) val = 1024;
        if (val > 32768000) val = 32768000;
        measure_value[whattok].value = val;
        val = (int)(val/1024);
        mysys->tunables.maxstatfilesize = (int) val;
        break;
    case TOK_TRACETHROTTLE:
        if (val < 0) val = 0;
        if (val > 1) val = 1;
        measure_value[whattok].value = val;
        mysys->tunables.tracethrottle = val;
        break;
    case TOK_SYNCMODE:
        if (val < 0) val = 0;
        if (val > 1) val = 1;
        measure_value[whattok].value = val;
        mysys->tunables.syncmode = val;
        break;
    case TOK_SYNCMODEDEPTH:
        if (val < 1) val = 1;
        measure_value[whattok].value = val;
        mysys->tunables.syncmodedepth = val;
        break;
    case TOK_SYNCMODETIMEOUT:
        if (val < 1) val = 1;
        if (val > 86400) val = 86400;
        measure_value[whattok].value = val;
        mysys->tunables.syncmodetimeout = val;
        break;
    case TOK_COMPRESSION:
        if (val < 0) val = 0;
        if (val > 1) val = 1;
        measure_value[whattok].value = val;
        mysys->tunables.compression = val;
        break;
    case TOK_NETMAXKBPS:
        if (val != -1 && val < 1 ) val = 1;
        measure_value[whattok].value = val;
        mysys->tunables.netmaxkbps = val;
        break;
    case TOK_CHUNKDELAY:
        if (val < 0) val = 0;
        if (val > 999) val = 999;
        measure_value[whattok].value = val;
        mysys->tunables.chunkdelay = val;
        break;
    default:
        return (0);
    }
    ...
    fprintf (throtfd, "\n>>>>> settunable lgnum=%d key=[%s] valuestring=[%s]\n\n",
             lgnum, key, valstr);
    fflush (throtfd);
#endif /* DEBUG_THROTTLE */
    (void) settunable (lgnum, key, valstr, 0x11);
    return (0);
}
...
exec_action (action_t* action)
{
    ...
    ostrlen = 1024;
    (void) substitute_actionstring (action->actionstring, ostring, &ostrlen);
    ...
    case ACTION_SLEEP:
        ...
        fflush (throtfd);
#endif /* DEBUG_THROTTLE */
        return (action_update_tunable (action->actionverb_tok, 
                                       TOK_CHUNKDELAY, 
                                       action->actionvalue));
    case ACTION_IODELAY:
#ifdef DEBUG_THROTTLE
    ...
    case TOK_CHUNKSIZE:
        ...
        return (action_update_tunable (action->actionverb_tok, 
                                       ...
                                       ((long long)action->actionvalue * 1024) > INT_MAX ? INT_MAX : ((long long)action->actionvalue * 1024) < INT_MIN ? INT_MIN : action->actionvalue * 1024));
    case TOK_STATINTERVAL:
#ifdef DEBUG_THROTTLE
 ...
 **************************************************************************/
int 
substitute_actionstring (char* instr, char* outstr, int* outsize)
{
    char* p1;
    char* p2;
    char* pk;
                    ...
                    if (0 != strncmp("%%", p1, 2)) {
                        ...
                        *pk = '\0';
                        ...
                        if (0 == strcmp(keyword, "PID")) {
                            sprintf(tempstr, "%d", lg->throtstats.pid);
 ...
 **************************************************************************/
void
printaction (char* outstr, action_t* action)
{
    int j;
    int outlen;
    int i;
    ...
    (void) substitute_actionstring (action->actionstring, outbuf, &outlen);
 ...
 **************************************************************************/
void
printthrottletrace (throttle_t* throttle, action_t* action)
{
    char otest[256];
    char oaction[256];
    ...
    /* -- figure out our logical group number */
    len = strlen(mysys->configpath);
    lg = (mysys->configpath[len-7] - '0') * 100;
    lg += (mysys->configpath[len-6] - '0') * 10;
    lg += mysys->configpath[len-5] - '0';
    ...
    printaction (oaction, action);
    
    reporterr (ERRFAC, M_THROTTRC, ERRWARN, lg, 
               otest, oaction);
...
#ifdef TDMF_TRACE
/***************************************************************************
 * printthrottle -- prints a throttle suitable for debugging
 **************************************************************************/
void
printthrottle (FILE* fd, throttle_t* throttle)
{
    char ostr[256];
    int j;
    int i;
    ...
    ostr[0] = '\0';
    j = strlen(ostr);
    strcpy (ostr, "THROTTLE ");
    j = strlen(ostr);
    printdowdom (&ostr[j], throttle);
    j = strlen(ostr);
    print_time (&ostr[j], throttle->from);
    j = strlen(ostr);
    print_time (&ostr[j], throttle->to);
    j = strlen(ostr);
    for (i = 0; i < throttle->num_throttest; i++) {
        ttest = &(throttle->throttest[i]);
        j = strlen(ostr);
        printmeasure (&ostr[j], ttest);
        j = strlen(ostr);
        printrelop (&ostr[j], ttest);
        j = strlen(ostr);
        printvalue (&ostr[j], ttest);
        j = strlen(ostr);
        printlogop (&ostr[j], ttest);
    }
    fprintf (fd, "%s\n", ostr);
    fprintf (fd, "    ACTIONLIST\n");
    for (i=0; i<throttle->num_actions; i++) {
        action = &throttle->actions[i];
        strcpy (ostr, "        ACTION:  ");
        j = strlen(ostr);
        printaction (&ostr[j], action);
        fprintf (fd, "%s\n", ostr);
    }
    fprintf (fd, "    ENDACTIONLIST\n");
}
