...
sig_action(int lgnum)
{
    ...
    ftd_trace_flow(FTD_DBG_FLOW1,
            "lg(%d) _pmd_state = %d, action = %d\n", lgnum, _pmd_state, action);
    ...
    case FTDRFDF:
        ...
        if (action == FTDQRFD) {
            ...
            _pmd_state = FTDRFD;
            _pmd_state_change = 1;
        /* FRF - Full Refresh Restart -- Added FTDQRFDR signal */
        } else if (action == FTDQRFDF || action == FTDQRFDR ) {
            /* start FULL refresh */
            _pmd_state = FTDRFDF;
            _pmd_state_change = 1;
    ...
    default:
        ...
        case FTDQRFDF:
            ...
            _pmd_state_change = 1;
            break;
        /* FRF: On running the launchrefresh -ra when the system is in
         * NORMAL mode, start the full refresh from zero % 
	 */
        case FTDQRFDR:
            /* start FULL refresh */
            _pmd_state = FTDRFDF;
 ...
 ****************************************************************************/
void
pmdrsync(int sig, char **pmdargv, int *net_analysis_flags)
{
    pid_t pid;
    char pname[16];
    int lgnum;
    ...
    for (i = 0; i < lgcnt; i++) {
        ...
        if (targets[lgnum]) {
            ...
            if ((pid = getprocessid(pname, 1, &pcnt)) > 0) {
                ...
                pid = startpmd(lgnum, sig, pmdargv);
				if( sig == FTDQNETANALYSIS )
				{
				    // Take note of PMDs started in network bandwidth analysis mode
					net_analysis_flags[lgnum] = 1;
				}
				else
				{
					net_analysis_flags[lgnum] = 0;
				}
            }
        }
    }
...
checkpoint(int cnt, char **pmdargv)
{
    ...
    ftd_trace_flow(FTD_DBG_FLOW1, "enter\n");
    ...
    for (i = 0; i < cnt; i++) {
        ...
        switch(sig) {
            ...
            case FTDQCPONS:
                ...
                lgcnt = GETCONFIGS(tmpcpaths, 2, 0);
                paths = (char*)tmpcpaths;
                cfgidx = numtocfg(lgnum);
                ...
                break;
            case FTDQCPOFFP:
                ftd_trace_flow(FTD_DBG_FLOW1, "FTDQCPOFFP\n");
                /* turn checkpoint off from primary */ 
                sprintf(pname, "PMD_%03d", lgnum);
                if ((pid = getprocessid(pname, 1, &pcnt)) > 0) {
                    if (ipcsend_sig(p_cp[lgnum].fd[1][STDOUT], 0, &sig) == -1) {
                        ftd_trace_flow(FTD_DBG_FLOW1, "ipc send failed\n");
                       continue; 
                    }
                } else {
                    reporterr(ERRFAC, M_NOPMD, ERRWARN, paths+(cfgidx*32), pname);
                    ipcsend_ack(ftdsock);
                    ftd_trace_flow(FTD_DBG_FLOW1, "ipc send ack\n");
                }
                break;
            case FTDQCPOFFS:
                ...
                if ((pid = getprocessid(pname, 1, &pcnt)) > 0) {
                    ...
                    if (jrncnt > 0) {
                        sprintf(prefix, FTD_JRNLGNUM_PREFIX_FMT, lgnum);
                        prefix_len = strlen(prefix);
                        cp_found = 0;
                        for (j = 0; j < jrncnt; j++) {
			    jrnname = jrnpath_get(jrnphp, j);
                            if (strncmp(jrnname, prefix, prefix_len)) {
                                continue;
                            }
                            if (cp_file(jrnname, CP_ON)) {
                                sprintf(path, "%s/%s", mysys->group->journal_path, jrnname);
                                UNLINK(path);
                                cp_found = 1;
                            }         
                        }
 ...
 ****************************************************************************/
void
rmdapply(int sig, char **pmdargv, pid_t rmda_pid[])
{
    pid_t pid = -1;
    char pname[16];
    int lgnum;
    ...
    for (i = 0; i < lgcnt; i++) {
        ...
        if (targets[lgnum]) {
            ...
            if ((pid = getprocessid(pname, 1, &pcnt)) <= 0) {
                ...
                if (pid > 0) {
                    rmda_pid[lgnum] = pid;
		    pid_hash_add( pid, lgnum, LGARRY_RMDA );
		}
            }
        }
    }
...
start_rmdapply(int waitflag)
{
    ...
    // If we have flagged corrupted journals, we'll refuse to apply them until we've recovered.
    if (check_corrupted_journal_flag(lgnum))
    {
        reporterr (ERRFAC, M_RMDA_CORR_SKIP, ERRWARN);
        return ;
    }
    ...
    */ 
    ...
    // This is a very trivial and imperfect implementation of waiting for the completion of the RMDA,
    // but considering that there wasn't anything done at all before, this isn't so bad!
    if (waitflag)
    {
        int seconds_waited_for_rmda;

        // We wait up to 5 seconds for the RMDA to get started.
        // Of course, it's possible that the RMDA starts and completes between two checks,
        // or that it only gets started after 5 seconds, but covering most cases is still better than
        // none at all like was done before.
        for(seconds_waited_for_rmda = 0; seconds_waited_for_rmda <= 5; seconds_waited_for_rmda++)
        {
            if (is_rmdapply_running())
            {
                break;
            }
            else
        ...
        // And then we wait for it to complete.
        while(is_rmdapply_running())
        {
            sleep(1);
        }
    }
...
/**
 * @brief Checks if the rmda is running or not.
 *
 * @return 1 if running, 0 if not.
 */
int is_rmdapply_running()
{
    pid_t pid;
    char pname[32];
    int pcnt;
...
connecttomaster(char* masterpath, int rmdflag)
{
    ...
    tries = 0;
    maxtries = 120; /* 1LG-ManyDEV ManyLG-1dtc (original was 20) */ 
    seltime->tv_sec = 0;
...
ipcrecv_sig(int fd, int *cnt)
{
    ...
    if ((rc = read(fd, (char*)header, n)) != n) {
        ...
        if (rc == -1) {
            ...
            EXIT(EXITNETWORK); /* hack to keep RMD from looping on this error */
        }
        rc = -1;
    }
