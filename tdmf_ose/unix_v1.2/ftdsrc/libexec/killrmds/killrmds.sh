...
# This particular version prints the process's argv0 value (RMD_??) by
# default (like -ef does) rather than the name of the binary that was
# given to exec when the master agent forked and executed the pmds (in.rmd).
...
RMD_REGEXP='RMD_[0-9]+|in\.rmd'
PIDS=`/bin/ps -e | /bin/egrep $RMD_REGEXP | /bin/awk '{print $1}'`
