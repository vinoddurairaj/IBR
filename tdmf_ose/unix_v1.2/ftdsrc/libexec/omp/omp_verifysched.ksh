...
#
# $Id: omp_verifysched.ksh,v 1.3 2008/12/28 10:12:16 naat0 Exp $
#
...
function display
{
	...
	Usage)
	    severity="USAGE"
	    no_log_output
	    message[0]="${PROGNAME} - Calls omp_genconf with the -v option"
	    ;;
...
[[ -x "./omp_genconf.awk" ]] && {
    ./omp_genconf.awk -v ValidateOnly=true
    exit $?
}
[[ -x "${OMPDIR_BIN}/omp_genconf.awk" ]] && {
    ${OMPDIR_BIN}/omp_genconf.awk -v ValidateOnly=true
    exit $?
}
