...
# Originaly, this script was designed using ksh. 
# On Redhat 6, ksh is no longer a default package installed.
# So we converted the script using sh but now it's requiring 
# to enable regular expresion option in the environment.
shopt -s extglob

# Expends backslash-excape by default
shopt -s xpg_echo
...
function raw_to_block
{
    ...
    if [[ $source = */rdsk/* ]]
    then
	echo ${source%/rdsk/*}/dsk/${source#*/rdsk/}
    elif [[ $source = */dsk/* ]]
    then
	echo $source
    else
...
ins=$(ps -ef | grep -v grep | grep -v sh | egrep ${CMD} | wc -l)
...
fstablist=(`ls -1d $prefixFSTAB+([0-9]) 2>/dev/null | sort -t. -k 3nr`)
