...
usage(void)
{
    ...
    fprintf(stderr, "            [state [passthru|normal|tracking]] ]\n");
}
...
set_state(int group, int state)
{
...
#ifdef ALLOW_GOING_TO_BACKFRESH
	...
	/* Last argument (clear_ODM, 0) specifies to leave ODM device entries ; fourth argument
	(also 0) is for autostart (it was missing) */
        ftd_stop_group(ps_name, group, &attr, 0, 0);
        return 0;
    } 
#endif
    ...
    ret = FTD_IOCTL_CALL(fd, FTD_OVERRIDE_GROUP_STATE, &st);
    close(fd);
...
{
    int fd, ret, count;
    ftd_dev_t_t group = (ftd_dev_t_t)grp;
...
pmds_running(char* pmd, int group, int req_state)
{
 ...
 if(((get_driver_mode(group)==FTD_MODE_NORMAL) && (req_state==FTD_MODE_TRACKING))
    || ((get_driver_mode(group)==FTD_MODE_NORMAL) && (req_state==FTD_MODE_PASSTHRU))
    || (get_driver_mode(group)==FTD_MODE_REFRESH && req_state==FTD_MODE_TRACKING)
    || (get_driver_mode(group)==FTD_MODE_REFRESH && req_state==FTD_MODE_PASSTHRU)
    || (get_driver_mode(group)==FTD_MODE_FULLREFRESH && req_state==FTD_MODE_TRACKING)
    || (get_driver_mode(group)==FTD_MODE_FULLREFRESH && req_state==FTD_MODE_PASSTHRU)
)
 {
...
{
    int ch, i, state, group, index, lgcnt, pcnt;
    ...
    group = -2;
    while ((ch = getopt(argc, argv, "ag:")) != EOF) {
        switch(ch) {
        case 'a':
            if (group != -2) {
		if (group == -1)
		    fprintf(stderr, "-a options are multiple specified\n");
		else
