...
********************************************************* {COPYRIGHT-END} **/
/* #ident "@(#)$Id: ftd_mngt_set_config.c,v 1.6 2014/05/06 16:19:16 paulclou Exp $" */
/* 
...
ftd_mngt_set_config(sock_t *sockID)
{
	...
	if (pRcvCfgData == NULL) { 
		...
		sprintf(logmsg, "while receiving " CAPGROUPNAME " group configuration from %s %s\n", ipstr, sockID->rhostname);
...
mmp_mngt_recv_file_data(sock_t *s, mmp_mngt_header_t *msghdr, 
	mmp_mngt_FileMsg_t **ppRcvFileData, int ignore_filename)
{
	int r;
	int toread;
	int iMsgSize;
	...
	*ppRcvFileData = 0;
	...
	if (r == toread) {
		...
		if (r == toread) {
            ...
            if( ignore_filename )
			{
			    sprintf(logmsg, "uiSize = %d\n", agentFile.data.uiSize);
			}
			else
			{
...
ftd_mngt_set_file(sock_t *sockID)
{
	...
	mmp_mngt_recv_file_data(sockID, NULL, &pRcvFileData, 0);

	if (pRcvFileData == NULL) { 
		/* problem receiving the message data */ 
		ip_to_ipstring( sockID->rip, ipstr );
		sprintf(logmsg, "while receiving "PRODUCTNAME" file from %s %s\n", ipstr, sockID->rhostname);
	...
	sprintf(logmsg, "Set "PRODUCTNAME" file msg received. file name = %s, size = %d bytes\n", pRcvFileData->data.szFilename, pRcvFileData->data.uiSize);
