...
********************************************************* {COPYRIGHT-END} **/
/* #ident "@(#)$Id: ftd_mngt_get_config.c,v 1.9 2014/05/06 16:19:16 paulclou Exp $" */
/* 
...
ftd_mngt_get_file(sock_t *sockID)
{
	...
	mmp_mngt_recv_file_data(sockID, NULL, &pRcvFileMsg, 0);

	/* in a Get_TDMFFILE request, only pRcvFileData->data.szFileName */
	if (pRcvFileMsg == NULL) {
		/* problem receiving the message data */
		ip_to_ipstring( sockID->rip, ipstr );
		sprintf(logmsg, "while receiving "PRODUCTNAME" file request from %s %s\n", ipstr, sockID->rhostname);
	...
	sprintf(logmsg, "Get "PRODUCTNAME" FILE msg received. "PRODUCTNAME" file requested = %s, size = %d bytes\n", pRcvFileMsg->data.szFilename, pRcvFileMsg->data.uiSize);
	...
	if ( strncmp(pRcvFileMsg->data.szFilename,"*.",2) == 0 ) {
		...
		if ( r != 0 ) {
			...
			sprintf(logmsg, "reading, "PRODUCTNAME" file %s\n", szFullPathFname);
			logout(4,F_ftd_mngt_get_file,logmsg);
    	}
