 ...
 * createkey.c
 *
 * Copyright (c) 2005  Softek Storage Solutions Corporation
 * All Rights Reserved.
 *
...
/***********************************************************************/
unsigned int parse_hostid (char* str)
{
...
/***********************************************************************/
static int
prompt_input (int* permflag, time_t* expires, unsigned int* host_id, char* custname)
{
  int ok;
...
/***********************************************************************/
static void
genlickey (char* lickey, int* permflag, time_t* expires, unsigned int* host_id, 
           char* custname)
{
    time_t ts;
    ...
    rv.checksum = x_license_calculate_checksum(&lic_out);
    lic_out.checksum = rv.checksum;

#if 0
    printf("expires=%d, rv=%d, rv.checksum=0x%x, rv.x=0x%x\n",
 	(short) ((int) (*expires >> 16)), rv, rv.checksum, rv.x);
    printf("expiration_date=%d, cksum=%d, expires=%d, level=%d, hostid=0x%x, machine_cksum=%d, customer_name_cksum=%d\n\n",
	lic_out.expiration_date, lic_out.checksum, lic_out.license_expires,
	lic_out.license_level, lic_out.hostid,
	lic_out.machine_model_checksum, lic_out.customer_name_checksum);
#endif
...
print_prompted_lickey (char* lickey, int permflag, time_t expires, 
		       unsigned int host_id, char* custname)
{
  time_t ts;
  FILE* fd;
  char *ptr;
  ...
  if ((ptr = strchr(buf, '\n')) != (char *) NULL)
    *ptr = '\0';
  fprintf (stdout, "%s License generated:  %s\n", 
	   PRODUCTNAME, buf);
  fprintf (fd    , "%s License generated:  %s\n", 
	   PRODUCTNAME, buf);
...
main(int argc, char **argv)
{
    ...
    myhostid = my_gethostid(HOSTID_LICENSE);
