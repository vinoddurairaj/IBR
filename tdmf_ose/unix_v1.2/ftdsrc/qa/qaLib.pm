...
DataStar Project QA Perl Library:
     ...
     DetermineqdsLogicalGroup - return the replication group number that contains a qds device
...
sub DetermineqdsLogicalGroup {
	...
	die "Failed to determine replication group for device $deviceName. Stopped"
		unless ($lg != -1);
	$devLG{$deviceName} = $lg;
	$lg;
}
