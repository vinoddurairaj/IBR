...
handle_overflow(int ctlfd, int fd, int lgnum)
{
    ...
    sb.addr = msgbuf;
    if (FTD_IOCTL_CALL(fd, FTD_SEND_LG_MESSAGE, &sb))
        err(1, "Send message failed");
    newstate.state = FTD_MODE_NORMAL;
    newstate.lg_num = lgnum;
    if (FTD_IOCTL_CALL(ctlfd, FTD_SET_GROUP_STATE, &newstate))
...
main(int argc, char *argv[])
{
    ...
    printf("Using %s to control replication group... The virtual pmd is online\n",
        group_name);
    ...
    if (fd < 0)
        err(1, "Can't open replication group control file %s", group_name);
    ...
    while (1) {
        ...
        if (FTD_IOCTL_CALL(fd, FTD_OLDEST_ENTRIES, oe) < 0) {
            if (errno == EINVAL)
         ...
         */
        if (FTD_IOCTL_CALL(fd, FTD_MIGRATE, mig))
