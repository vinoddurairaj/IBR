 ...
 *
 * $Id: ftd_aix.c,v 1.27 2012/02/13 22:12:23 paulclou Exp $
 *
 */
...
ftd_int32_t
ftd_ioctl(dev_t dev, ftd_int32_t cmd, ftd_intptr_t arg, ftd_uint32_t devflag, chan_t chan, ftd_int32_t ext);
ftd_int32_t
...
ftd_int32_t
ftd_dev_ioctl(dev_t dev, unsigned int cmd, void * arg, ulong flag, chan_t chan, ext_t ext);
...
extern ftd_int32_t *sync_time_out_flg;	/* for Dynamic Mode Change */
...
struct devsw    ftd_devsw =
{
 ...
 (struct tty *)0,		/* pointer to tty device structure      */
 ...
 (caddr_t)0,			/* pointer to device specific data      */
#ifdef __64BIT_KERNEL || _IA64
 (caddr_t)0,	 		/* ptr to outstanding select cntl blks  */
 (ulong)DEV_MPSAFE|DEV_64L
#else
 (caddr_t)0	 		/* ptr to outstanding select cntl blks  */
#endif
};
...
ftd_config(dev_t dev, ftd_int32_t cmd, struct uio * uiop)
{
	...
	case CFG_INIT:
		 ...
		 */
		if (err = ftd_config_drv(dev, uiop))
            break;
...
ftd_config_drv(dev_t dev, struct uio * uiop)
{
	...
	/* Make the behavior the same as in the other platforms.
	 *
	 * In general, ftd_global_state should be NULL.  
	...
	if (ftd_global_state) {
        // If we are being initialized and that the global state isn't null, it means that the driver wasn't unloaded since the last termination.
        // In such a state, it's dangerous to try to recover/clean up from the error, as we'll try to re-free memory that was already freed on the
        // last termination.
        // C.F. WR PROD00005936.
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Aborting driver initialization as the ftd_global_state is already initialized.", DRVNAME, inst);
        Return(-1);
	}
	...
	if (ddi_soft_state_init(&ftd_dev_state, sizeof(ftd_dev_t), 16) != 0) {
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Failed to initialize the dev state.", DRVNAME, inst);
		if (ftd_global_state)
			kmem_free(ftd_global_state, sizeof(ftd_ctl_t));	
		return -1;
	}
	if (ddi_soft_state_init(&ftd_lg_state, sizeof(ftd_lg_t), 16) != 0) {
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Failed to initialize the lg state.", DRVNAME, inst);
		if (ftd_global_state)
			kmem_free(ftd_global_state, sizeof(ftd_ctl_t));	
	...
	if (ctlp->chunk_size < MIN_MEMCHUNK_SIZE) {
		FTD_ERR(FTD_WRNLVL,
			"%s-ctl%d: chunk_size must be larger than %d",
			DRVNAME, inst, MIN_MEMCHUNK_SIZE);
	...
	if (ctlp->chunk_size != DEFAULT_MEMCHUNK_SIZE) {
		FTD_ERR(FTD_WRNLVL,
			"%s-ctl%d: %s.conf chunk_size is %d, reset to default %d",
			DRVNAME, inst, DRVNAME, DEFAULT_MEMCHUNK_SIZE);
    ...
    if (ftd_dynamic_activation_init(dev) != 0) {
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Failed to initialize dynamic activation.", DRVNAME, inst);
        goto error;
    }
	...
	if (sync_time_out_flg == NULL) {
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Failed to allocate the sync_time_out_flg.", DRVNAME, inst);
		goto error;
	}
...
error:
	...
	/* * ... private buffer pool ... */
	FTD_FINI_BUF_POOL();
	/* free timer request block pool: driver is yet not loaded */
	ftd_timer_fini(0);
...
ftd_unconfig_drv(dev_t dev)
{
    ...
    {
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Cannot uninitialize the driver. Unexisting global state.", DRVNAME, inst);
		Return(-1);
    }
	...
	if (ftd_dev_n_open(ctlp)) {
        ...
        FTD_ERR(FTD_WRNLVL, "%s-ctl%d: Cannot uninitialize the driver. Devices are opened.", DRVNAME, inst);
		Return(-1);
	}
	while (ctlp->lghead)
	...
	/* ... timer request block pool.  Driver is loaded.  param = 1 */
	if (ftd_timer_fini(1) < 0) {
...
ftd_ioctl(dev_t dev, ftd_int32_t cmd, ftd_intptr_t arg, ftd_uint32_t devflag, chan_t chan, ftd_int32_t ext)
{
	...
	if (minor == FTD_CTL)
		...
		err = ftd_dev_ioctl(dev, cmd, passarg, devflag, chan,ext);
 ...
 * 	results.
 *
 * @attention Can also be called by ftd_captured_driver_replacement_strat().
 */
ftd_void_t
ftd_strat(struct buf * bp)
{
	...
	if ((softp = ftd_dynamic_activation_get_replication_device_structure(BP_DEV(bp))) == NULL) {
		bioerror(bp, ENXIO);
		biodone(bp);
		Returnv;
	}
...
ftd_select(dev_t dev, ftd_uint16_t events, ftd_uint16_t * reventp, ftd_int32_t chan)
{
	...
	/* asynch poll */
    /*
     * The documentation mentions that the event id is an int, but that's not true under 64 bit kernels,
     * where it's a long to properly hold a 64 bits dev_t.
     * We must then not cast our device to an int, but just leave it as is.
     */
	lgp->ev_id = dev;
	lgp->ev_subid = 0;
...
ftd_bioclone(ftd_dev_t * softp, struct buf * sbp, struct buf * dbp)
{
	...
	/* MP Safe flag */
        dbp->b_flags |= (b_flags_sv & B_MPSAFE_INITIAL) ? B_MPSAFE_INITIAL : 0;
...
ftd_dev_ioctl(dev_t dev, unsigned int cmd, void * arg, ulong flag, chan_t chan, ext_t ext)
{
	...
	if (softp == NULL)
		if (softp == NULL)
			return (ENXIO);
        err = rdevioctl(softp->localbdisk,  cmd, arg, flag, chan, ext);
	return (err);
}
