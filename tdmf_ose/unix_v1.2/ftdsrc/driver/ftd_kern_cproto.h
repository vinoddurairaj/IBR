...
********************************************************* {COPYRIGHT-END} **/
#if !defined(FTD_KERN_CPROTO_H) && (defined(_KERNEL) || defined(KERNEL) || defined(__KERNEL__))
#define FTD_KERN_CPROTO_H

/* include me in all kernel xlation units */

#if defined(SOLARIS) || defined(HPUX) || defined(_AIX) || defined(linux)
 ...
 * -DVERSIONBUILD=\"" Build 000"\" -DSYSVERS=570 -DSOLARIS -D_KERNEL -DKERNEL 
 * 
 */
/* ftd_sun.c */
#if defined(SOLARIS) 
...
FTD_PRIVATE ftd_open (dev_t dev, ftd_int32_t flag, intptr_t dummy, int mode);
FTD_PRIVATE ftd_int32_t ftd_close (dev_t dev, ftd_int32_t flag, int mode);
...
FTD_PRIVATE ftd_int32_t ftd_ioctl (dev_t dev, ftd_int32_t cmd, caddr_t arg, ftd_int32_t flag);
...
struct buf *getpvtrbuf (ftd_int32_t sleep_code);
...
FTD_PUBLIC ftd_void_t ftd_fini_pvt_buf_pool(ftd_void_t);
FTD_PUBLIC ftd_void_t bufzero(struct buf *bp);
FTD_PUBLIC struct buf *getpvtrbuf(ftd_int32_t sleep_code);
...
/* ftd_aix.c */
ftd_void_t ftd_wakeup(ftd_lg_t * lgp);
#endif

#if defined(linux)
/* ftd_linux.c */
FTD_PRIVATE ftd_int32_t ftd_dev_ioctl (struct inode *inode, struct file *filp, ftd_int32_t cmd, ftd_intptr_t arg);
...
FTD_PUBLIC int ftd_ctl_siocgifconf(unsigned long arg);
...
FTD_PUBLIC ftd_int32_t ftd_bab_copy_out(bab_mgr_t *mgr, ftd_int32_t offset64, ftd_uint64_t *addr, ftd_int32_t len64, ftd_lock_t *pLock, ftd_context_t* pContext);
...
FTD_PUBLIC void ftd_compute_dirtybits(ftd_lg_t *lginfo, ftd_int32_t type);

/* ftd_ioctl.c */
FTD_PRIVATE ftd_int64_t ftd_get_msb(ftd_int64_t num);
...
#if defined(_AIX)
FTD_PRIVATE ftd_int32_t ftd_ctl_get_started_group(ftd_intptr_t arg, ftd_int32_t flag);
#endif
FTD_PRIVATE ftd_int32_t ftd_ctl_set_group_state(ftd_intptr_t arg, ftd_int32_t override);
...
FTD_PRIVATE ftd_int32_t ftd_ctl_set_lrdb(ftd_intptr_t arg);
FTD_PRIVATE ftd_int32_t ftd_ctl_set_lrdb_mode(ftd_intptr_t arg);
...
FTD_PUBLIC ftd_int32_t ftd_del_device (ftd_dev_t * softp, minor_t minor);
...
FTD_PRIVATE ftd_int32_t ftd_ctl_hrdb_type(ftd_intptr_t arg);
...
#if defined(linux)
FTD_PUBLIC ftd_void_t biodone_psdev(struct buf *bp, int uptodate);
#else
...
#elif defined(_AIX)
FTD_PRIVATE ftd_void_t        ftd_synctimo(struct trb *);
#elif defined(linux)
FTD_PUBLIC ftd_void_t ftd_synctimo (unsigned long data );
#else
...
FTD_PUBLIC ftd_int32_t ftd_rw_validate_params(dev_t dev, struct uio *uio);
#endif /* !defined(linux) */
...
FTD_PUBLIC ftd_int32_t ftd_flush_lrdb(ftd_dev_t *softp, void *,  ftd_context_t context);
#if defined(linux)
FTD_PRIVATE int ftd_write_lrdb(ftd_dev_t *softp, struct buf *bio);
#endif
...
FTD_PUBLIC ftd_void_t ftd_clear_hires_historical_dirtybits(ftd_lg_t *lgp);
...
FTD_PUBLIC ftd_void_t ftd_clear_hrdb(ftd_lg_t *lgp);
#else
...
FTD_PUBLIC ftd_int32_t ftd_del_lg (ftd_lg_t * lgp, minor_t minor);
...
#if defined(_AIX)
/* ftd_timers.c */
ftd_int32_t ftd_timer_init(ftd_void_t);
ftd_int32_t ftd_timer_fini(int drv_loaded);
struct trb * ftd_timer_alloc(ftd_void_t (*func)(), caddr_t arg);
ftd_int32_t ftd_timer_rele(struct trb *trbp);
struct trb * ftd_timeout(ftd_void_t (*func)(), caddr_t arg, ftd_int32_t ticks);
ftd_void_t ftd_aixuntimeout(struct trb *trbp);
#endif /* defined(_AIX) */
