 ...
 *
 * $Id: ftd_sun.c,v 1.23 2012/02/13 22:12:23 paulclou Exp $
 *
 */
...
ftd_getutsname() 
{
  ...
  if (strncmp("5.9", utsname.release, strlen("5.9")) == 0) 
    {
      return (ftd_solvers = SOL_VERS_590);
    }
  if (strncmp("5.10", utsname.release, strlen("5.10")) == 0) 
    {
      return (ftd_solvers = SOL_VERS_5100);
    }
...
ftd_attach (dev_info_t * devi, ddi_attach_cmd_t cmd)
{
  ...
  /* should not occur ??
   * driver instance existed.  But reload/re-attach occur ??
   */
  ...
  if (ctlp == NULL)
    {
      FTD_ERR (FTD_WRNLVL, "%s-ctl%d ftd_attach: Can't allocate state info",
	       DRVNAME, inst);
      goto error;
    }
  ...
  if (ctlp->chunk_size < MIN_MEMCHUNK_SIZE) {
      FTD_ERR (FTD_WRNLVL,
               "%s (inst=%d) ftd_attach: driver failed to attach, please check %s.conf, chunk_size must be larger than %d\n",
               DRVNAME, inst, DRVNAME, MIN_MEMCHUNK_SIZE);
      goto error;
  }
#else /* TUNABLE_CHUNKSIZE */
  if (ctlp->chunk_size != DEFAULT_MEMCHUNK_SIZE) {
      FTD_ERR (FTD_WRNLVL,
               "%s (inst=%d) ftd_attach: %s.conf chunk_size is modified %d, "
               "reset to default %d\n",
               DRVNAME, inst, DRVNAME, ctlp->chunk_size, DEFAULT_MEMCHUNK_SIZE);
  ...
  if (ctlp->bab_size == 0) {
      FTD_ERR (FTD_WRNLVL, "%s (inst=%d) ftd_attach: Can't init BAB.\n",
	       DRVNAME, inst);
      goto error;
  }
  ctlp->dev_info = devi;
  ...
  if (ctlp->ftd_buf_pool_headp == NULL) {
      FTD_ERR (FTD_WRNLVL, "%s (inst=%d) ftd_attach: failed to FTD_INIT_BUF_POOL.\n",
	       DRVNAME, inst);
      goto error;
  }
   ...
   */
  if (ddi_create_minor_node (devi, "ctl", S_IFCHR, FTD_CTL, DDI_PSEUDO,
			     0) == DDI_FAILURE)
    {
  ...
  if(ftd_dynamic_activation_init(makedevice(ddi_name_to_major((char*)ddi_driver_name(devi)),FTD_CTL)) != 0) {
      goto error;
  }
...
ftd_layered_open (struct dev_ops ** dopp, dev_t * devp)
{
  ...
  *dopp = ddi_hold_installed_driver (getmajor (*devp));
  ...
  return ((*dopp)->devo_cb_ops->
	  cb_open (devp, FREAD | FWRITE, OTYP_LYR, kcred));
}
...
ftd_layered_close (struct dev_ops * dop, dev_t dev)
{
  ...
  retval = dop->devo_cb_ops->cb_close (dev, FREAD | FWRITE, OTYP_LYR, kcred);
