 ...
 */
ftd_int32_t
ftd_timer_fini (int drv_loaded)
{
  ftd_int32_t i;

  /* whether inited */
  if (drv_loaded && !trbblk_inited) {
      /* driver is loaded.  timer block is marked not initialized.
       * Log the error accordingly.
       */
      FTD_ERR (FTD_WRNLVL, "ftd_timer_fini: not inited.");
      return (-1);

  } else if (!trbblk_inited) {
      /* driver should not be loaded yet.  Timer block is marked not 
       * initialized.
       */
      return 0;
  }

  /* free timer request blocks if trbblk_inited is not NULL ! */
  for (i = 0; i < NTRBSLOTS; i++)
    {
...
ftd_timer_enqueue (ftd_lg_t * lgp, wlheader_t * wl)
{
  ...
  if (!lgp->syncHead && !lgp->syncTail)
    ...
    {				/* all other entry is appended to syncTail */
      lgp->syncTail->syncForw = (ftd_uint64ptr_t) wl;
      wl->syncForw = NULL;
      wl->syncBack = (ftd_uint64ptr_t) lgp->syncTail;
      lgp->syncTail = wl;
    }
