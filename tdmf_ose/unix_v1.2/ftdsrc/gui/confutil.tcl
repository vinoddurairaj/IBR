...
#    -2=operation failed in the target system because requisite number of devices are not
#       present on the system.  
#============================================================================================

proc ftdmkdevlist {sysname {portnum 575} {timeout 60000} isprim} {
    global tcl_platform
    set SYS(platform) $tcl_platform(os)
    if {$sysname == ""} { return }
regsub -all {::} $sysname :0: modsysname
    set gn "devlist-$modsysname"
    global $gn
    upvar #0 $gn d
    if {[info exists $gn]} {
	catch {unset d(names)}
    ...
    if {[catch "socket $sysname $portnum" sock]} {
         set d(status) -1
         set d(errmsg) " Socket creation at $sysname:$portnum failed with error:$sock."
          return $gn
       }
    ...
    set ndev [expr ([llength $devs]-1)]
    set min_dev_prim 3                                    
    set min_dev_sec 2                                     

    if {$ndev == 0} {
       set d(status) -2
       set d(errmsg) " Failed to recognize disk device/volumes on $sysname."
       return $gn
        }

    if {$isprim == 1} {
    if {$ndev < $min_dev_prim} {
        set d(status) -2
        set d(errmsg) "Requisite number of disk devices not discovered by the Master daemon on $sysname. Need minimum 
$min_dev_prim devices on primary."
        return  $gn
        }
       }
    ...
    if {$ndev < $min_dev_sec} {
        set d(status) -2
        set d(errmsg) "Requisite number of disk devices not discovered by the Master daemon on $sysname. Need minimum 
$min_dev_sec devices on secondary."
        return $gn
       }
     }
...
regsub -all {::} $sysname :0: modsysname
    ...
    if {[info exists d(names)] && [info exists d($devpath..size)]} {
        # right now start/stop regions are not used so set devsize to arbitrary
        # value to avoid a stack trace when working with >2TB devs
        set devsize 999999999
	if {$stop == 999999999} {
