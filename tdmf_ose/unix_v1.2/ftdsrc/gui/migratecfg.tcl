...
#                   installations to current expectations of
#                    %PRODUCTNAME% %VERSION%.
#
...
proc migratecfgs {} {
    ...
    global MIG_INFO
    if {![chkmigrate%PKGNM%]} {
       # No longer migrating from Softek brand
       # chkmigrateFTSWftd
    }
...
%PRODUCTNAME% configuration files to %PRODUCTNAME% format
...
#   set MIG_INFO(fromdir) $tname
    set MIG_INFO(fromdir) $newestdir
...
#-----------------------------------------------------------------------------
# chkmigrateFTSWftd -- returns 1 if we need to migrate a Softek
#                      set of configuration files
#-----------------------------------------------------------------------------
proc chkmigrateFTSWftd {} {
    ...
    set newestdir ""
    set tname ""
    # -- see if there is an environment saved for Softek configurtion files
...
#-----------------------------------------------------------------------------
# domigrateFTSWftd -- migrate Softek .cfg files
#-----------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------
# domigrate%PKGNM% -- migrate .cfg files
#-----------------------------------------------------------------------------
proc domigrate%PKGNM% {} {
    ...
    foreach fname "[glob -nocomplain $MIG_INFO(fromdir)/\[ps\]\[0-9\]\[0-9\]\[0-9\].cfg]" {
        set fn [lindex [file split $fname] end]
#        exec /bin/cp $fname /%FTDCFGDIR%/$fn
         if {$tcl_platform(os) == "Linux"} {
            exec /bin/sed "s/^\[ \|\\t\]\*TDMF-DEVICE:/  %CAPQ%-DEVICE:/g" \
                           $fname > /%FTDCFGDIR%/$fn
         } else { 
            exec /usr/bin/sed "s/^\[ \|\\t\]\*TDMF-DEVICE:/  %CAPQ%-DEVICE:/g" \
                           $fname > /%FTDCFGDIR%/$fn
         }
    }
    killpopmigratingmsg
