...
#################################
# 
#  perl -w -I.. /bIrecreateiso.pl
#
#	recreate QA'ed iso image with refreshed 
#	(final  ????) docs
#
#	usage  :
#	perl -w -I.. bIrecreateiso.pl -u
#
#################################
###################################
...
system qq(ncftpget -f ../bmachine.txt . '/builds/TDMFUNIX\(IP\)'/$RFXver/$Bbnum/$RFXver.$Bbnum.ALL.tar);
...
if (-d "Softek/Replicator/doc" ) {print "\ndoc exists\n"; system qq(rm Softek/Replicator/doc/*);} 
else {mkdir -p "Softek/Replicator/doc";}

system qq(cp -p ../../doc/*.pdf Softek/Replicator/doc);
print "\n\ncp *.pdf doc  :  rc = $?\n";
...
###############################
#iso vars  :   straight from ../iso/bIcreateiso.pl
###############################
...
system qq(ncftpput -f ../bmachine.txt /builds/TDMF/Srv2Srv/UNIX/$RFXver/$Bbnum $RFXver.$Bbnum.$isorev.iso isochangenotification.txt);
...
print <<END;
bIrecreateiso.pl usage :

bIrecreateiso.pl -b -i -u 
