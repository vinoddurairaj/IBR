...
#if (_WIN32_IE >= 0x0400)
// Custom Draw Struture
typedef struct tagTBCD {
    HFONT hfont;                    // Font for button text
    ...
    BOOL fNoEdges;                  // No edges
    BOOL fHighlightHotTrack;        // Color of the bk of the button when hottracked
    BOOL fNoOffset;                 // Don't offset button if pressed
    BOOL fNoMark;                   // Don't draw default highlight of image/text for TBSTATE_MARKED
} TBCD, * LPTBCD;
#endif
...
#define TB_MAPACCELERATOR       (WM_USER + 78)  // wParam == ch, lParam int * pidBtn
...
#define HICF_DROPDOWN       0x00000008          // Button dropped down
