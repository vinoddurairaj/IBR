...
package MODEM;          # The name of this module. For our purposes,
                        #  must be all in caps.
    #---------------------------------------------------#
    # Overhead required by every package.
    #---------------------------------------------------#
    require(Exporter);
    @ISA=qw(Exporter);
    @EXPORT=qw(Exists);
    ...
    my(%ModemErrorTable) = (1027 => ["Blank identifier."],
                            ...
                            3049 => ["Mandatory field [Value-entry-name] should be blank."],
                            ...
                            3073 => ["AT-Command string should end with \"&ltCR&gt\"."],
                            3074 => ["&ltCELLULAR_O&gt key - can no longer be used."],
                            3075 => ["Expect &ltKey&gt to be a quoted string."],
                            3076 => ["&ltKey&gt can't be empty string."],
                            3077 => ["Expect response to be in binary."],
                            3078 => ["Properties should have 8 DWORDS."],
                            3079 => ["InactivityScale should have 1 DWORD."],
                            3080 => ["&ltValue&gt is expected to have only hex digits."],
                            3081 => ["Expect &lt;Value&gt to be a quoted string."],
                            3082 => ["&ltValue&gt can't be empty string."],
    ...
    my(%ModemWarningTable)=(2011 => ["White spaces in identifier \"%s\"."],
                            ...
                            4025 => ["%s: Same &ltValue&gt for at least two of SpeakerVolume_XXX Settings."],
                            4026 => ["%s: Same &ltValue&gt for at least two of SpeakerMode_XXX Settings."],
                            4027 => ["%s: Same &ltValue&gt for Compression_On/Off Settings."],
                            4028 => ["%s: Same &ltValue&gt for ErrorControl_On/Off/Forced Settings."],
                            4029 => ["%s: VoiceView strings discontinued."],
                            4030 => ["%s: Same &ltValue&gt for ErrorControl_Cellular/Cellular_Forced Settings."],
                            4031 => ["%s: Same &ltValue&gt for at least two of the FlowControl_XXX Settings."],
                            4032 => ["%s: Same &ltValue&gt for Modulation_XXX Settings."],
                            4033 => ["%s: Possible inversion of &ltValue&gt between Settings Modulation_CCITT/Bell."],
                            4034 => ["%s: Same &ltValue&gt for SpeedNegotiation_On/Off Settings."],
                            4035 => ["%s: Possible inversion of &ltValue&gt between Settings SpeedNegotiation_On/Off."],
                            4036 => ["%s: Same &ltValue&gt for Tone/Pulse Settings."],
                            4037 => ["%s: Possible inversion of &ltValue&gt between Settings Tone/Pulse."],
                            4038 => ["%s: Same &ltValue&gt for Blind_On/Off Settings."],
                            4039 => ["%s: Possible inversion of &ltValue&gt between Settings Blind_On/Off."],
...
sub InfCheck {
    ...
    print(STDERR "\tInvoking InfCheck...\n") if ($DEBUG);
    # Mark All Sections as Unchecked by MODEM.PM
    ...
    # modem specific
    while (($key, $ModelInfo) = (each %AllModels))
    {
        $ModelDescription = $$ModelInfo[0];
        if ($ModelDescription =~ /^%(.*)%$/)
        {
            my($TempVar) = quotemeta($1);
            if (defined $StringVals{uc $TempVar})
            {
                $name = $StringVals{uc $TempVar};
                if ($name =~  /^__QUOTED_STR_\(([0-9]*)\)__$/)
                {
                    $name = ChkInf::QuotedString($1);
                    #AddModemError(3033, ($SecRef{'STRINGS'}), $ModelDescription)
                    #   if ($name =~ /[\^\/:\|"<>]|\.$/);
                    $$ModelInfo[0] = $name;
                }
            }
...
my($line)   = $SecRef{"STRINGS"};
my(@lines)  = @InfLines;
my($DONE)   = FALSE;
my($val);

    while (! $DONE) {
        die("Can't find line $line\n") if (! defined($lines[$line]));
        $line++, next if ($lines[$line] =~ /^\s$/); # Skip if it's a blank line
        if ($lines[$line] =~ /^\[.*\]/) {           # Quit once we get to the next section
            $DONE = TRUE;
            next;
        }
        $key = ChkInf::GetDirective($line,$lines[$line]);
        if ($lines[$line] =~ /=/) 
        {
            $val     = ChkInf::GetDirectiveValue($line,$lines[$line]);
            if ($val =~  /^__QUOTED_STR_\(([0-9]*)\)__$/)
            {
                $name = ChkInf::QuotedString($1);
    ...
    if (($bDupWarnings == 1) && ($bAllWarnings == 0))
        { AddModemWarning(4110,$TotalLines - 1);}
    if (($bSecondDwordSpeed == 1) && ($bAllWarnings == 0))
        { AddModemWarning(4111,$TotalLines - 1);}

    &modem_section_usage;

    print(STDERR "\tExiting InfCheck...\n") if ($DEBUG);

    
    $EndTime = time();

    $ElapsedTime = $EndTime-$StartTime;
    $TotalElapsed = $EndTime-$TotalTime;
#    printf(STDERR  "\n\tChecking Time : %02dh %02dm %02ds\n", int($ElapsedTime / 3600), int($ElapsedTime / 60) % 60, $ElapsedTime % 60);

#    printf(STDERR  "\tTotal Elapsed Time : %02dh %02dm %02ds\n", int($TotalElapsed / 3600), int($TotalElapsed / 60) % 60, $TotalElapsed % 60);
#   print "\n";
...
%AllModels = ();
    ...
    if ((-e "faxreg.pm") && ($FaxCheck == 1))
    {
        require faxreg;
        print "\tModule \"FAXREG\" Loaded\n";
...
sub LearnModels {
    ...
    while (! $DONE) {
        ...
        if (defined($SecRef{uc($Value)})) {
            ...
            ExpandModelSection($SecRef{uc($Value)}, uc($Value) );
        }
        $line++;
    }
...
sub ExpandModelSection {
    ...
    while (! $DONE) {
        ...
        $Directive = getLHS($lines[$line],$line);
        @id = ();
        @id = split(',', ChkInf::GetDirectiveValue($line, $lines[$line]));
        $sz = @id;
        foreach $element (@id) 
        {
...

        {
            ...
            if (defined ($SecRef{uc $inst}))
            {   $SectionsDefined++; }

            foreach $Extension (keys %InstallExtensions)
            {
                if (defined ($SecRef{uc ($inst.".$Extension")}))
                {
                    $SectionsDefined++; 
            ...
            {
                ChkInf::AddErrorRef(1052, $line, $inst);
            }
        }

        ChkInf::AddErrorRef(1017, $line) if ($sz > 3);

        for($i=0; $i < $sz; $i++)
        {
            if($id[$i] =~ /^__QUOTED_STR_\(([0-9]*)\)__$/)
            {
                $id[$i] = ChkInf::QuotedString($1);
                AddModemError(1051, $line);
            }
        }
        $count = 0;
        foreach $ExcludeID (@id)
        {
            ...
            ChkInf::AddErrorRef(1049, $line, $MaxIDLen)
        ...
        ChkInf::AddErrorRef(1018, $line)  if (($sz < 1) || ($id[0] =~ /^$/));

        if (defined $AllModels{uc $model})  # same modem under diff mfg!!
        {
            ChkInf::AddWarnRef(2006,$line, $model);
        }
    ...
    if ( ($count = keys(%NewModelList)) == 0)
    {
        ChkInf::AddErrorRef(1040, $SecRef{$Section}, $Section);
    }
...
InstProc
{
    my($Section, $IsHWInstall) = @_;
    my($line)       = $SecRef{$Section};
    my($sec_name, @sec_list, $count);
    ...
    while (! $DONE) {
        ...
        if ($count < 1)
        {
            ChkInf::AddErrorRef(1025, $line, $Directive);
            $line++;
            next;
        }
        ...
        foreach $sec_name (@sec_list)
        {
            ...
            {
                ChkInf::AddErrorRef(1026, $line, $Directive);
                $line++;
                next;
            }
        } 
    ...
    print(STDERR "\tExiting InstProc (@_)...\n") if ($DEBUG);
}
#-------------------------------------------------------------
#
...
print(STDERR "\tExiting ModelIDTypes...\n") if ($DEBUG);
}
#-------------------------------------------------------------
# 
# Globals changed:
...
print(STDERR "\tInvoking CheckInstSections...\n") if ($DEBUG);
    ...
    {
        next if (!defined($LLnum = $SecRef{$sec}));
...
my(@lines)      = ChkInf::GetInfLines;
    ...
    while (! $DONE) {
        ...
        ($key, $value) = &get_addreg_list($lines[$line], $line);
...
IsStringKey
{
    ...
    foreach $String (@StringList)
    {
        ...
        {
            $TempVar = quotemeta($1);
            ChkInf::AddErrorRef(1036, $CLnum, $String)
                if (!defined($StringVals{uc($TempVar)}))
        }
    }
}
...
#           $ResponseStruct[0], $SearchResp;
    ...
    if  (($ResponseStruct[1] != 0 || $ResponseStruct[0] == 0x01 || $ResponseStruct[0] == 0x02))
    {
        my($NegotiatedKey);
...
#                   $NegotiatedKey;
    ...
    #if (($AdvisoryMode == 1) and
    if (($ResponseStruct[0] == 0x01 ||  $ResponseStruct[0] == 0x02))
    {
        my($i);
        foreach $i (2,3)
        {
...
#       inst_wrn("$model: Init not robust. Expect \"$str\" also.", $L);
        AddModemWarning(4018, $L, $model, $str);
    }
...
#       add_ref_err("$model does not have any Responses.", $Lnum);
    ...
    {
        AddModemError(3129, $Lnum, "&ltcr&gt");
    }
    ...
    {
        AddModemError(3129, $Lnum, "&ltlf&gt");
    }
}
...
{
my(@HKRLines)=@_;
my($command, @HKRArg, @aux, $lineNo, $CmdString, $NoErrs);
    ...
    {   @HKRArg = split(",", $command);
        ...
        { 
            $CmdString = uc ChkInf::QuotedString($1);
...
print(STDERR "\tInvoking CheckVoice...\n") if ($DEBUG);
    ...
    while (($ModemKey, $ModemRef) = each %AllModels)
    {
        ...
        if ($$ModemRef[0] =~ /^%(.*)%$/)
        {
            my($TempVar) = quotemeta($1);
            if (defined($StringVals{uc($TempVar)}))
            {   $ModemName  = $StringVals{uc($TempVar)};}
        }
...
print(STDERR "\tInvoking CheckIds...\n") if ($DEBUG);
    ...
    #   $$ModelProp[6])
        ...
        if ($IsSTAR >= 0)
        {
            if (!defined ($SecRef{uc "$InstSection.POSDUP"}))
            {
                #   add_ref_err("Section [$InstSection.POSDUP] expected but
                #   missing.", $TotalLines)
                AddModemError(3044, $IdLine[$IsSTAR], $InstSection);
            }
        }
...
< 0 && 
            ...
            { 
                ...
                AddModemError(3037, $IdLine[$IsUNKNOWN]);
                $bUnimodemRequired = 1;
            }
        }
        ...
        if ($IsSERENUM >= 0)
        {
            ...
            if (!defined ($SecRef{uc "$InstSection.NORESDUP"}))
            {
            ...
            else
            {
                ...
                { 
                    ...
                    AddModemError(3039, $IdLine[$IsSERENUM]);
                }
                else
                {
                    if ($SerenumMatchModel =~ /^%(.*)%$/)
                    {
                        $TempVar1 = quotemeta($1);
                    }
                    if ($Model =~ /^%(.*)%$/)
                    {
                        $TempVar2 = quotemeta($1);
                    }
                    if ((defined($Model) and defined($StringVals{uc $TempVar1})
                        and defined($StringVals{uc $TempVar2}) and defined($SerenumMatchModel)) and
                         ((uc $SerenumMatchModel) cmp (uc $Model)) != 0 &&
                         ($StringVals{uc $TempVar1} cmp $StringVals{uc $TempVar2}) == 0)
                    {   
        ...
        if (!$bUnimodemRequired && $IsUNIMODEM < 0)
        {
            ...
            { 
                ...
                # $$ModelProp[6]);
                AddModemError(3037, $IdLine[$IsPCMCIA]);
            }
            ...
            { 
                ...
                AddModemError(3091, $IdLine[$IsMF]);
            }
            ...
            { 
                ...
                AddModemError(3041, $IdLine[$IsSTAR]);
            }
        }
    }
...
my($DONE) = FALSE;
    ...
    return if (!defined ($line = $SecRef{uc $section}));
    ...
    while (! $DONE) {
        ...
        @IDs = &get_list($lines[$line], $line, $FlagErrs);
        foreach $Id (@IDs)
        {
...
my(@list, $element);
    ...
    if ($check_empty)
    {
        ChkInf::AddErrorRef(1041, $Lnum) if ($line =~ /,$/);
    }
    @list = split(',', $line);
...
my($DONE) = FALSE;
    ...
    if ($count == 0)
    {
        AddModemError(3086, $line-1, $section);
        return;
    }

    if ($present == 0)
    {
        AddModemError(3087, $line-1, $section);
        return;
    }

    if ($count > 1)
    {
        AddModemError(3088, $line-1, $section);
    }
}
...
my($LHS, $this_num) = @_;
    ...
    $LHS =~ s/^\s*//;
    $LHS =~ s/[\s\t]*=//;
    $LHS = ChkInf::QuotedString($1) if($LHS =~ /^__QUOTED_STR_\(([0-9]*)\)__$/);
    $LHS; #return value
}
