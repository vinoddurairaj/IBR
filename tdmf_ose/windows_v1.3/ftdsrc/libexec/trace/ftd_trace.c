...
usage(char **argv)
{
    ...
    fprintf(stderr, "Set trace level of Server or Block device driver.\n");
    ...
    fprintf(stderr, "         -b   => send trace level to Block device driver.\n");
    fprintf(stderr, "                 Without -b, trace level request is sent only to Server.\n");
}
...
setDriverTraceLevel(int level)
{
    ...
    r = ftd_ioctl_set_trace_level(ctlfd, level);
...
main(int argc, char *argv[])
{
    ...
    while ((ch = getopt(argc, argv, "bo:l:")) != EOF) {
        ...
        case 'l':
            ...
            if ( llevel > TRACEINF6 || llevel < 0 ) //rddev 0301015
                goto usage_error;
    ...
    if ( bDriverTraceLvl )
    {
        exitcode = setDriverTraceLevel(level);
        if ( exitcode == 0 )
            printf("\nBlock driver trace level successfully set to %d.",level);
        else
            printf("\nERROR, could not set Block driver trace level !"
                   "\nThe installed version of the Block driver may not support this feature.");
        exit( exitcode );
    }
...
/*
        ...
        printf("\nERROR, unable to connect to Server (localhost, port=%d).",portnum);
        exitcode = 1;
        goto errexit;
    } else {
        printf("\nERROR, unable to send data to Server."
               "\nThe installed version of the Server may not support this feature.");
        exitcode = 1;
        goto errexit;
    }
    ...
    if (rc < 0) {
        printf("\nERROR, Server did not respond to request."
               "\nThe installed version of the Server may not support this feature.");
        exitcode = 1;
        goto errexit;
    }
    ...
    if ( ack.msgtype != FTDCSETTRACELEVELACK ) {
        ...
        printf("\nERROR, Server did not acknowledged."
               "\nThe installed version of the Server may not support this feature.");
        exitcode = 1;
        goto errexit;
    }
    printf("\nServer trace level successfully set to %d.",level);
