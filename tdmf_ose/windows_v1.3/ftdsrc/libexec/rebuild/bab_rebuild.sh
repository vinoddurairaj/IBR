...
#  For HP-UX 10.20.
#
#  (c) Copyright FullTime Software, Inc., 1998
#
#########################################################################
#
...
Save_vmunix ()
{
    ...
    #
    if [ -f ${KERNEL_TARGET}.pre%PRODUCTNAME% ]
    then
        KERNEL_TARGET_SVD=${KERNEL_TARGET}.%PRODUCTNAME%bak
    else
        KERNEL_TARGET_SVD=${KERNEL_TARGET}.pre%PRODUCTNAME%
    fi
...
newBuild()
{
   ...
   csize=`cat $CONFIGSYS | awk '/^SZ/ { print $3 }'`
   nsize=`echo "${BAB_SIZE} $csize" | awk ' { printf "%d", ($1 + $2) } '`
...
rebuild()
{
   ...
   nsize=`echo "${BAB_SIZE} $osize" | awk ' { printf "%d", ($1 + $2) } '`
   line=`cat $CONFIGSYS | sed -n -e '/^SZLIMIT/='`
...
#
    ...
    # check if CONFIGSYS is touched by Legato
...
#
    ...
    $CONFIG -s -m $MASTER $SYSPATH
    exitval=$?
    if [[ $exitval -eq 0 ]]
    then
        cd $KERNEL_BUILDDIR
        set LINE=`grep "^SZLIMIT" config.mk`
        # if the line looks like we expect it to
        if [ `expr "$LINE" : 'SZLIMIT= -l'` -eq 11 ]; then
            CURSIZELIMIT=`echo $LINE | awk '{print $3}'`
            let SIZELIMIT=$BAB_SIZE+$CURSIZELIMIT
        else
            let SIZELIMIT=$BAB_SIZE+16000000
        fi
        $MAKE -f config.mk SZLIMIT=" -l $SIZELIMIT"
        exitval=$?
    fi 
