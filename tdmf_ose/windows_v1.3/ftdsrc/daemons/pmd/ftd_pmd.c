...
PrimaryThread(LPDWORD param)
{
    ftd_lg_t    *lgp = NULL;
#endif
    int     lgnum, rc = 0,tcpwin;
    int     cstate, dstate, pstate;
    u_char      inchar;
    int     portnum, n;
    ftd_header_t    header;
    char        procname[80],tcpwinsize[256];
    int lgnum_trace;
    ...
    if (ftd_init_errfac("Replicator", procname, NULL, NULL, 1, 0) == NULL) {
        Return(1);
    }
...
// SAUMYA_FIX:: Remove this code, not needed for new architecture
    ...
    if (ftd_sock_set_opt(lgp->dsockp, SOL_SOCKET, SO_KEEPALIVE,
		...
		rc = FTD_LG_NET_BROKEN;
        goto errexit;
    }
     
	//Changed by Veera, to account for relaunch of the PMD 04-20-2004
	rc = ftd_sock_connect_forever(lgp, portnum); 
    if (rc < 0) // caution: socket is disconnecting here , ....
    {
    ...
    error_tracef( TRACEWRN, "PMD%d THREAD:TCP_WINDOWS_SIZE (SO_SNDBUF) = %d,rc=%d",lgp->lgnum,tcpwin,rc );      
    n = sizeof (int);
    tcpwin = 0;
    rc = ftd_sock_get_opt(lgp->dsockp, SOL_SOCKET, SO_RCVBUF,(char*)&tcpwin, &n);  
    error_tracef( TRACEWRN, "PMD%d THREAD:TCP_WINDOWS_SIZE (SO_RCVBUF) = %d,rc=%d",lgp->lgnum,tcpwin,rc );    
// PB --
    ...
    // Indicate ACTIVE level for this LG in PSTORE
    ps_set_group_state_stamp(lgp->cfgp->pstore, lgp->devname, LG_ACTIVE);
    ...
    if (rc == FTD_LG_NET_BROKEN || rc == FTD_EXIT_NETWORK) 
        error_tracef( TRACEINF,"%s (FTD_EXIT_NETWORK): Id %d, ftd_fsm rc = 0x%x" ,procname, GetCurrentThreadId(), rc);
    else
        error_tracef( TRACEINF,"%s (FTD_EXIT_DIE): Id %d, ftd_fsm rc = 0x%x" ,procname, GetCurrentThreadId(), rc);
...
errexit:
    ...
    // set connection status to ACCUMULATE !!!
        ftd_stat_set_connect(lgp, -1);
    // Keep the RUN STATE in PSTORE for Remote Wakeup !!!
