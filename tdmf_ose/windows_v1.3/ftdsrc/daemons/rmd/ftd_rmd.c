...
static int rmd_initialized_from_pmd(ftd_lg_t *lgp)
{
    char           tcpwinsize[256];
    int            rc, n, i, tcpwin;
    int            rmd_initialized_ok=FALSE;
    ...
    // PB: the following code was missing, so the tcpwindow size was using the default (8k) !!! ++
    // maybe easier usings args but ...
    ...
    if (ftd_sock_set_opt(lgp->dsockp, SOL_SOCKET, SO_SNDBUF,(char*)&n, sizeof(int)) < 0)
        {
        error_tracef( TRACEERR, "THREAD:Calling ftd_sock_set_opt():SO_SNDBUF" );
        return -1;
        }
    if (ftd_sock_set_opt(lgp->dsockp, SOL_SOCKET, SO_RCVBUF, (char*)&n, sizeof(int)) < 0)
        {
    ...
    n = sizeof (int);                                                                               // pb ++
    tcpwin = 0;
    rc = ftd_sock_get_opt(lgp->dsockp, SOL_SOCKET, SO_SNDBUF,(char*)&tcpwin, &n);  
    error_tracef( TRACEWRN, "RMD%d THREAD:TCP_WINDOWS_SIZE (SO_SNDBUF) = %d,rc=%d",lgp->lgnum,tcpwin,rc );      

    n = sizeof (int);
    tcpwin = 0;
    rc = ftd_sock_get_opt(lgp->dsockp, SOL_SOCKET, SO_RCVBUF,(char*)&tcpwin, &n);  
    error_tracef( TRACEWRN, "RMD%d THREAD:TCP_WINDOWS_SIZE (SO_RCVBUF) = %d,rc=%d",lgp->lgnum,tcpwin,rc );    // pb --    
...
*/
    ...
    while (TRUE) 
        ...
        if (i++ == 2) 
            {
            rmd_initialized_ok = TRUE;
            break;
            }
        }

    if (rmd_initialized_ok == FALSE)    
        {error_tracef( TRACEERR, "rmd_initialized_from_pmd= ERROR in handshake" );}
    else
        {error_tracef( TRACEINF, "rmd_initialized_from_pmd: OK!" );}


    return (rmd_initialized_ok);
}
//------------------------------------------------------
...
static int ftd_rmd(ftd_lg_t *lgp)
{
...
#if _DEBUG
    OutputDebugString("*** Ftd_rmd ***\n");
#endif

    // get journal state 
    if ((ret = ftd_journal_get_cur_state(lgp->jrnp, 1)) < 0) {
        reporterr(ERRFAC, M_JRNINIT, ERRCRIT, LG_PROCNAME(lgp));
        return FTD_EXIT_DIE;
    }

    error_tracef( TRACEINF4, "<< ftd_rmd+ftd_journal_get_cur_state () ret=%d",ret );
	...
	if (GET_LG_JLESS(lgp->flags) && GET_LG_CPON(lgp->flags))
        {
        UNSET_LG_CPON(lgp->flags);
        UNSET_JRN_CP_ON(lgp->jrnp->flags);
...
ftd_rmd_apply(ftd_lg_t *lgp, int cpon)
{
    ...
    if (!GET_JRN_JLESS(lgp->jrnp->flags))
		{
        #if _DEBUG
        OutputDebugString("*** Ftd_rmd_apply ***\n");
        #endif
        ...
        if ( ret == FTD_EXIT_DRV_FAIL )
            {
            // Signal this error to Master
            memset(&header, 0, sizeof(header));

            header.msgtype = FTDCREFRSRELAUNCH;
    ...
    // SVG2004
    // Added:
    // don't close the device for RMD_A (let RMD do this)
...
RemoteThread(LPDWORD param)
{
	...
	if (ftd_init_errfac("Replicator", procname, NULL, NULL, 1, 1) == NULL) {
		goto errexit;
	}
    ...
    if (!apply)
        {
        if (!rmd_initialized_from_pmd(lgp))
            {
            error_tracef( TRACEERR, "RemoteThread main():%s : rmd_initialized_from_pmd()", procname );
            goto errexit;
            }
        }            
	...
	if (apply) //RMDA  - remote mirror apply 
		...
		if ( rc == 0) 
			...
			{
			if ( rc == FTD_EXIT_DRV_FAIL )  //WR 17511
			{
				//goto errexit; //Fatal error with remote drive(s)
			}
			else
		...
		if ( ( rc == FTD_EXIT_DIE ) || ( rc == FTD_EXIT_DRV_FAIL ) ) 
			{
			err_msg_t	*lerrmsg;
			...
			if (FTD_SOCK_CONNECT(lgp->dsockp)) 
				...
				else
				{
					...
					header.msg.lg.data  = ~ROLEPRIMARY;    // PRIMARY ROLE will process 
					header.msg.lg.flags = ROLEPRIMARY;    //
					FTD_SOCK_SEND_HEADER( FALSE,LG_PROCNAME(lgp) ,"RemoteThread",lgp->dsockp, &header);
				}
