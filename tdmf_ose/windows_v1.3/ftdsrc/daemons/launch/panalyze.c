 ...
 */
unsigned long proc_argv(int argc, char **argv)
{
    int             devcnt, ch;
...
verify_journal_size(int lgnum)
{
    ...
    for (i = 0; i< group_attributes.num_device;i++)
    {
        ...
        {
            fprintf(stderr, 
                    "\nSize for device: %s Disk %d%s\n", 
                    dev_info[group_attributes.num_device - i -1].name, 
                    uiDiskRequired < 5000 ? uiDiskRequired : (uiDiskRequired < 1500000 ? uiDiskRequired/1000 : uiDiskRequired/(1000*1000)),
                    uiDiskRequired < 5000 ? "KB" : (uiDiskRequired < 1500000 ? "MB" : "GB") );
        }
        ...
        if (bhighres) // use high resolution bitmaps if possible (don't use pstore if at all possible!)
        {
            ...
            uiDiskRequired = (((dip[i].disksize / 2) + 1) * dPercentage);

            if (verbose)
            {
                fprintf(stderr,
                        "HRDB usage is %d/%dbits %2.3f%% - disk size required is %d%s\n", 
                        total,
                        dip[i].hrdb_numbits,
                        dPercentage*100, 
                        uiDiskRequired < 5000 ? uiDiskRequired : (uiDiskRequired < 1500000 ? uiDiskRequired/1000 : uiDiskRequired/(1000*1000)),
        ...
        if (blowres) // use low resolution bitmaps from driver if possible (don't use pstore)
        {
            ...
            {
                fprintf(stderr,
                        "LRDB usage is %d/%dbits %2.3f%% - disk size required is %d%s\n", 
                        total,
                        dip[i].lrdb_numbits,
                        dPercentage*100, 
                        uiDiskRequired < 5000 ? uiDiskRequired : (uiDiskRequired < 1500000 ? uiDiskRequired/1000 : uiDiskRequired/(1000*1000)),
    ...
    if (verbose)
    {
        ...
        {
            fprintf(stderr, 
                    "HRDB based size requires %d%s for journals\n",
                    uiMBrequiredHRDB < 5000 ? uiMBrequiredHRDB : (uiMBrequiredHRDB < 1500000 ? uiMBrequiredHRDB/1000 : uiMBrequiredHRDB/(1000*1000)),
                    uiMBrequiredHRDB < 5000 ? "KB" : "MB");
        }

        if (blowres)
        {
            fprintf(stderr, 
                    "LRDB based size requires %d%s for journals\n",
                    uiMBrequiredLRDB < 5000 ? uiMBrequiredLRDB : (uiMBrequiredLRDB < 1500000 ? uiMBrequiredLRDB/1000 : uiMBrequiredLRDB/(1000*1000)),
                    uiMBrequiredLRDB < 5000 ? "KB" : "MB");
        }
    }
    else
    {
        ...
        {
            fprintf(stderr, 
                    "Journal size required for group %d is %d%s\n",
                    lgnum,
                    uiMBrequiredHRDB < 5000 ? uiMBrequiredHRDB : (uiMBrequiredHRDB < 1500000 ? uiMBrequiredHRDB/1000 : uiMBrequiredHRDB/(1000*1000)),
                    uiMBrequiredHRDB < 5000 ? "KB" : (uiMBrequiredHRDB < 1500000 ? "MB" : "GB"));
        }
        else
        {
            fprintf(stderr, 
                    ...
                    lgnum,
                    uiMBrequiredLRDB < 5000 ? uiMBrequiredLRDB : (uiMBrequiredLRDB < 1500000 ? uiMBrequiredLRDB/1000 : uiMBrequiredLRDB/(1000*1000)),
                    uiMBrequiredLRDB < 5000 ? "KB" : (uiMBrequiredLRDB < 1500000 ? "MB" : "GB" ));
        }
    }
...
int main (int argc, char **argv, char **envp)
{
    ftd_lg_cfg_t    **cfgpp;
    int             found;
    unsigned long   ulTarget = 0;
    char            VersionName[20];

    memset(targets, 0, sizeof(targets));
    memcpy(VersionName,VERSION,strlen(VERSION));
    strtok(VersionName," ");
    ...
    if (ftd_init_errfac("Replicator", argv[0], NULL, NULL, 1, 1) == NULL) 
        goto errexit;
