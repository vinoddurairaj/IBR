...
//// NOTES
//
// The SFTK_TDI_xxx utilities are general purpose, and are intended to be used
...
static NTSTATUS
_I_SFTK_TDI_SimpleTdiRequestComplete(
...
VOID
_I_SFTK_TDI_RefTransportAddress(
   PSFTK_TDI_ADDRESS             pSFTK_TDI_Address
   );

VOID
_I_SFTK_TDI_DerefTransportAddress(
   PSFTK_TDI_ADDRESS             pSFTK_TDI_Address
   );

NTSTATUS
_I_SFTK_TDI_OpenConnectionContext(
...
NTSTATUS
_I_SFTK_TDI_CloseConnectionContext(
   PSFTK_TDI_ENDPOINT   pSFTK_TDI_Endpoint
   );

VOID
_I_SFTK_TDI_RefConnectionEndpoint(
   PSFTK_TDI_ENDPOINT   pSFTK_TDI_Endpoint
   );

VOID
_I_SFTK_TDI_DerefConnectionEndpoint(
   PSFTK_TDI_ENDPOINT   pSFTK_TDI_Endpoint
   );

NTSTATUS
_I_SFTK_TDI_AssociateAddress(
   PSFTK_TDI_ADDRESS    pSFTK_TDI_Address,
   PSFTK_TDI_ENDPOINT pSFTK_TDI_Endpoint
   );

NTSTATUS
_I_SFTK_TDI_DisassociateAddress(
   PSFTK_TDI_ENDPOINT pSFTK_TDI_Endpoint
   );
...
typedef
struct _SFTK_TDI_ADDRESS_REQUEST_CONTEXT
{
   PSFTK_TDI_ADDRESS                      m_pSFTK_TDI_Address;
   HANDLE                           m_CompletionEvent;
   PSFTK_TDI_REQUEST_COMPLETION_ROUTINE   m_CompletionRoutine;
   PVOID                            m_CompletionContext;
   PIO_STATUS_BLOCK                 m_pIoStatusBlock;
   IO_STATUS_BLOCK                  m_SafeIoStatusBlock;
   ULONG                            m_Reserved;
}
   SFTK_TDI_ADDRESS_REQUEST_CONTEXT, *PSFTK_TDI_ADDRESS_REQUEST_CONTEXT;


typedef
struct _SFTK_TDI_ENDPOINT_REQUEST_CONTEXT
{
   PSFTK_TDI_ENDPOINT                     m_pSFTK_TDI_Endpoint;
   HANDLE                           m_CompletionEvent;
   PSFTK_TDI_REQUEST_COMPLETION_ROUTINE   m_CompletionRoutine;
   PVOID                            m_CompletionContext;
   PIO_STATUS_BLOCK                 m_pIoStatusBlock;
   ...
   SFTK_TDI_ENDPOINT_REQUEST_CONTEXT, *PSFTK_TDI_ENDPOINT_REQUEST_CONTEXT;
...
//
//   pSFTK_TDI_Address
//      Pointer to a caller-provided SFTK_TDI_ADDRESS structure that will be
...
//
// The SFTK_TDI_BuildEaBuffer function is used to build the extended attributes
// buffer.
//
...
// will remain valid.
//
// Callers of SFTK_TDI_OpenTransportAddress must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   ASSERT( pSFTK_TDI_Address );

   //
   // Initialize SFTK_TDI_ADDRESS Structure
   //
   pSFTK_TDI_Address->m_nStatus = STATUS_UNSUCCESSFUL;
   pSFTK_TDI_Address->m_pFileObject = NULL;
   pSFTK_TDI_Address->m_pAtomicIrp = NULL;
   pSFTK_TDI_Address->m_ReferenceCount = 1;

   InitializeListHead( &pSFTK_TDI_Address->m_ConnectionList );

   TransportAddressLength = SFTK_TDI_TransportAddressLength( pTransportAddress );
   ...
   {
      return( pSFTK_TDI_Address->m_nStatus = Status );
   }
   ...
   //
   Status = ZwCreateFile(
               &pSFTK_TDI_Address->m_hAddress,                   // Handle
...
//			   |FILE_SHARE_WRITE|FILE_SHARE_DELETE,                            // Sharing attributes
   ...
   if (NT_SUCCESS(Status))
   {
      ...
      Status = ObReferenceObjectByHandle(
                  pSFTK_TDI_Address->m_hAddress,   // Object Handle
                  FILE_ANY_ACCESS,           // Desired Access
                  NULL,                      // Object Type
                  KernelMode,                // Processor mode
                  (PVOID *)&pSFTK_TDI_Address->m_pFileObject,   // File Object pointer
                  NULL                       // Object Handle information
                  );

      pDeviceObject = IoGetRelatedDeviceObject( pSFTK_TDI_Address->m_pFileObject );

      //
      // Allocate IRP For General Purpose Use
      //
      pSFTK_TDI_Address->m_pAtomicIrp = IoAllocateIrp(
                                    pDeviceObject->StackSize,
                                    FALSE
                                    );

      if( !pSFTK_TDI_Address->m_pAtomicIrp )
      {
         SFTK_TDI_CloseTransportAddress( pSFTK_TDI_Address );

         return( pSFTK_TDI_Address->m_nStatus = STATUS_INSUFFICIENT_RESOURCES );
      }
   }
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_RefTransportAddress (INTERNAL/PRIVATE)
//
// Purpose
...
// Parameters
//   pSFTK_TDI_Address
//      Pointer to the SFTK_TDI_ADDRESS structure whose reference count should
//      be incremented.
//
// Return Value
// None.
...
{
   ASSERT( pSFTK_TDI_Address );
   ASSERT( pSFTK_TDI_Address->m_ReferenceCount > 0 );

   if( pSFTK_TDI_Address )
   {
      InterlockedIncrement( &pSFTK_TDI_Address->m_ReferenceCount );
   }
}


/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_DestroyTransportAddress (INTERNAL/PRIVATE)
//
// Purpose
...
// Parameters
//   pSFTK_TDI_Address
//      Pointer to the SFTK_TDI_ADDRESS structure that specifies the transport
...
//
// Remarks
// This function will free resources allocated in SFTK_TDI_OpenTransportAddress,
// dereference the transport address file object pointer anc actually
// close the transport address file handle.
//
// Callers of _I_SFTK_TDI_DestroyTransportAddress must be running at IRQL
// PASSIVE_LEVEL.
//

NTSTATUS
_I_SFTK_TDI_DestroyTransportAddress(
   PSFTK_TDI_ADDRESS pSFTK_TDI_Address
   )
{
   NTSTATUS Status = STATUS_SUCCESS;

   KdPrint(("SFTK_TDI_DestroyTransportAddress: Entry...\n"));

   ASSERT( pSFTK_TDI_Address );
   ASSERT( pSFTK_TDI_Address->m_ReferenceCount == 0 );
   ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
   ASSERT( IsListEmpty( &pSFTK_TDI_Address->m_ConnectionList ) );
   ...
   if( pSFTK_TDI_Address->m_pAtomicIrp )
   {
      IoFreeIrp( pSFTK_TDI_Address->m_pAtomicIrp );
   }
   ...
   //
   if( pSFTK_TDI_Address->m_pFileObject != NULL )
   {
      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
      ObDereferenceObject( pSFTK_TDI_Address->m_pFileObject );
   }

   pSFTK_TDI_Address->m_pFileObject = NULL;

   ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
   ZwClose( pSFTK_TDI_Address->m_hAddress );
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_DerefTransportAddress (INTERNAL/PRIVATE)
//
// Purpose
...
// Parameters
//   pSFTK_TDI_Address
//      Pointer to the SFTK_TDI_ADDRESS structure whose reference count should be
//      decremented.
//
// Return Value
// None.
//
// Remarks
// The SFTK_TDI_ADDRESS structure will continue to exist until the reference count
// is deceremented to zero. When ther reference count is decremented to zero
// the _I_SFTK_TDI_DestroyTransportAddress function is finally called to destroy
   ...
   )
{
   ...
   if( pSFTK_TDI_Address )
   {
      if( pSFTK_TDI_Address->m_ReferenceCount ) // ATTENTION!!! Is this test OK on SMP???
      {
         Result = InterlockedDecrement( &pSFTK_TDI_Address->m_ReferenceCount );
         ...
         if( !Result )
         {
            ...
            //
            _I_SFTK_TDI_DestroyTransportAddress( pSFTK_TDI_Address );
         }
      }
   }
}
...
//
// Purpose
// This routine closes the transport address object specified by pSFTK_TDI_ADDRESS.
//
// Parameters
//   pSFTK_TDI_Address
//      Pointer to the SFTK_TDI_ADDRESS structure that specifies the transport
...
//
// Remarks
// This function calls _I_SFTK_TDI_DerefTransportAddress to decrement the reference
// count for the SFTK_TDI_ADDRESS structure. If the reference count is decremented
// to zero by this call, the transport address object specified by pSFTK_TDI_ADDRESS
// will actually be closed.
//
// Callers of SFTK_TDI_TransportAddress must be running at IRQL PASSIVE_LEVEL.
//
...
{
   KdPrint(("SFTK_TDI_CloseTransportAddress: Entry...\n"));

   ASSERT( pSFTK_TDI_Address );

   if( pSFTK_TDI_Address )
   {
      _I_SFTK_TDI_DerefTransportAddress( pSFTK_TDI_Address );
   }
...
// you pass a pEventContext value to SFTK_TDI_SetEventHandlers; this value is simply
...
// for more information.
//
// Callers of SFTK_TDI_SetEventHandlers must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   KdPrint(("SFTK_TDI_SetEventHandlers: Entry...\n") );
   ...
   do
   {
      ...
      TdiBuildSetEventHandler(
         pSFTK_TDI_Address->m_pAtomicIrp,
         pDeviceObject,
         pSFTK_TDI_Address->m_pFileObject,
         NULL,
         NULL,
         TDI_EVENT_CONNECT,
         ConnectEventHandler,
      ...
      //
      Status = SFTK_TDI_MakeSimpleTdiRequest(
                  pDeviceObject,
                  pSFTK_TDI_Address->m_pAtomicIrp
                  );
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_AddressRequestComplete (INTERNAL/PRIVATE)
//
// Purpose
//
// Parameters
...
static NTSTATUS
_I_SFTK_TDI_AddressRequestComplete(
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP pIrp,
    IN PVOID Context
    )
{
   PSFTK_TDI_ADDRESS_REQUEST_CONTEXT   pSFTK_TDI_RequestContext;
   PKEVENT                       pEvent;

   pSFTK_TDI_RequestContext = (PSFTK_TDI_ADDRESS_REQUEST_CONTEXT )Context;

   if( pSFTK_TDI_RequestContext != NULL )
   {
      //
      // Fill IoStatusBlock With Final Status And Information
      //
      (pSFTK_TDI_RequestContext->m_pIoStatusBlock)->Status = pIrp->IoStatus.Status;
      (pSFTK_TDI_RequestContext->m_pIoStatusBlock)->Information = pIrp->IoStatus.Information;

      //
      // Set The Completion Event, If Specified
      //
      if( pSFTK_TDI_RequestContext->m_CompletionEvent )
      {
         KeSetEvent( pSFTK_TDI_RequestContext->m_CompletionEvent, 0, FALSE);
      }

      //
      // Call The Completion Routine, If Specified
      //
      if( pSFTK_TDI_RequestContext->m_CompletionRoutine )
      {
         (pSFTK_TDI_RequestContext->m_CompletionRoutine)(
               pSFTK_TDI_RequestContext->m_CompletionContext,
               pSFTK_TDI_RequestContext->m_pIoStatusBlock,
               pSFTK_TDI_RequestContext->m_Reserved
               );
      }
      ...
      //
      _I_SFTK_TDI_DerefTransportAddress( pSFTK_TDI_RequestContext->m_pSFTK_TDI_Address );

      //
      // Free Memory Allocated For Private Completion Context
      //
      NdisFreeMemory(
         pSFTK_TDI_RequestContext,
         sizeof( SFTK_TDI_ADDRESS_REQUEST_CONTEXT ),
         0
         );
   }
...
// calling SFTK_TDI_SendDatagramOnAddress.
//
...
// SFTK_TDI_SendDatagramOnAddress must be running at IRQL PASSIVE_LEVEL. If the
   ...
   )
{
   ...
   PSFTK_TDI_ADDRESS_REQUEST_CONTEXT   pSFTK_TDI_RequestContext = NULL;
   PDEVICE_OBJECT                pDeviceObject;
   PMDL                          pNextMdl;
   ...
   RtlZeroMemory( pSFTK_TDI_RequestContext, sizeof( SFTK_TDI_ADDRESS_REQUEST_CONTEXT ) );

   if( pIoStatusBlock )
   {
      pSFTK_TDI_RequestContext->m_pIoStatusBlock = pIoStatusBlock;
   }
   else
   {
      pSFTK_TDI_RequestContext->m_pIoStatusBlock = &pSFTK_TDI_RequestContext->m_SafeIoStatusBlock;
   }

   pSFTK_TDI_RequestContext->m_pSFTK_TDI_Address = pSFTK_TDI_Address;

   if( UserCompletionRoutine )
   {
      pSFTK_TDI_RequestContext->m_CompletionEvent = UserCompletionEvent;
      pSFTK_TDI_RequestContext->m_CompletionRoutine = UserCompletionRoutine;
      pSFTK_TDI_RequestContext->m_CompletionContext = UserCompletionContext;
   ...
   {
      pSFTK_TDI_RequestContext->m_CompletionEvent = UserCompletionEvent;
      KeResetEvent( pSFTK_TDI_RequestContext->m_CompletionEvent );

      pSFTK_TDI_RequestContext->m_CompletionRoutine = NULL;
      pSFTK_TDI_RequestContext->m_CompletionContext = NULL;

      bSynchronous = FALSE;
   }
   else
   {
      pSFTK_TDI_RequestContext->m_CompletionEvent = &SyncEvent;

      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );

      KeInitializeEvent(
         pSFTK_TDI_RequestContext->m_CompletionEvent,
         NotificationEvent,
         FALSE
         );
   ...
   TdiBuildSendDatagram(
      ...
      pDeviceObject,
      pSFTK_TDI_Address->m_pFileObject,
      _I_SFTK_TDI_AddressRequestComplete, // Completion routine
      pSFTK_TDI_RequestContext,           // Completion context
   ...
   //
   _I_SFTK_TDI_RefTransportAddress( pSFTK_TDI_RequestContext->m_pSFTK_TDI_Address );

   //
   // Submit the request
   //
   (pSFTK_TDI_RequestContext->m_pIoStatusBlock)->Status = STATUS_UNSUCCESSFUL;
   (pSFTK_TDI_RequestContext->m_pIoStatusBlock)->Information = 0;
...
//
// If the synchronous mode of operation is employed, then the caller of
// SFTK_TDI_ReceiveDatagramOnAddress must be running at IRQL PASSIVE_LEVEL. If the
// caller intends to wait on the event specified by UserCompletionEvent,
// then the waiting must be performed at IRQL PASSIVE_LEVEL.
//
// For all methods the IO_STATUS_BLOCK pointed to by pIoStatusBlock will
...
//
// Purpose
// SFTK_TDI_QueryAddressInfo makes a TDI_QUERY_INFORMATION request for
// TDI_QUERY_ADDRESS_INFO for the transport address or connection endpoint
// specified by its file object pointer.
//
// Parameters
//   pFileObject
//      Can be either a TDI transport address file object (from SFTK_TDI_ADDRESS)
//      or TDI connection endpoint file object (from SFTK_TDI_ENDPOINT).
//
//    pInfoBuffer
...
// function.
//
// Callers of SFTK_TDI_QueryAddressInfo must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   pMdl = SFTK_TDI_AllocateAndProbeMdl(
   ...
   TdiBuildQueryInformation(
      ...
      pDeviceObject,
      pFileObject,
      _I_SFTK_TDI_SimpleTdiRequestComplete,  // Completion routine
   ...
   //
   SFTK_TDI_UnlockAndFreeMdl(pMdl);
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_OpenConnectionContext (INTERNAL/PRIVATE)
//
// Purpose
...
//
// Callers of _I_SFTK_TDI_OpenConnectionContext must be running at IRQL PASSIVE_LEVEL.
//
  ...
  )
{
   ...
   //
   pSFTK_TDI_Endpoint->m_nOpenStatus = STATUS_UNSUCCESSFUL;
   pSFTK_TDI_Endpoint->m_pFileObject = NULL;
   ...
   if( !NT_SUCCESS(Status) )
   {
      return( pSFTK_TDI_Endpoint->m_nOpenStatus = Status );
   }
   ...
   //
   Status = ZwCreateFile(
               &pSFTK_TDI_Endpoint->m_hContext,                // Handle
               GENERIC_READ | GENERIC_WRITE | SYNCHRONIZE, // Desired Access
               &ObjectAttributes,                          // Object Attributes
               &IoStatusBlock,                             // Final I/O status block
               0,                                          // Allocation Size
   ...
   if (NT_SUCCESS(Status))
   {
      ...
      Status = ObReferenceObjectByHandle(
                  pSFTK_TDI_Endpoint->m_hContext,// Object Handle
                  FILE_ANY_ACCESS,           // Desired Access
                  NULL,                      // Object Type
                  KernelMode,                // Processor mode
                  (PVOID *)&pSFTK_TDI_Endpoint->m_pFileObject, // File Object pointer
                  NULL                       // Object Handle information
                  );
   }
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_CloseConnectionContext (INTERNAL/PRIVATE)
//
// Purpose
...
//
// Remarks
// Callers of _I_SFTK_TDI_CloseConnectionContext must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   KdPrint(("_I_SFTK_TDI_CloseConnectionContext: Entry...\n"));
   ...
   // Dereference The File Object
   //
   if( pSFTK_TDI_Endpoint->m_pFileObject != NULL )
   {
      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
      ObDereferenceObject( pSFTK_TDI_Endpoint->m_pFileObject );

      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
      ZwClose( pSFTK_TDI_Endpoint->m_hContext );
   }
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_AssociateAddress (INTERNAL/PRIVATE)
//
// Purpose
//
// Parameters
   ...
   )
{
   ...
   pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address = NULL;
   pSFTK_TDI_Endpoint->m_nAssociateStatus = STATUS_UNSUCCESSFUL;
   ...
   TdiBuildAssociateAddress(
      pSFTK_TDI_Address->m_pAtomicIrp,
      pDeviceObject,
      pSFTK_TDI_Endpoint->m_pFileObject,
      NULL,
      NULL,
      pSFTK_TDI_Address->m_hAddress
      );

   //
   // Submit The Request To The Transport
   //
   pSFTK_TDI_Endpoint->m_nAssociateStatus = SFTK_TDI_MakeSimpleTdiRequest(
                                          pDeviceObject,
                                          pSFTK_TDI_Address->m_pAtomicIrp
                                          );

   if( NT_SUCCESS( pSFTK_TDI_Endpoint->m_nAssociateStatus ) )
   {
      pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address = pSFTK_TDI_Address;

      _I_SFTK_TDI_RefTransportAddress( pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address );
   }

   return( pSFTK_TDI_Endpoint->m_nAssociateStatus );
}


/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_DisassociateAddress (INTERNAL/PRIVATE)
//
// Purpose
//
// Parameters
   ...
   )
{
   ...
   pDeviceObject = IoGetRelatedDeviceObject( pSFTK_TDI_Endpoint->m_pFileObject );
   
   // Disassociate the local endpoint with the address object.
   if( pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address )
   {
      TdiBuildDisassociateAddress(
         (pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address)->m_pAtomicIrp,
         pDeviceObject,
         pSFTK_TDI_Endpoint->m_pFileObject,
         NULL,
         NULL
   ...
   if( NT_SUCCESS( Status ) )
   {
      if( pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address )
      {
         _I_SFTK_TDI_DerefTransportAddress( pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address );
      }
   }
...
//
//   pSFTK_TDI_Address
//      Pointer to the SFTK_TDI_ADDRESS structure previously opened using the
//      LS_OpenTransportAddress function. The SFTK_TDI_ADDRESS pointer specifies
//      the transport address object to be used with the new connection
//      endpoint.
//
//   pSFTK_TDI_Endpoint
//      Pointer to a caller-provided SFTK_TDI_ENDPOINT structure that will be
...
// address object using  SFTK_TDI_OpenTransportAddress.
//
...
// an internal function called _I_SFTK_TDI_AssociateAddress to associate the new connection
...
// SFTK_TDI_ENDPOINT structure. A pointer to a SFTK_TDI_ENDPOINT structure that has been successfully
// initialized by SFTK_TDI_OpenConnectionEndpoint is used by subsequent calls that operate
// on a connection endpoint.
//
// It is important to note that SFTK_TDI_OpenConnectionEndpoint will call ZwCreateFile in the
...
// information.
//
// Callers of SFTK_TDI_OpenConnectionEndpoint must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   KdPrint(("SFTK_TDI_OpenConnectionEndpoint: Entry...\n") );

   ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );

   //
   // Initialize SFTK_TDI_CONTEXT Structure
   //
   NdisZeroMemory( pSFTK_TDI_Endpoint, sizeof( SFTK_TDI_ENDPOINT ) );

   pSFTK_TDI_Endpoint->m_ReferenceCount = 1;
   ...
   //
   Status = _I_SFTK_TDI_OpenConnectionContext(
                  TransportDeviceNameW,
                  pSFTK_TDI_Endpoint,
                  pContext
                  );
   ...
   if( !NT_SUCCESS( Status ) )
   {
      return( pSFTK_TDI_Endpoint->m_nOpenStatus );
   }

   KdPrint(("SFTK_TDI_OpenConnectionEndpoint: Opened Connection Context\n") );

   //
   // Associate Address
   //
   Status = _I_SFTK_TDI_AssociateAddress( pSFTK_TDI_Address, pSFTK_TDI_Endpoint );

   if( NT_SUCCESS( Status ) )
   {
      KdPrint(("SFTK_TDI_OpenConnectionEndpoint: Associated Address\n") );

      //
      // Add The Connection To The Address Object's Connection List
      //
      InsertTailList(
         &pSFTK_TDI_Address->m_ConnectionList,
         &pSFTK_TDI_Endpoint->m_ListElement
         );
   }
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_RefConnectionEndpoint (INTERNAL/PRIVATE)
//
// Purpose
...
{
   InterlockedIncrement( &pSFTK_TDI_Endpoint->m_ReferenceCount );
}


/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_DestroyConnection (INTERNAL/PRIVATE)
//
// Purpose
//
// Parameters
   ...
   )
{
   ...
   // --------------------------
   // Logically one would think it appropriate to call _I_SFTK_TDI_DisassociateAddress
   ...
   //
   // Further, on TCP it appears that a call to _I_SFTK_TDI_DisassociateAddress
   // will often fail with STATUS_CONNECTION_ACTIVE (0xC000023B).
   //
   // So, instead of calling _I_SFTK_TDI_DisassociateAddress we just remove
   // the reference counts to the transport address and connection context
   // objects that was created by _I_SFTK_TDI_AssociateAddress.
   //
   _I_SFTK_TDI_DerefTransportAddress( pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address );
   ...
   //
   RemoveEntryList( &pSFTK_TDI_Endpoint->m_ListElement );

   //
   // Close Connection Context
   //
   Status = _I_SFTK_TDI_CloseConnectionContext( pSFTK_TDI_Endpoint );

#ifdef DBG
   if( NT_SUCCESS( Status ) )
   {
      KdPrint(("SFTK_TDI_DestroyConnection: Closed Connection Context\n") );
   }
#endif // DBG
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_DerefConnectionEndpoint (INTERNAL/PRIVATE)
//
// Purpose
   ...
   )
{
   ...
   Result = InterlockedDecrement( &pSFTK_TDI_Endpoint->m_ReferenceCount );
   ...
   if( !Result )
   {
      ...
      //
      _I_SFTK_TDI_DestroyConnection( pSFTK_TDI_Endpoint );
   }
}
...
//
// Purpose
// SFTK_TDI_CloseConnectionEndpoint closes the connection endpoint specified by
// pSFTK_TDI_ENDPOINT.
//
// Parameters
// pSFTK_TDI_Endpoint
//    Pointer to the SFTK_TDI_ENDPOINT structure that specifies the connection endpoint
...
//
// Remarks
// This function calls the internal function _I_SFTK_TDI_DerefConnectionEndpoint to
// decrement the reference count for the SFTK_TDI_ENDPOINT structure. If the reference
// count is decremented to zero by this call, the connection endpoint object
// specified by pSFTK_TDI_ENDPOINT will actually be closed.
//
// Callers of SFTK_TDI_CloseConnectionEndpoint must be running at IRQL PASSIVE_LEVEL.
//
...
{
   KdPrint(("SFTK_TDI_CloseConnectionEndpoint: Entry...\n"));

   _I_SFTK_TDI_DerefConnectionEndpoint( pSFTK_TDI_Endpoint );
...
// Parameters
//   pSFTK_TDI_Endpoint
//      Pointer to the SFTK_TDI_ENDPOINT structure that specifies the local connection
...
//
// It terms of the KS library functions, this means that the SFTK_TDI_ENDPOINT
// structure pointed to by pSFTK_TDI_Endpoint must have been successfully initialized
// by calling SFTK_TDI_OpenConntectionEndpoint.
//
// See the NT DDK documentation topic "5.5 Making an Endpoint-to-Endpoint
// Connection" for more information.
//
// As currently written, callers of SFTK_TDI_Connect must be running at
// IRQL PASSIVE_LEVEL.
//
...
//	TDI_CONNECTION_INFORMATION ReturnConnectionInfo;
   ...
   KdPrint(("SFTK_TDI_Connect: Entry...\n"));
...
//	TDI_CONNECTION_INFORMATION ReturnConnectionInfo;
   ...
   //
	...
	//		&ReturnConnectionInfo
		...
		Status = IoCallDriver( pDeviceObject, (pSFTK_TDI_Endpoint->m_pSFTK_TDI_Address)->m_pAtomicIrp );
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_EndpointRequestComplete (INTERNAL/PRIVATE)
//
// Purpose
//
// Parameters
...
static NTSTATUS
_I_SFTK_TDI_EndpointRequestComplete(
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP pIrp,
    IN PVOID Context
    )
{
   PSFTK_TDI_ENDPOINT_REQUEST_CONTEXT  pSFTK_TDI_RequestContext;
   PKEVENT                       pEvent;
   BOOLEAN                       bSynchronous;

   pSFTK_TDI_RequestContext = (PSFTK_TDI_ENDPOINT_REQUEST_CONTEXT )Context;

   if( pSFTK_TDI_RequestContext != NULL )
   {
      bSynchronous = pSFTK_TDI_RequestContext->m_bSynchronous;
      ...
      //
      _I_SFTK_TDI_DerefConnectionEndpoint( pSFTK_TDI_RequestContext->m_pSFTK_TDI_Endpoint );
      ...
      NdisFreeMemory(
         pSFTK_TDI_RequestContext,
         sizeof( SFTK_TDI_ENDPOINT_REQUEST_CONTEXT ),
         0
         );
   ...
   )
{
   ...
   KEVENT                        SyncEvent;
   BOOLEAN                       bSynchronous = FALSE;
   PSFTK_TDI_ENDPOINT_REQUEST_CONTEXT  pSFTK_TDI_RequestContext = NULL;
   PDEVICE_OBJECT                pDeviceObject;
   ...
   RtlZeroMemory( pSFTK_TDI_RequestContext, sizeof( SFTK_TDI_ENDPOINT_REQUEST_CONTEXT ) );
   ...
   //
   pSFTK_TDI_RequestContext->m_pSFTK_TDI_Endpoint = pSFTK_TDI_Endpoint;
   ...
   pSFTK_TDI_RequestContext->m_bSynchronous = bSynchronous;
   ...
   TdiBuildDisconnect(
      ...
      pDeviceObject,
      pSFTK_TDI_Endpoint->m_pFileObject,
      _I_SFTK_TDI_EndpointRequestComplete,// Completion routine
      pSFTK_TDI_RequestContext,           // Completion context
//		&DisconnectTimeOut,           // Timeout Information
   ...
   //
   _I_SFTK_TDI_RefConnectionEndpoint( pSFTK_TDI_RequestContext->m_pSFTK_TDI_Endpoint );
   ...
   if( !NT_SUCCESS( Status ) )
   {
      KdPrint(( "SFTK_TDI_Disconnect: Return Status 0x%8.8x\n", Status ));
   }
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_DoSendOrReceive (INTERNAL/PRIVATE)
//
// Purpose
   ...
   )
{
   ...
   if( !NT_SUCCESS( Status ) )
   {
      KdPrint(( "_I_SFTK_TDI_DoSendOrReceive: Return Status 0x%8.8x\n", Status ));
   }
...
//
//    pSFTK_TDI_Endpoint
//       Pointer to the SFTK_TDI_ENDPOINT structure that specifies the connection
...
//
// Each of the MDLs in the chain must have been probed and locked before
// calling SFTK_TDI_SendOnEndpoint.
//
// Synchronous/Asynchronous Operation
// ----------------------------------
// Synchronous Send
...
//
// If the synchronous mode of operation is employed, then the caller of
// SFTK_TDI_SendOnEndpoint must be running at IRQL PASSIVE_LEVEL. If the caller
   ...
   )
{
   ...
   //
   Status = _I_SFTK_TDI_DoSendOrReceive(
               pSFTK_TDI_Endpoint,
               UserCompletionEvent,
               UserCompletionRoutine,
               UserCompletionContext,
   ...
   )
{
   ...
   // Call Common Routine To Do The Work
   //
   Status = _I_SFTK_TDI_DoSendOrReceive1(
               pSFTK_TDI_Endpoint,
               UserCompletionEvent,
               UserCompletionRoutine,
               UserCompletionContext,
...
//
// Remarks
// Callers of SFTK_TDI_QueryConnectionInfo must be running at IRQL PASSIVE_LEVEL.
//
...
//
// Remarks
// Callers of SFTK_TDI_SetConnectionInfo must be running at IRQL PASSIVE_LEVEL.
//
...
//
// Remarks
// Callers of SFTK_TDI_OpenControlChannel must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   //
   // Initialize SFTK_TDI_CHANNEL Structure
   //
   pSFTK_TDI_Channel->m_pFileObject = NULL;
   ...
   Status = ZwCreateFile(
               &pSFTK_TDI_Channel->m_hControlChannel,// Handle
   ...
   Status = ObReferenceObjectByHandle (
               pSFTK_TDI_Channel->m_hControlChannel,      // Object Handle
               FILE_ANY_ACCESS,                     // Desired Access
               NULL,                                // Object Type
               KernelMode,                          // Processor mode
               (PVOID *)&pSFTK_TDI_Channel->m_pFileObject,// Object pointer
               NULL);                               // Object Handle information

   if( !NT_SUCCESS(Status) )
   {
      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
      ZwClose( pSFTK_TDI_Channel->m_hControlChannel );
...
//
// Purpose
// Close a control chanel previously opened using SFTK_TDI_OpenControlChannel.
//
// Parameters
//
// Return Value
//
// Remarks
// Callers of SFTK_TDI_CloseControlChannel must be running at IRQL PASSIVE_LEVEL.
//
...
{
   if( pSFTK_TDI_Channel->m_pFileObject )
   {
      ASSERT( KeGetCurrentIrql() == PASSIVE_LEVEL );
      ObDereferenceObject( pSFTK_TDI_Channel->m_pFileObject );
...
//
// Remarks
// Callers of SFTK_TDI_QueryProviderInfo must be running at IRQL PASSIVE_LEVEL.
//
...
{
   NTSTATUS          Status = STATUS_SUCCESS;
   SFTK_TDI_CHANNEL        SFTK_TDI_Channel;
   PDEVICE_OBJECT    pDeviceObject = NULL;
   PIRP              pIrp = NULL;
   PMDL              pMdl;
   ...
   // Open A TDI Control Channel To The Specified Transport
   //
   Status = SFTK_TDI_OpenControlChannel(
               TransportDeviceNameW,
               &SFTK_TDI_Channel
               );
   ...
   //
   pDeviceObject = IoGetRelatedDeviceObject( SFTK_TDI_Channel.m_pFileObject );
   ...
   if( !pIrp )
   {
      SFTK_TDI_CloseControlChannel( &SFTK_TDI_Channel );
   ...
   TdiBuildQueryInformation(
      pIrp,
      pDeviceObject,
      SFTK_TDI_Channel.m_pFileObject,
      _I_SFTK_TDI_SimpleTdiRequestComplete,    // Completion routine
      NULL,                            // Completion context
   ...
   // Submit The Query To The Transport
   //
   Status = SFTK_TDI_MakeSimpleTdiRequest( pDeviceObject, pIrp );
...
//
// Remarks
// Callers of SFTK_TDI_QueryNetworkAddress must be running at IRQL PASSIVE_LEVEL.
//
   ...
   )
{
   ...
   try
   {
	   ...
	   *ppTransportAddress = NULL;
	   ...
	   NdisZeroMemory(&SFTK_TDI_Channel,sizeof(SFTK_TDI_CHANNEL));
		//
		// Open A TDI Control Channel To The Specified Transport
		//
		Status = SFTK_TDI_OpenControlChannel(
   ...
   finally
   {
	   ...
	   //Free up the MDl
		...
		if(SFTK_TDI_Channel.m_pFileObject != NULL)
		{
			SFTK_TDI_CloseControlChannel( &SFTK_TDI_Channel );
		}
...
//
// Remarks
// Callers of SFTK_TDI_QueryProviderStatistics must be running at IRQL PASSIVE_LEVEL.
//
...
/////////////////////////////////////////////////////////////////////////////
//// _I_SFTK_TDI_SimpleTdiRequestComplete (INTERNAL/PRIVATE)
//
// Purpose
// Completion callback for driver calls initiated by SFTK_TDI_MakeSimpleTdiRequest.
//
// Parameters
   ...
   )
{
   ...
   IoSetCompletionRoutine(
      pIrp,                         // The IRP
      _I_SFTK_TDI_SimpleTdiRequestComplete, // The completion routine
   ...
   if ((Status == STATUS_PENDING) || (Status == STATUS_SUCCESS))
   {
      ...
      if (!NT_SUCCESS(Status))
      {
         KdPrint(("SFTK_TDI_MakeSimpleTdiRequest could not wait Wait returned %lx\n",Status));
         return Status;
      }
   ...
   )
{
   ...
   if (UnicodeDosDeviceName.Buffer != NULL)
   {
      ...
      KdPrint(("SFTK_TDI_CreateSymbolicLink: %s %ws\n",
...
{
	return( SFTK_TDI_htons( netshort ) );
}
...
ULONG
SFTK_TDI_InitProcessNameOffset( VOID )
{
   PEPROCESS   curproc;
   int         i;
...
/////////////////////////////////////////////////////////////////////////////
//// SFTK_TDI_GetCurrentProcessName
//
// Purpose
...
BOOLEAN
SFTK_TDI_GetCurrentProcessName( PCHAR ProcessName )
{
   PEPROCESS   curproc;
   char        *nameptr;
   ULONG       i;
   ...
   )
{
	if( nAddressType == TDI_ADDRESS_TYPE_IP )
	{
		...
		KdPrint(( "sin_port: %d (0x%2.2X)\n",
         SFTK_TDI_ntohs( pIPAddress->sin_port ),
         SFTK_TDI_ntohs( pIPAddress->sin_port )
         ));
	}
}
