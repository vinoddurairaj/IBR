...
//This is a temperory Cache that will be used to Test the TDI Sessions.
//This Cache will have one Big Buffer that will be written to by a Single
//Worker thread in descreet integral values. 
//Like (1K, 4K, 8k, 64k, 256K, 512K, 1MB, 2MB, 4MB, 8MB , 16MB)
//I will use a combination of the above input to write to the Cache.
//The Data Read from the Cache is always up to a max of 4MB
//If the Cache has more data I will simply call it again and again.

struct CACHEFIFO;
typedef struct _CACHEFIFO
{
	PLONG lpRawBuffer;	//Actual Buffer		//This Pointer will be allocated in LONG
	PLONG lpStart;		//Represents the start of the Buffer
	PLONG lpWritePointer;	//The Write Position	//This Pointer will be in LONG
	PLONG lpReadPointer;	//Actual Read Position will be in LONG
	PLONG lpEnd;			//represents the End of the Buffer
	LONG nTotalSize;	//TotalSize of the LONGs Can be only a multiple of SizeOf(LONG)
	FAST_MUTEX	mCacheLock;	//Lock that is used to control Access to the Cache.
	KEVENT	CacheWaitEvent;	//This is used by the GetNextBuffer() to Block until there is some
							//data in the Cache
	LONG nUniqueIndex;	//This Represents a Unique id for this cache
//	CACHEFIFO* lpNext;	//This will be for the Future When we want to chain multiple Cache.
} CACHEFIFO, *PCACHEFIFO;
...
typedef
struct _TCP_DEVICE_CONTEXT
{
   ULONG    TcpStuff;
   TDI_PROVIDER_INFO TdiProviderInfo;

}TCP_DEVICE_CONTEXT, *PTCPC_DEVICE_CONTEXT;

typedef struct _DEVICE_EXTENSION
{
   PDEVICE_OBJECT       pDeviceObject;
   BOOLEAN       bSymbolicLinkCreated;
   CACHEFIFO	CacheManager;
   SESSION_MANAGER	SessionManager;
   PDRIVER_DISPATCH     MajorFunction[IRP_MJ_MAXIMUM_FUNCTION + 1];
   PDEVICE_UNLOAD       DeviceUnload;
   TCP_DEVICE_CONTEXT  TcpContext;
...
///////Event Handlers for various network events
