   ...
   )
{
   ...
   if ( pFileObject )
   {
      ...
      else
      {
         ...
         dStatus = TDI_TdiCall( pIrp, pDeviceObject, &IoStatusBlock );
   ...
   )
{
   ...
   return( TDI_TCPQueryDeviceControl(                        // send down device control call
   ...
   )
{
   ...
   status = TDI_TCPQueryInformation( 
   ...
   )
{
   ...
   status = TDI_TCPQueryDeviceControl(                    // send down device control call
               pFileObject,                              // current transport/connection object
               &QueryInfo,                               // input buffer
               sizeof ( TCP_REQUEST_QUERY_INFORMATION_EX ), // input buffer length
