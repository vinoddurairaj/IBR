...
#pragma message(__LOC__ "High priority threads will be created!")
   		SetThreadPriority(procp->pid, THREAD_PRIORITY_ABOVE_NORMAL);
#else
#pragma message(__LOC__ "Normal priority threads will be created!")
		SetThreadPriority(procp->pid, THREAD_PRIORITY_NORMAL);
#endif
...

		{
			...
			path = strdup(argv[0]); // ardeb moica ! [020724]
									// Does 'path' get released ?
			path[cmd - argv[0]] = '\0';
...
//	   argv[0], cmd, path);
	...
	// ardeb, the following commented code should be added
	// if ( path )
	// {
	//		delete path;
	// }
