...
ftd_lg_cppend(ftd_lg_t *lgp) 
{
	...
	error_tracef( TRACEINF, ">>%s ftd_lg_cppend...now FTDJRNMIR", LG_PROCNAME(lgp) );    
	...
	if ((rc = ftd_journal_create_next_cp(lgp->jrnp, jrnfp)) < 0) 
	    ...
	    if (GET_LG_JLESS(lgp->flags))
		    {
            // No journal to apply, so we fake RMD_APPLY response to go faster
            memset(&header, 0, sizeof(header));
            header.msgtype = FTDCAPPLYDONECPON;
            header.msg.lg.lgnum = lgp->lgnum;
            rc = FTD_SOCK_SEND_HEADER( TRUE,LG_PROCNAME(lgp),"FAKE ftd_rmd_apply",lgp->isockp, &header);
            //rc = FTD_SOCK_RECV_HEADER(LG_PROCNAME(lgp) ,"FAKE ftd_rmd_apply",lgp->isockp, &ack);      
            UNSET_LG_CPPEND(lgp->flags);
	        SET_JRN_MODE(lgp->jrnp->flags, FTDMIRONLY);
            error_tracef(  TRACEINF4, "ftd_lg_cppend return 0");
            return 0;
            }
...
void ftd_lg_TellErrorSecondary(ftd_lg_t *lgp,ftd_header_t *ack,char *txt)
{
	// tell secondary
	ftd_sock_send_err(lgp->dsockp, ftd_get_last_error(ERRFAC));
	
	sprintf(txt, "Group: %03d Primary", lgp->lgnum);
	reporterr(ERRFAC, M_CPONERR, ERRWARN, txt);
...
//				if (ftd_lg_sync_prim_devs(lgp) < 0) {
				...
				if (ftd_lg_unlock_devs(lgp) < 0) {
                    ...
                    ftd_lg_TellErrorSecondary(lgp,&ack,groupstr);				
					cperr = TRUE;
					} 
				else 
					{
...
//					ftd_lg_unlock_devs(lgp);
                    ...
                    if ( GET_LG_JLESS(lgp->flags))
                        {
                        // we should tell to Driver to not add anymore users IOS in the BAB
                        // once the checkpoint will be off, a smartrefresh will be done automatically                       
				
                        if (ftd_lg_set_driver_run_state(lgp->lgnum, FTD_MODE_CHECKPOINT_JLESS) < 0) 
                            {
                            ftd_lg_TellErrorSecondary(lgp,&ack,groupstr);				
                            UNSET_LG_CPSTART(lgp->flags);
                            return (-1);
...
ftd_lg_cpon(ftd_lg_t *lgp) 
{
	...
	case ROLEPRIMARY:
        ...
        ftd_mngt_performance_set_group_cp((short)lgp->lgnum, 1);
...
ftd_lg_cpoff(ftd_lg_t *lgp) 
{
	...
	case ROLEPRIMARY:
        ...
        if (GET_LG_JLESS(lgp->flags)) 
            {           
            //PMD should go in tracking mode then a SmartRefresh will be kicked.
            
            if ((rc = ftd_lg_set_pstore_run_state(lgp->lgnum, lgp->cfgp, FTD_STRACKING)) < 0) 
				{
                return rc;
				}          
		...
		break;
	case ROLESECONDARY:
		sprintf(command, "%s\\%s%03d.bat",PATH_CONFIG, PRE_CP_OFF, lgp->lgnum);
		// if it doesn't have any executable commands,
		// don't try to run it
		if (!existCommands(command)	|| ftd_proc_exec(command, 1) == 0)
		{
			UNSET_LG_CPON(lgp->flags);
			UNSET_LG_CPSTART(lgp->flags);
			UNSET_LG_CPPEND(lgp->flags);
...
ftd_lg_cpstop(ftd_lg_t *lgp) 
{
	...
	case ROLEPRIMARY:
		...
		if (!GET_LG_CPON(lgp->flags) && !GET_LG_CPSTART(lgp->flags)) 
            ...
            {
			if (ftd_ioctl_send_lg_message(lgp->devfd,lgp->lgnum, MSG_CPOFF) < 0) {
				return -1;
			}

            if (GET_LG_JLESS(lgp->flags)) 
                {SET_LG_CPOFF_JLESS_ACKPEND(lgp->flags);}
		}
