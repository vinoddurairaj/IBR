...
static char JournalLog[JRN_MAXFILES][MAXPATHLEN];
...
char *DisplayJrnMode(int flags)
{
    switch(GET_JRN_MODE(flags)) 
        {
        case FTDMIRONLY:
            return ("MIRONLY\0");
...
ftd_journal_create(char *jrnpath, char *fileprefix)
{
    ...
    error_tracef(  TRACEINF4, ">>ftd_journal_create()");
...
ftd_journal_get_cur_state(ftd_journal_t *jrnp, int prune_inco)
{
	...
	error_tracef( TRACEINF4, ">>ftd_journal_get_cur_state() jrnmode= %s, jrnstate= %s", DisplayJrnMode(jrnp->flags), (GET_JRN_STATE(jrnp->flags) == FTDJRNCO ? "COH\0":"INCOH"));
			...
			if (ftd_journal_file_is_cp(jrnfp)) 
				...
				if (i == 0) 
                    ...
                    SET_JRN_CP_ON(jrnp->flags);
                  	error_tracef( TRACEINF4, ">>ftd_journal_get_cur_state() CP_ON");
				    } 
			    else 
			        {
    ...
    error_tracef( TRACEINF, "ftd_journal_get_cur_state() jrnmode= %s, jrnstate= %s", DisplayJrnMode(jrnp->flags), (GET_JRN_STATE(jrnp->flags) == FTDJRNCO ? "COH\0":"INCOH"));
...
ftd_journal_get_all_files(ftd_journal_t *jrnp)
{
    ...
    char                paths[JRN_MAXFILES][MAXPATHLEN];
    ...
    if ((jrncnt = ftd_journal_get_file_names(paths,
    ...
    ForEachLLElement(jrnp->jrnlist, jrnfpp) {
        ...
        found = 0;
        for (i = 0; i < jrncnt; i++) {
            if (strcmp((*jrnfpp)->name, paths[i]) == 0) {
                found = 1;
                break;
            }
        }
    ...
    for (i = 0; i < jrncnt; i++) {
        if (ftd_journal_file_name_is_cp(paths[i])) {
            ftd_journal_parse_name(jrnp->flags, paths[i],
                &lnum, &lstate, &lmode);
            jrnp->cpnum = lnum;
        }
        found = 0;
...
ftd_journal_set_state(ftd_journal_t *jrnp, int tstate)
{
    ...
    if (GET_JRN_JLESS(jrnp->flags))
		{
		SET_JRN_STATE(jrnp->flags, tstate);	   //??? to be verified
		return 0;
		}
...
#endif
        error_tracef( TRACEINF4, "ftd_journal_set_state(): rename old to new = %s, %s", path, tpath );
        rename(path, tpath);
        //unlink(jrnfp->name);
    }
    ...
    error_tracef( TRACEINF, "ftd_journal_set_state() jrnmode= %s, jrnstate= %s", DisplayJrnMode(jrnp->flags), (GET_JRN_STATE(jrnp->flags) == FTDJRNCO ? "COH\0":"INCOH"));
    return 0;
}
...
ftd_journal_create_next(ftd_journal_t *jrnp, int tstate)
{
    ...
    error_tracef(  TRACEINF4, ">>ftd_journal_create_next");
...
ftd_journal_create_next_cp(ftd_journal_t *jrnp, ftd_journal_file_t *jrnfp)
{
    ...
    error_tracef(  TRACEINF4, ">>ftd_journal_create_next_cp ()");
...
ftd_journal_file_close(ftd_journal_file_t *jrnfp)
{
    ...
    if (jrnfp && (jrnfp->fd != INVALID_HANDLE_VALUE) ) {
        ...
        FTD_CLOSE_FUNC(__FILE__,__LINE__,jrnfp->fd);
...
ftd_uint64_t ftd_journal_get_journal_files_total_size(ftd_journal_t *jrnp)
{
    ftd_journal_file_t  **jrnfpp;
    ftd_uint64_t         totalsz = 0;
