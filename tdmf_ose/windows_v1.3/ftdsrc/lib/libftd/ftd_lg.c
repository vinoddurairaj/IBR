...
//defined in ftd_mngt_lg.cpp
int  ftd_mngt_init_lg_monit(ftd_lg_t* lgp);
void ftd_mngt_do_lg_monit(ftd_lg_t *lgp);                       /* called in ftd_lg_housekeeping() */

int bDbgLogON =0;

static int ftd_lg_parse_state(ftd_lg_t *lgp, char *buf, int buflen);
static int ftd_lg_parse_state_value(char *key, char *value,  ftd_lg_t *lgp, int verify, int set);
...
// SAUMYA_FIX_INITIALIZATION_MODULE
    ...
    if ((lgp->monitp = ftd_mngt_create_lg_monit()) == NULL) {
        goto errexit;
    }
...
ftd_lg_cleanup(ftd_lg_t *lgp)
{
    ...
    {
        FTD_CLOSE_FUNC(__FILE__,__LINE__,lgp->ctlfd);
        lgp->ctlfd = INVALID_HANDLE_VALUE;
    }
    ...
    if (lgp->monitp)
        ftd_mngt_delete_lg_monit(lgp->monitp);
...
{
    //
    // WR 20361 ADD DEBUG HERE
    // 
    // This is one of two places where we add to the dev list
...
ftd_lg_init(ftd_lg_t *lgp, int lgnum, int role, int startflag)
{
	...
	ftd_sz_context	*sz_next = NULL, *sz_prev = NULL; 
    int             dev_bsize;
    int             checklock = 0;
    ...
    if(startflag == 2)  // Special case for STOP cmd  , rddeb 030512
    {
        checklock = 1;
        startflag = 1;
    }

    if ((rc = ftd_config_read(lgp->cfgp, TRUE, lgp->lgnum, role, startflag)) < 0) 
    {
        ftd_config_lg_delete(lgp->cfgp);
        error_tracef(TRACEERR, "ftd_lg_init(%d) role %d start %d", lgp->lgnum, role, startflag);
        return rc;
    }
    ...
    if (lgp->cfgp->role == ROLEPRIMARY) 
    {
        ...
        {
            reporterr(ERRFAC, M_CTLOPEN, ERRCRIT, FTD_CTLDEV, ftd_strerror());
            goto errret;
        }
    ...
    {       
        ...
        if (role == ROLEPRIMARY) 
        {
            if (startflag) 
            {
                ...
                for (i = 0; i < ndevs; i++) 
                {
                    if (!strcmp(devcfgp->devname, strchr(devstat[i].devname, ':') - 1 ))
                    {
                        // found it
                        found = TRUE;
                        break;
                ...
                // Verify Drive Lock on Primary disk
                if ( checklock )
                {
                    ...
                    if (fd == INVALID_HANDLE_VALUE) 
                    {
                        ...
                        //
                        // Make sure errors are displayed for the proper group!
                        // change the procname of the errfac temporarly
                        //
                        char TempProcName[255];
                        char PMDName[255];
                        if (ERRFAC)
                        {
                            strcpy(TempProcName,ERRFAC->procname);
                            sprintf(PMDName," PMD_%03ld",lgp->lgnum);
                            strcat(ERRFAC->procname,PMDName);
                        }
                        reporterr(ERRFAC, M_DEVLOCK, ERRCRIT, devcfgp->pdevname, ftd_strerror());
                        if (ERRFAC)
                        {
                            strcpy(ERRFAC->procname,TempProcName);
                        }
            ...
            ftd_dev_init(devp, devcfgp->devid, devcfgp->pdevname, devcfgp->devname, size, num);
        } 
        else // ROLE == SECONDARY
        {
            if (!startflag) 
			...
			// Allocate the sector zero structure for the device. 

			if(sz_prev == NULL)
			{							// First Device in the chain
				lgp->SectorZero = (ftd_sz_context *)LocalAlloc(LPTR, sizeof(ftd_sz_context)); 
				lgp->SectorZero->next = NULL;
				lgp->SectorZero->prev = NULL; 

				sz_prev = lgp->SectorZero; 

				CopyMemory(&(lgp->SectorZero->devcfgp), devcfgp, sizeof(ftd_dev_cfg_t)); 

			} 
			else
			{ 
				sz_next = (ftd_sz_context *)LocalAlloc(LPTR, sizeof(ftd_sz_context));

				if(sz_next == NULL)
				{
					printf("Error allocating sector zero buffer!!\n"); 
				} 
				else
				{ 
					sz_next->next = NULL;
					sz_next->prev = sz_prev; 

					sz_prev->next = sz_next;	

					CopyMemory(&(sz_next->devp),	devp, sizeof(ftd_dev_t)); 
					CopyMemory(&(sz_next->devcfgp), devcfgp, sizeof(ftd_dev_cfg_t)); 

					// Set up the pointers for next time. 
					sz_prev = sz_next; 
					sz_next = NULL;
				} 
			} 
            ...
            // num not used by UNIX
            ftd_dev_init(devp, devcfgp->devid, devcfgp->sdevname, devcfgp->sdevname, size, num);
...
ftd_lg_close(ftd_lg_t *lgp)
{
    ...
    {
        FTD_CLOSE_FUNC(__FILE__,__LINE__,lgp->devfd);
        lgp->devfd = INVALID_HANDLE_VALUE;
    }
    ...
    {
        FTD_CLOSE_FUNC(__FILE__,__LINE__,lgp->babfd);
        lgp->babfd = INVALID_HANDLE_VALUE ;
    }

    if (lgp->perffd != INVALID_HANDLE_VALUE)
    {
        FTD_CLOSE_FUNC(__FILE__,__LINE__,lgp->perffd);
        lgp->perffd = NULL;
    }
    return 0;
}
...
ftd_lg_open(ftd_lg_t *lgp)
{
    ...
    {
        reporterr(ERRFAC, M_LGDOPEN, ERRCRIT, lgp->devname, ftd_strerror());
        return -1;
    }
...
// we don't need this event anymore
    ...
    if ((lgp->babfd = OpenEvent(SYNCHRONIZE | EVENT_MODIFY_STATE, FALSE, event)) == NULL)
    {
        lgp->babfd = INVALID_HANDLE_VALUE;
        reporterr(ERRFAC, M_OPENEVENT, ERRCRIT, event, ftd_strerror());
        return -1;
    }
#endif
    ...
    {
        lgp->perffd = INVALID_HANDLE_VALUE;
        reporterr(ERRFAC, M_CRTEEVENT, ERRCRIT, event, ftd_strerror());
        return -1;
    }
...
/*
 * Extract Pri and Sec IP Adresses from CONFIG file using Server role 
 */

int
ftd_lg_get_hostip(int piLgNum, int iRole , char *szPriHostIp, char *szSecHostIp)
{
    ftd_lg_t    *lgp;
    int         ret;
    ...
    if (iRole == ROLEPRIMARY)
        sprintf(lgp->cfgp->cfgpath, "%s\\%s%03d.cfg", PATH_CONFIG, PRIMARY_CFG_PREFIX, lgp->lgnum); 
    else
        sprintf(lgp->cfgp->cfgpath, "%s\\%s%03d.cfg", PATH_CONFIG, SECONDARY_CFG_PREFIX, lgp->lgnum); 

    if ((ret = ftd_config_read_2( lgp->cfgp, TRUE, lgp->lgnum, iRole )) < 0) 
    {
        ftd_config_lg_delete(lgp->cfgp);
        ftd_lg_delete(lgp);
        return 1;
    ...
    // Get PRI hostname for this group
    strcpy(szPriHostIp, lgp->cfgp->phostname);

    // Get SEC hostname for this group
    strcpy(szSecHostIp, lgp->cfgp->shostname);  
    ret = 0;
    ...
    ftd_stat_t *lgstat, int regen, int autostart, int grpstarted)
{
    ...
    //
    // This is one of the places that has a problem with WR20361
    //
    ...
    {       
        ...
        if (!getDeviceNameSymbolicLink(devcfgp->pdevname, devcfgp->vdevname, MAXPATHLEN)) 
        {
        ...
        if (devp->devsize == -1) 
        {
            ...
            //
            // Retry 10 times with 50ms delay
            //

            do 
            {
                if ( strlen(devcfgp->pdevname) == 2 )  // Drive Letter
                {
                if (IsXporBetter())
                {
                    char szDosDeviceName[5];
                    sprintf(szDosDeviceName, "%c:\\",devcfgp->pdevname[0]);
                    bDismounted = DeleteAMountPointVolume(szDosDeviceName);
                }
                else
                {
                ...
                {
                    bDismounted = DeleteAMountPointVolume(devcfgp->pdevname);
                }

                if (!bDismounted)
                {
                    ftd_lockmsgbox(devcfgp->pdevname);
                    Sleep(50);
                }
            } 
            while ( !bDismounted && --dwRetry);

        if (!bDismounted) 
        {
            //
            // Make sure dismount errors are displayed for the proper group!
            // change the procname of the errfac temporarly
            //
            char TempProcName[255];
            char PMDName[255];
            ...
            reporterr(ERRFAC, M_DISMOUNT, ERRCRIT, devcfgp->devname, lgp->devname);
            if (ERRFAC)
            {
                strcpy(ERRFAC->procname,TempProcName);
            }
    ...
    {
        if ( strlen(partp->pdevname) == 2 )  // Drive Letter
        {
            if (IsXporBetter())
            {
                char szDosDeviceName[5];
                sprintf(szDosDeviceName, "%c:\\",partp->pdevname[0]);
                CreateAMountPointVolume(szDosDeviceName, partp->symname);
            }
            else
            {
            ...
            CreateAMountPointVolume(partp->pdevname, partp->symname);
    }
 ...
 * add socket connections to this group
 */
int
sftk_lg_add_connections(ftd_lg_t *lgp)
{
...
/*
 * sftk_lg_remove_connections --
 * remove socket connections to this group
 */
int
sftk_lg_remove_connections(ftd_lg_t *lgp)
{
...
/*
 * sftk_lg_enable_connections --
 * enable socket connections to this group
 */
int
sftk_lg_enable_connections(ftd_lg_t *lgp)
{
...
/*
 * sftk_lg_disable_connections --
 * disable socket connections to this group
 */
int
sftk_lg_disable_connections(ftd_lg_t *lgp)
{
 ...
 */
int
sftk_lg_launch_pmd(ftd_lg_t *lgp)
{
...
/*
 * sftk_lg_stop_all_connections --
 * stop all socket connections to this group
 */
int
sftk_lg_stop_all_connections(ftd_lg_t *lgp)
{
...
/*
 * sftk_qyery_connections --
 * get info for all the connections to this group
 */
int
sftk_qyery_connections(ftd_lg_t *lgp)
{
...
// SAUMYA_FIX_INITIALIZATION_MODULE
    ...
    {       
        ...
        if (group_info.state_stamp != LG_INACTIVE) 
        {
            // LG_INACTIVE isn't needed for PASSTHRU mode 
	...
	// SAUMYA_FIX_INITIALIZATION_MODULE
	// the driver should not expect any PStore ??
	// info.vdevname = psname;
 ...
 */
static int
ftd_lg_rem_devs(ftd_lg_t *lgp, ftd_stat_t *lgstat, int autostart, int silent, int all)
{
    int             *savedbbufp, 
                    i, 
                    found;
    ...
    if (ftd_ioctl_get_lrdbs(lgp->ctlfd, lgp->devfd, lgp->lgnum, lgp->db, dip) != 0)
        goto errret;
            
    // get the HRDB's for the group 
    lgp->db->dbbuf = (int*)hrdb;
    if (ftd_ioctl_get_hrdbs(lgp->ctlfd, lgp->devfd, lgp->lgnum, lgp->db, dip) != 0)
        goto errret;
    ...
    {       
        ...
        ForEachLLElement(lgp->cfgp->devlist, devcfgpp) 
        {
            ...
            if (!strcmp(devcfgp->devname, strchr(devstat.devname, ':') - 1 ))
            {
                    // found it
                    found = TRUE;
                    break;
        ...
        if (!all && found) // skip it
            continue;

        //
        // WR 20361 after a while, we end up with two devices
        // here even if there should be only 1! This causes the problem!
        //
        ...
        if ( strlen(devcfgp->devname) == 2 )  // Drive Letter
        {
            if (IsXporBetter())
            {
                char szDosDeviceName[5];
                sprintf(szDosDeviceName, "%c:\\",devcfgp->devname[0]);
                bGotSymbolicLink = getDeviceNameSymbolicLink(szDosDeviceName, part.symname, MAXPATHLEN);
            }
            else
            {
                bGotSymbolicLink = getDeviceNameSymbolicLink(devcfgp->devname, part.symname, MAXPATHLEN);
            }
        }
        else
        {
        ...
        if (!bGotSymbolicLink)
            {
                ...
                {
                    strcpy(part.symname, devcfgp->vdevname); // Recover from link lost
                    strcpy(part.devname, devcfgp->devname);  // in case of bDismounted error                
                }
            }               
        ...
        //
            ...
            do 
            {
                ...
                {
                    bDismounted = DeleteAMountPointVolume(devcfgp->devname);
                }
        ...
        if (!bDismounted) 
        {
            ...
            part.dev_num = dip[i].bdev;              // get the device number 
            AddToTailLL(partlist, &part);            // Register it even in error mode.
            goto errret;
        }
    ...
    ForEachLLElement(partlist, partp) 
    {
        if ( strlen(partp->devname) == 2 )  // Drive Letter
        {
            if (IsXporBetter())
            {
                char szDosDeviceName[5];
                sprintf(szDosDeviceName, "%c:\\",partp->devname[0]);
                CreateAMountPointVolume(szDosDeviceName, partp->symname);
            }
            else
            {
            ...
            CreateAMountPointVolume(partp->devname, partp->symname);
    }
...
ftd_lg_update_bitmaps(ftd_lg_t *lgp, ftd_stat_t *lgstat, int silent)
{
    if (lgstat->state == FTD_MODE_REFRESH  || 
    	 ...
    	 lgstat->state == FTD_MODE_FULLREFRESH)
    {
...
ftd_lg_rem_group(ftd_lg_t *lgp, int state, int autostart, int silent)
{
    ...
    if (autostart) 
    {
        ...
        {
            if (!silent) 
                reporterr(ERRFAC, M_PSWTGATTR, ERRCRIT, lgp->devname, lgp->cfgp->pstore);

            goto errret;
        }
    }
    
    if (ftd_ioctl_get_groups_info(lgp->ctlfd, lgp->lgnum, &lginfo, silent) < 0)
        goto errret;
    ...
    // write out the LG_INACTIVE stamp for the group in PSTORE
    ps_set_group_state_stamp(lgp->cfgp->pstore, lgp->devname, LG_INACTIVE);
...
ftd_lg_rem(ftd_lg_t *lgp, int autostart, int silent)
{
    ...
    //
    // If we do the following code, the mount manager is no longer
    // able to get the mountpoint for a volume.
    //
    ...
    if (ftd_ioctl_get_group_stats(lgp->ctlfd, lgp->lgnum, &lgstat, silent) < 0)
        goto errret;
...
// SAUMYA_FIX_INITIALIZATION_MODULE
    ...
    {   // no, so create it         
        ...
        if (rc == 0)
        {
            reporterr(ERRFAC, M_PSCREAT_SUCCESS, ERRINFO, psname, lgp->lgnum, max_dev);
        }
        else 
        {
    ...
    // add group
    if (ftd_lg_add_group(lgp, 0, &state, &regen, &grpstarted, autostart) < 0)
        goto errret;
    ...
    // get the stats for the group
    if (ftd_ioctl_get_group_stats(lgp->ctlfd, lgp->lgnum, &lgstat, 0) < 0)
        goto errret;
...
// SAUMYA_FIX_INITIALIZATION_MODULE
    ...
    if (grpstarted) 
    {
        ...
        if (ftd_lg_rem_devs(lgp, &lgstat, autostart, 0, 0) < 0)             
            goto errret; // if some of the deleted devs get removed then that's ok
    }
#endif  // SAUMYA_FIX_INITIALIZATION_MODULE
    ...
    if (ftd_lg_add_devs(lgp, &lgstat, regen, autostart, grpstarted) < 0) 
    {
        ...
        if (!grpstarted)  // no devs in group
        {
            //
            // Close lgp before invalidating everything!
            // Otherwise we have an event hanging, because
            // closing after attempts to do it with an lgp
 ...
 // SAUMYA_COMMENT
// puts the group into tracking state
// SAUMYA_FIX_INITIALIZATION_MODULE
#if 1
    ...
    // set the state stamp of the group to LG_STARTED in PSTORE
    ps_set_group_state_stamp(lgp->cfgp->pstore, lgp->devname, LG_STARTED);
#endif  // SAUMYA_FIX_INITIALIZATION_MODULE
     ...
     */
    sprintf(tunesetpath, "%s/settunables%d.bat",PATH_CONFIG, lgp->lgnum);
...
ftd_lg_sigusr1(ftd_lg_t *lgp) 
{
    ...
    if (lgp->cfgp->role == ROLEPRIMARY) 
    {
        ...
        // Indicate STARTED level for this active LG
        ps_set_group_state_stamp(lgp->cfgp->pstore, lgp->devname, LG_STARTED);
                break;
            case FTD_SBACKFRESH:
...
{
    if (lgp->cfgp->role == ROLEPRIMARY) 
        ftd_stat_init_driver(lgp); // reset driver stats

    return FTD_CINVALID; // force an exit
}
...
ftd_lg_sigpipe(ftd_lg_t *lgp) 
{
    ...
    return FTD_LG_NET_BROKEN; // force an exit
}
...
ftd_lg_set_driver_state(ftd_lg_t *lgp)
{
    ...
    {
        if (ftd_ioctl_set_sync_depth(lgp->ctlfd, lgp->lgnum, lgp->tunables->syncmodedepth)!=0) 
            return -1;
    } 
    else 
    {
    ...
    if (ftd_ioctl_set_sync_timeout(lgp->ctlfd, lgp->lgnum, lgp->tunables->syncmodetimeout)!=0)
        return -1;

    memset(buf,0,sizeof(buf));
    memcpy(buf, lgp->tunables, sizeof(ftd_tune_t));
    if ((rc = ftd_ioctl_set_lg_state_buffer(lgp->ctlfd, lgp->lgnum, buf)) < 0)
        return -1;
...
ftd_lg_get_driver_state(ftd_lg_t *lgp, int silent)
{
    ...
    if (ftd_ioctl_get_lg_state_buffer(lgp->ctlfd, lgp->lgnum, buf, silent) < 0)
        return -1;

    memcpy(lgp->tunables, buf, sizeof(ftd_tune_t));

    if (lgp->compp && lgp->compp->magicvalue == COMPMAGIC)      
        lgp->compp->algorithm = lgp->tunables->compression; // set socket object compression algorithm 
...
ftd_lg_parse_state(ftd_lg_t *lgp, char *lbuf, int buflen)
{
    ...
    while (getbufline(&bufp, &keyp, &valuep, '\n')) {
        if (ftd_lg_parse_state_value(keyp, valuep, lgp, 0, 1)!=0) {
            rc = -1;
        }
    }
 ...
 */
static int
ftd_lg_parse_state_value(char *key, char *value,  ftd_lg_t *lgp, int verify, int set)
{  
    int intval,state;
    ...
    if (strcmp("CHUNKSIZE:", key) == 0) 
	{
        ...
        if (set)
		{
            lgp->tunables->chunksize = intval * 1024;
        }
#if !defined(_WINDOWS)
    } else if (strcmp("STATINTERVAL:", key) == 0) 
	{
        ...
        if (set) {
            lgp->tunables->statinterval = intval;
        }
    ...
    } else if (strcmp("SYNCMODEDEPTH:", key) == 0) 
	{
        ...
        if (set) 
		{
            lgp->tunables->syncmodedepth = intval;
        }
    } else if (0 == strcmp("SYNCMODETIMEOUT:", key)) 
	{
        ...
        if (set) 
		{
            lgp->tunables->syncmodetimeout = intval;
        }
    } else if (strcmp("SYNCMODE:", key) == 0)
	{
        if (strcmp (value, "on") == 0 || strcmp (value, "ON")==0 || strcmp (value, "1") == 0) 
		{
            if (set)
			{
                lgp->tunables->syncmode = 1;
            }
        } else if (strcmp(value, "off") == 0 || strcmp(value, "OFF") == 0 || strcmp (value, "0") == 0) 
		{
            if (set) 
			{
                lgp->tunables->syncmode = 0;
            }
        } else 
		{
            if (verify) 
    ...
    } else if (strcmp("TRACETHROTTLE:", key) == 0)
	{
        if (strcmp (value, "on") == 0 || strcmp (value, "ON")==0 || strcmp (value, "1") == 0) 
		{
            if (set) 
			{
                lgp->tunables->tracethrottle = 1;
            }
        } else if (strcmp(value, "off") == 0 || strcmp(value, "OFF") == 0 || strcmp (value, "0") == 0) 
		{
            if (set) 
			{
                lgp->tunables->tracethrottle = 0;
            }
		}
#endif
    ...
    } else if (strcmp("JOURNAL:", key) == 0) // support of JOURNAL_LESS or JLESS
        {
        if (strcmp (value, "on") == 0
            || strcmp (value, "ON")== 0
            || strcmp (value, "1") == 0)
            ...
            if (set) 
            {
                state = ftd_lg_get_driver_run_state(lgp->lgnum);

                if (state != FTD_MODE_TRACKING && state != FTD_MODE_PASSTHRU)
                    {
                    if (lgp->tunables->use_journal == FALSE)
                        {reporterr(ERRFAC, M_JOURNAL_ON, ERRINFO);}
                    }
                                      
                lgp->tunables->use_journal = TRUE;
            }
       } else if (strcmp(value, "off") == 0 || strcmp(value, "OFF") == 0|| strcmp (value, "0") == 0)
	   {
            if (set) 
                {
                // get the group state to decide to display ERRINFO message
                state = ftd_lg_get_driver_run_state(lgp->lgnum);

                if (state != FTD_MODE_TRACKING && state != FTD_MODE_PASSTHRU)
                    {
                    if (lgp->tunables->use_journal == TRUE)
                        {reporterr(ERRFAC, M_JOURNAL_OFF, ERRINFO);}
                    }

                lgp->tunables->use_journal = FALSE;
                }
            } 
        else 
            {
            if (verify) 
                {
                reporterr(ERRFAC, M_JOURNALVAL, ERRWARN);
                return -1;
                }
            }
        ...
        if (set) {
            lgp->tunables->netmaxkbps = intval;
        }
    ...
    } else if (strcmp("CHUNKDELAY:", key) == 0) 
	{
        ...
        if (set) 
		{
            lgp->tunables->chunkdelay = intval;
        }
    } else if (strcmp("REFRESHTIMEOUT:", key) == 0) 
	{
        ...
        if (set) {
            lgp->tunables->refrintrvl = intval;
        }
    }  else { 
        return -1;
    }
...
ftd_lg_set_state_value(ftd_lg_t *lgp, char *inkey, char *invalue) 
{
    ...
    /* verify new value */
	rc = ftd_lg_parse_state_value(inkey, invalue, lgp, 1, 0);
    if (rc < 0) {
        return rc;
    }
...
ftd_lg_open_devs(ftd_lg_t *lgp, int mode, int permis, int tries)
{
    ...
    {       
        ...
        else if (lgp->cfgp->role == ROLESECONDARY) // WR 17511
        {  
            if (!getDeviceNameSymbolicLink((*devcfgpp)->sdevname, szVolumeName, MAX_PATH) )
            {
                return FTD_LG_SEC_DRIVE_ERR;  // Report Secondary Drive error to Sec FSM
            }
        }
            ...
            {               
                {
                    ...
                    {
                        if ((ret = ftd_sock_send_err(lgp->dsockp, ftd_get_last_error(ERRFAC))) < 0)
                        {
                            return ret;
                        }
                    }
...
ftd_lg_housekeeping(ftd_lg_t *lgp, int tune)
{
    ...
    ftd_mngt_do_lg_monit(lgp);
#endif

// SAUMYA_FIX 
// we don't need this anymore. 
// This house keeping will be done inside the driver
#if 0
...
ftd_lg_get_bab_chunk(ftd_lg_t *lgp, int force)
{
    ...
    case FTD_MODE_TRACKING:
        ...
        if (GET_LG_STATE(lgp->flags) == FTD_STRACKING) 
        ...
        else if ((GET_LG_STATE(lgp->flags) == FTD_SNORMAL) && (GET_LG_CPSTART(lgp->flags) || GET_LG_CPON(lgp->flags)))
            break; /* that's ok */
           ...
           // WR17010, always try a REFRESH after a BABOVERFLOW
        }
...
ftd_lg_start_journaling(ftd_lg_t *lgp, int jrnstate) 
{
    ...
    error_tracef( TRACEINF, "ftd_lg_start_journaling jrnmode= %s, jrnstate= %s", DisplayJrnMode(lgp->jrnp->flags), (GET_JRN_STATE(lgp->jrnp->flags) == FTDJRNCO ? "COH\0":"INCOH"));
    return 0;
}
...
ftd_lg_stop_journaling(ftd_lg_t *lgp, int jrnstate, int jrnmode) 
{
    ...
    error_tracef( TRACEINF, "ftd_lg_stop_journaling++ jrnmode= %s, jrnstate= %s", DisplayJrnMode(lgp->jrnp->flags), (GET_JRN_STATE(lgp->jrnp->flags) == FTDJRNCO ? "COH\0":"INCOH\0"));
    ...
    case FTDJRNMIR: 
        ...
        //
            ...
            rc = ftd_lg_open_devs(lgp, O_RDONLY, 0, 5);
            if ( rc < 0) 
            {
...
ftd_lg_traverse_chunk(ftd_lg_t *lgp)
{
    ...
    if (lgp->cfgp->role == ROLESECONDARY) {
        ...
        if ((lgp->jrnp->cur && (lgp->jrnp->cur->fd != INVALID_HANDLE_VALUE))
            && GET_JRN_MODE(lgp->jrnp->flags) == FTDJRNMIR)
        {
            ...
            if (ftd_lg_journal_apply_done(lgp))
            {
                ...
                offset = ftd_llseek(lgp->jrnp->cur->fd, 0, SEEK_END);
                ...
                lgp->jrnp->cur->size = offset-1;
                lock_len = lgp->datalen;
            
                rc = ftd_journal_file_lock(lgp->jrnp->cur, offset, lock_len);
                if (rc == -1) {
                    return rc;
                } else {
        ...
        if (wlhead->offset == -1) 
            ...
            if (!memcmp(dataptr, MSG_INCO, strlen(MSG_INCO))) 
                ...
                if (lgp->cfgp->role == ROLESECONDARY) 
                    ...
                    UNSET_LG_FULLREFRESH_PHASE2(lgp->flags);    // we want a standard smart refresh...so to be sure
                    goto retstate;
                }
                } 

            else if (!memcmp(dataptr, MSG_AVOID_JOURNALS, strlen(MSG_AVOID_JOURNALS))) 
                {
                error_tracef( TRACEINF, "ftd_lg_traverse_chunk():[%s] got MSG_AVOID_JOURNALS", LG_PROCNAME(lgp) );
                dataptr += strlen(MSG_AVOID_JOURNALS)+1;
                if (lgp->cfgp->role == ROLESECONDARY) 
                    {
                    rc = FTD_CREFRESH;  // transition to refresh state PHASE 2
                ...
                if (lgp->cfgp->role == ROLESECONDARY) 
                    ...
                    UNSET_LG_FULLREFRESH_PHASE2(lgp->flags);    // Phase2  is complete if active
                    goto retstate;
                }
                } 
            ...
            if (lgp->cfgp->role == ROLEPRIMARY) 
                ...
                { // we are SECONDARY
                if ((devp =  ftd_lg_minor_to_dev(lgp, wlhead->diskdev)) == NULL)
                {
                    // report err
                }   
                
                switch(GET_JRN_MODE(lgp->jrnp->flags)) 
                {
                case FTDMIRONLY:
                    ...
                    if (ftd_dev_write(devp, dataptr, offset, entrylen,devcfgp->sdevname) < 0)
                    {
                        return FTD_LG_SEC_DRIVE_ERR;   //Report Secondary Drive error to Sec FSM
                        // return -1; //WR 17511
                    }
...
retstate:
    ...
    if (lgp->cfgp->role == ROLESECONDARY
        && lgp->jrnp->cur
        && GET_JRN_LOCK(lgp->jrnp->cur->flags))
    {
        ...
        if (lgp->jrnp->cur && (lgp->jrnp->cur->fd != INVALID_HANDLE_VALUE) ) {
            ftd_fsync(lgp->jrnp->cur->fd);
        
#if !defined(_WINDOWS)
            unlock_len = ~(lgp->jrnp->cur->locked_length);

            rc = ftd_journal_file_unlock( lgp->jrnp->cur,
                lgp->jrnp->cur->locked_offset + lgp->jrnp->cur->locked_length,
                unlock_len );
#else
            rc = ftd_journal_file_unlock(lgp->jrnp->cur, lgp->jrnp->cur->locked_offset, lgp->jrnp->cur->locked_length);
#endif
            if (rc == -1) {
                reporterr(ERRFAC, "JRNUNLOCK", ERRWARN,
                    lgp->jrnp->cur->name, ftd_strerror());
                return -1;
            }
...
{
    static  unsigned long   ulWaitTimeout   = 0;
    char                    cPriority[32];
    int                     rc, 
                            ret             = 0;
    long                    io_sleep;
    ...
    error_tracef( TRACEINF5, "ftd_lg_wait_all_events():%s", LG_PROCNAME(lgp) ); 
    ...
    {
        //
        // On first time in this function, read registry value
        //
        if ( cfg_get_software_key_value("WAE_Timeout", cPriority, CFG_IS_NOT_STRINGVAL) == CFG_OK )
        {
            ulWaitTimeout = atoi(cPriority);
        }
        //
        // Default to 1000 if we can't read the value!
        //
        else
        {
            ulWaitTimeout = 1000;
    ...
    {
        //
        // SVG sleep only for ulWaitTiemout ms instead of 1000ms!
        //
        rc = WaitForMultipleObjects(nEvent, (CONST HANDLE *)hEvents, FALSE, io_sleep * ulWaitTimeout);
        ...
        case WAIT_TIMEOUT:
            ...
            if ( bDbgLogON ) 
            { 
                error_tracef( TRACEINF4, "ftd_lg_wait_all_events():%s:WAIT_TIMEOUT", LG_PROCNAME(lgp) ); 
            }
            return 0;
    ...
    if ( bDbgLogON ) 
    { 
        error_tracef( TRACEINF4, "ftd_lg_wait_all_events():%s:End Of Function", LG_PROCNAME(lgp) ); 
    }
...
ftd_lg_dispatch_io(ftd_lg_t *lgp)
{
    ...
    if ( bDbgLogON ) 
    { 
        error_tracef( TRACEINF4, "ftd_lg_dispatch_io():%s", LG_PROCNAME(lgp) ); 
    }

    // SECONDARY Server
    if ( lgp->cfgp->role == ROLESECONDARY && GET_LG_STATE(lgp->flags) != FTD_SAPPLY  )
    {
        ...
        else   // verify Device presence if no incoming chunks sent by PMD, only in NORMAL mode
        {
            if (GET_LG_STATE(lgp->flags) == FTD_SNORMAL ) //WR17512
            {   
                (void)time(&nowts);
                if ( ( nowts - lgp->ts ) > 5 ) // 5 sec Timeout for Device check
                {
                    lgp->ts = nowts;
                    ForEachLLElement(lgp->devlist, devpp) 
                    {   // Retreive Device Cfg Info
                        if ((devcfgp = ftd_lg_devid_to_devcfg(lgp, (*devpp)->devid)) == NULL) 
                        {
                            reporterr(ERRFAC, M_PROTDEV, ERRCRIT, (*devpp)->devid);
                            return FTD_LG_SEC_DRIVE_ERR;
                        }
                        else
                        {   // Is the Device Name still have a Symbolic Link, Remote/Removable drive case 
                            if (!getDeviceNameSymbolicLink(devcfgp->sdevname, szVolumeName, MAX_PATH) )
                            {
                                reporterr(ERRFAC, M_MIRSTAT, ERRCRIT, devcfgp->sdevname, ftd_strerror());
                                return FTD_LG_SEC_DRIVE_ERR;  // Report Secondary Drive error to Sec FSM
                            }
                        }
                    }
...
// SAUMYA_FIX
...
#else
    {
        if (lgp->cfgp->role == ROLEPRIMARY) 
        {
            ...
            if ( bDbgLogON )    
            { 
                error_tracef( TRACEINF4, "ftd_lg_dispatch_io():%s:After ftd_lg_wait_all_events(), rc == %d", LG_PROCNAME(lgp), rc ); 
            }
...
ftd_lg_get_driver_run_state(int lgnum)
{
    ...
    if (state == FTD_MODE_CHECKPOINT_JLESS)        //???
...
ftd_lg_get_pstore_run_state(int lgnum, ftd_lg_cfg_t *cfgp)
{
    ...
    if (!strcmp(statestr, "NORMAL")) {
        ...
        state = FTD_SREFRESHF;
    } else if (!strcmp(statestr, "TRACKING")) {
        state = FTD_STRACKING;
    } else if (!strcmp(statestr, "ACCUMULATE")) {        // was missing ???
        state = FTD_SPASSTHRU;
...
ftd_lg_apply(ftd_lg_t *lgp)
{
    ...
    {
        if ( rc == FTD_LG_SEC_DRIVE_ERR )  //WR 17511, Detect ftd_dev_write error.
            return FTD_LG_SEC_DRIVE_ERR;
        // error
        else
            return FTD_CINVALID;
...
BOOL IsXporBetter(void)
{
    ...
    //
    // XP is 5.1 .NET is 5.2
    //
    // Anything higher than 5.1 is good ;)
    //
