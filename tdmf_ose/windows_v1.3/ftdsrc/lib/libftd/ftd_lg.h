...
#define FTD_CREATE_GROUP_NAME(group_name, group_num) \
                      sprintf(group_name, "\\\\.\\DTC\\lg%d", group_num);
#define FTD_CREATE_GROUP_BAB_EVENT_NAME(group_name, group_num) \
                      sprintf(group_name, "DTClg%dbab", group_num);
#else // #ifdef NTFOUR
#define FTD_CREATE_GROUP_NAME(group_name, group_num) \
                      sprintf(group_name, "\\\\.\\Global\\DTC\\lg%d", group_num);
#define FTD_CREATE_GROUP_BAB_EVENT_NAME(group_name, group_num) \
                      sprintf(group_name, "Global\\DTClg%dbab", group_num);
#endif // #ifdef NTFOUR
// #undef NTFOUR

#define FTD_CREATE_GROUP_DIR(dir_name, group_num) \
	sprintf(dir_name, "DTC/lg%d", group_num);
...
#define	FTD_LG_SEC_DRIVE_ERR	-103  //WR17511,WR17512
...
typedef struct ftd_tune_s {
    ...
    int use_journal;            /* TUNABLE: default - on : use journal, flag for JLESS   */
} ftd_tune_t;
...
// context structure for sector zero save/restore during periods of inconsistency
typedef struct sz_context { 

    ftd_dev_t       devp;						// device state 
    ftd_dev_cfg_t   devcfgp;

	char			*SectorZeroBuffer;			// Buffer containing the data 
	ftd_uint64_t	SectorZeroLength;			// Length of the  buffer 

	void			*next;						// Linked list, since we can have multiple 
	void			*prev; 

} ftd_sz_context, *pftd_sz_context; 
...
extern int ftd_lg_get_hostip(int piLgNum, int iRole, char *szPriHostIp, char *szSecHostIp);
...
#if 0
extern int sftk_lg_add_connections(ftd_lg_t *lgp);
extern int sftk_lg_remove_connections(ftd_lg_t *lgp);
extern int sftk_lg_enable_connections(ftd_lg_t *lgp);
extern int sftk_lg_disable_connections(ftd_lg_t *lgp);
extern int sftk_lg_launch_pmd(ftd_lg_t *lgp);
extern int sftk_lg_stop_all_connections(ftd_lg_t *lgp);
extern int sftk_qyery_connections(ftd_lg_t *lgp);
#endif
// end SAUMYA_FIX_INITIALIZATION_MODULE
