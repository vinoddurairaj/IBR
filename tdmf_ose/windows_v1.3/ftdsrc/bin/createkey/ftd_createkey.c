...
time_t parse_date (char* str)
{
    ...
    {
        //
        // This case will be caught by parse_days if incorrect
        //
        return ((time_t)0);
    }
    ...
    for (i=0; i<len; i++) 
    {
        if (0 == isdigit(str[i])) 
        {
            fprintf (stdout, "Invalid format (all values must be numeric)\n");
            fflush (stdout);
    ...
    if (year < 2003 || year > 2037)
    {
        fprintf (stdout, "Invalid year must be (2003 =< YYYY < 2038)\n");
        fflush (stdout);
    ...
    if (mon > 12)
    {
        fprintf (stdout, "Invalid month (<=12)\n");
        fflush (stdout);
    ...
    if (expires < now)
    {
        fprintf (stdout, "Expiration date is before current date\n");
        fflush (stdout);
...
time_t parse_days (char* str)
{
    ...
    {
        //
        // Make sure we don't spew anything when it's just the date that is incorrect
        //
        if (len != 8)
        {
            fprintf (stdout, "Invalid format (1-3 digits for days, 8 digits for date)\n");
            fflush (stdout);
        }
...
prompt_input (int* permflag, time_t* expires, long* host_id, char* custname)
{
    ...
    fprintf (stdout, "\n--------------------------------- \n");
    fprintf (stdout, "Replication License key generator \n");
    fprintf (stdout, "--------------------------------- \n\n");
    ...
    if (!(*permflag)) 
    {
        ...
        while (!ok) 
        {
            *expires = defexpires;
            buffer[0] = '\0';
            fprintf (stdout, "Expiration date or number of day(s) license [YYYYMMDD or N (1-999), default 90 days]: ");
            fflush (stdout);
		       ...
		       long host_id, char* custname)
{
    ...
    fprintf(stdout, 
            "Replication License generated on:\n[%s]\n", 
            buf);
    fprintf(fd, 
            "Replication License generated on:\n[%s]\n", 
            buf);
    ...
    if (!permflag) 
    {
        ...
        if ((ptr = strchr(buf, '\n')) != (char *) NULL)
            *ptr = '\0';
        fprintf(stdout, 
	            "NON-PERMANENT LICENSE -- Expires on:\n[%s]\n", 
                buf);
        fprintf(fd, 
	            "NON-PERMANENT LICENSE -- Expires on:\n[%s]\n", 
                buf);
...
main(int argc, char **argv)
{
    ...
    //
    // Allow always to run, and do not validate hostid
    //
    oktorun = 1;
    permkeyokflag = 1;
    ...
    {
      fprintf ( stderr, 
                "You may not run %s from this system, expired.\n",
	            argv0);
      goto errexit;
    }
