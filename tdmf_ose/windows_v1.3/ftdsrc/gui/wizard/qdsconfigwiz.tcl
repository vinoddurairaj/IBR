...
proc QdsConfigWiz::frame11 {} {
    ...
    set WIZ(step) "Step 3:" 
    set WIZ(title) "Create Logical Groups"
    set p $WIZ(parent)

    set WIZ(text) "Datastar groups qds devices together into a collection of\
	    devices called a logical group.  All devices in a logical group\
	    ...
	    into one logical group to maintain this coherency.  You may want\
	    to define several logical groups to obtain a greater aggregate\
    ...
    radiobutton $p.select.onelg -variable ::QdsConfigWiz::cfg(grouping)\
	    -value 0 \
	    -text "Put all qDS devices in one logical group" \
	    -highlightthickness 0
    radiobutton $p.select.separatelg -variable ::QdsConfigWiz::cfg(grouping)\
	    -value 1 \
	    -text "Put each qDS device in its own logical group" \
	    -highlightthickness 0
...
proc QdsConfigWiz::frame12 {} {
    ...
    set WIZ(title) "Create Logical Groups: Group Mirrors"
    set p $WIZ(parent)
    ...
    label $p.notesF.notesL \
	    -text "Description/Notes for Logical Group $curlg:"
    ...
    label $p.selectF.selL \
	    -text "Select mirrors for Logical Group $curlg:"
