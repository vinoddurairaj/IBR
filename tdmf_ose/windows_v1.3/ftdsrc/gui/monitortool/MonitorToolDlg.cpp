...
void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_MTI, m_Icon);
	DDX_Text(pDX, IDC_STATIC_COPYRIGHT, m_cstrCopyright);
	DDX_Text(pDX, IDC_STATIC_VERSION, m_cstrAboutVersion);
	//}}AFX_DATA_MAP
}
...
BOOL CAboutDlg::OnInitDialog() 
{
	...
	CMonitorToolApp* app = (CMonitorToolApp*)AfxGetApp();

	CString strVersion	= app->m_ResourceManager.GetFullProductName();
	strVersion			+= _T(" Monitor Tool Version ");
	strVersion			+= app->m_ResourceManager.GetProductVersion();
    strVersion			+= _T("  Build ");
    strVersion			+= app->m_ResourceManager.GetProductBuild();
	m_cstrAboutVersion = strVersion;

	m_Icon.SetIcon(app->m_ResourceManager.GetApplicationIcon());
	
	CString cstrWndTitle;
	cstrWndTitle.Format("About %s Monitor Tool", app->m_ResourceManager.GetFullProductName());
...
//	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
	...
	CMonitorToolApp* app = (CMonitorToolApp*)AfxGetApp();
	m_hIcon = app->m_ResourceManager.GetApplicationIcon();
	if(m_hIcon == NULL)
	{
	...
	productName = _tcsdup(app->m_ResourceManager.GetProductName());
	CString cstrAppName = fullProductName = _tcsdup(app->m_ResourceManager.GetFullProductName());
	cstrAppName += " Monitor Tool";
	appName = _tcsdup(cstrAppName);
}

void CMonitorToolDlg::OnDestroy() 
{
	free(appName);
	free(productName);
	free(fullProductName);
...
BOOL CMonitorToolDlg::OnInitDialog()
{
	...
	CWnd* cWndAppStarted = NULL;

	cWndAppStarted = FindWindow(NULL, appName);
	
	if(cWndAppStarted==NULL)
	{
		#ifdef SFTK
			cWndAppStarted = FindWindow(NULL, "Softek TDMF Monitor Tool");
		#else
			cWndAppStarted = FindWindow(NULL, PRODUCTNAME " Monitor Tool");
		#endif
	}
...
void CMonitorToolDlg::readEventLog()
{
	...
	for (i = dwNumEvents; i > 0; i--) {
		...
		bool bMatchesSourceName = false;

		if(fullProductName != NULL && strlen(fullProductName)!=0 )
		{
			if(!strcmp(SourceName, fullProductName))
			{
				bMatchesSourceName = true;
			}
		}
		else if (!strcmp(SourceName, PRODUCTNAME)) 
		{

			bMatchesSourceName = true;
		}

		if (bMatchesSourceName) 
		{
...
void CMonitorToolDlg::OnDblclkListErrors() 
{
	...
	if(appName !=NULL)
	{
		::MessageBox(m_hWnd, strError, appName, MB_OK);
	}
	else
	{
		#ifdef SFTK
...
void CMonitorToolDlg::OnButtonConfigTool() 
{
	...
	char	strConfig[] = CMDPREFIX "ConfigTool";
	char	strConfigExe[] = CMDPREFIX "ConfigTool.exe";
...
void CMonitorToolDlg::initLogicalGroupList()
{
	...
	memset(strFileName,0,_MAX_PATH);
	memset(strPath,0,_MAX_PATH);
...
void CMonitorToolDlg::fillDeviceListCtrl()
{
	...
	char	strModes[7][12] = {"PASSTHRU", "NORMAL", "TRACKING", "REFRESH", "BACKFRESH", "FULLREFRESH", "CHECKPOINT"};
	int		iMode = 1;
	int		iNum = 0;
	CSize	size(0, 17);
...
//		char tmpMode[10];
//		itoa(m_DeviceInstanceData[i].drvmode,tmpMode,10);;
//		AfxMessageBox(tmpMode);
		...
		else if(iMode == 3 || iMode == 4 || iMode == 5)
		{
			// YELLOW
