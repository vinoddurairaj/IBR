 ...
 *
 * $Id: ftd_ioctl.c,v 1.14 2004/08/27 20:26:26 vqba0 Exp $
 *
 */
...
ftd_clear_bab_and_stats(ftd_lg_t *lgp, FTD_CONTEXT context)
{
	...
	//DbgPrint("LG?-ftd_clear_bab_and_stats >>>>>>>>>>");
    ...
    if (!iRetry)
    {
        EvLogMessage(FTD_DRIVER_BAB_FLUSH_ERROR,NULL);
#if _DEBUG
...
/************************************************************/
char *ftd_ctl_display_group_state (int mode)
{
    switch (mode)
        {
...
ftd_ctl_set_group_state(int arg, int flag)
{
    ...
    minor_num = getminor(pmd.lg_num) & ~FTD_LGFLAG; 
    lgp = (ftd_lg_t *) ddi_get_soft_state(ftd_lg_state, minor_num);
    if (lgp == NULL) 
    {
#if _DEBUG
        DbgPrint("No logical group - exiting\n");
#endif
    ...
    if (lgp->state == pmd.state) 
    {
#if _DEBUG
        DbgPrint("No state change - exiting\n");
#endif
        RELEASE_MUTEX(lgp->mutex, context);
        OUT_FCT(ftd_ctl_set_group_state)
        return FTD_SUCCESS;;
...
#if _DEBUG
    DbgPrint("LG?(%d) Driver state changed from %10.10s --> %10.10s",lgp->dev & ~FTD_LGFLAG, ftd_ctl_display_group_state(lgp->state), ftd_ctl_display_group_state(pmd.state));
#endif


    FTD_ERR (FTD_WRNLVL, "LG(%d) %10.10s --> %10.10s",lgp->dev & ~FTD_LGFLAG, 
                    ftd_ctl_display_group_state(lgp->state), 
                    ftd_ctl_display_group_state(pmd.state));

    switch (lgp->state)
    {
        case FTD_MODE_NORMAL:
        if (pmd.state == FTD_MODE_REFRESH     ||
            pmd.state == FTD_MODE_FULLREFRESH || 
            pmd.state == FTD_MODE_TRACKING    )  
            {
            ftd_clear_dirtybits(lgp);
            } 

        else if (pmd.state == FTD_MODE_CHECKPOINT_JLESS) 
            {
			}
        else if (pmd.state == FTD_MODE_PASSTHRU) 
		    {
	...
	case FTD_MODE_CHECKPOINT_JLESS:
		...
		else if (pmd.state == FTD_MODE_FULLREFRESH) 
			{
            ftd_clear_dirtybits(lgp);
            }
        else if (pmd.state == FTD_MODE_NORMAL) 
        {
            //???
        } 
        else if (pmd.state == FTD_MODE_PASSTHRU) 
    ...
    case FTD_MODE_REFRESH:   // is SMART REFRESH....
    
        if (pmd.state == FTD_MODE_NORMAL) 
        {
            ftd_clear_dirtybits(lgp);
#if _DEBUG
            DbgPrint("LG %d SRefresh-> Normal.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_TRACKING) 
        {
            /* clear the journal */
            ftd_clear_bab_and_stats(lgp, context);
#if _DEBUG
            DbgPrint("LG %d SRefresh-> Tracking.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_PASSTHRU) 
        {
            ...
            ftd_clear_bab_and_stats(lgp, context);
#if _DEBUG
            DbgPrint("LG %d SRefresh-> PassThru.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_FULLREFRESH)             
        {
#if _DEBUG
            DbgPrint("LG %d SRefresh-> FRefresh???.", lgp->dev & ~FTD_LGFLAG);
#endif
        }
        ...
        {
#if _DEBUG
            DbgPrint("LG %d SRefresh -> ??? %x ", lgp->dev & ~FTD_LGFLAG,pmd.state );
#endif
        }
    ...
    case FTD_MODE_FULLREFRESH:
        if (pmd.state == FTD_MODE_NORMAL) 
        {
            ftd_clear_dirtybits(lgp);
#if _DEBUG
            DbgPrint("LG %d FullRefresh-> Normal.?", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_TRACKING) 
        {
            ...
            ftd_clear_bab_and_stats(lgp, context);
#if _DEBUG
            DbgPrint("LG %d FullRefresh-> Tracking.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_PASSTHRU) 
        {
            ...
            ftd_clear_bab_and_stats(lgp, context);
#if _DEBUG
            DbgPrint("LG %d FullRefresh-> PassThru.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 

        else if (pmd.state == FTD_MODE_REFRESH)             
        {
#if _DEBUG        
            DbgPrint("LG %d FullRefresh-> SmartRefresh.", lgp->dev & ~FTD_LGFLAG);
#endif
        }
        else 
        {
#if _DEBUG        
            DbgPrint("LG %d FullRefresh -> ??? %x ", lgp->dev & ~FTD_LGFLAG,pmd.state );
#endif
        }
        break;
...
#if _DEBUG
            DbgPrint("LG %d Tracking -> Normal.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_PASSTHRU) 
        {
            ...
            ftd_clear_bab_and_stats(lgp, context);
#if _DEBUG
            DbgPrint("LG %d Tracking -> PassThru.", lgp->dev & ~FTD_LGFLAG);
#endif
        } 
        else if (pmd.state == FTD_MODE_REFRESH)              
        {
#if _DEBUG        
            DbgPrint("LG %d Tracking -> SmartRefresh.", lgp->dev & ~FTD_LGFLAG);
#endif
        }
        else if (pmd.state == FTD_MODE_FULLREFRESH)              
        {
#if _DEBUG            
            DbgPrint("LG %d Tracking -> FullRefresh.", lgp->dev & ~FTD_LGFLAG);
#endif
            ftd_clear_bab_and_stats(lgp, context);              
            ftd_clear_dirtybits(lgp);                             
        }
    ...
    case FTD_MODE_PASSTHRU: 
        ...
        if (pmd.state == FTD_MODE_REFRESH || pmd.state == FTD_MODE_FULLREFRESH) 
        {
            ftd_set_dirtybits(lgp);
        } 
        else if (       (pmd.state == FTD_MODE_NORMAL) 
...
*/
    ...
    while(tempdev) 
    {
        ftd_flush_lrdb(tempdev, NULL, NULL);
...
ftd_ctl_clear_dirtybits(int arg, int type)
{
    ...
    DbgPrint("LG %d - Clearing Dirty Bits !!!",lgp->dev & ~FTD_LGFLAG );
...
ftd_ctl_new_device(dev_t dev, int cmd, int arg, int flag)
{
    ...
    {
        char Buffer[50];
        EvLogMessage(FTD_DRIVER_NEW_DEV_ERROR_LGP,_itoa(lg_minor_num,Buffer,10));
        ftd_nt_del_device(softp);
        OUT_FCT(ftd_ctl_new_device)
        return ENXIO;
    }
    ...
    ALLOC_LOCK(softp->lock, "Dtc device specific lock");
...
ftd_ctl_new_lg(dev_t dev, int cmd, int arg, int flag)
{
    ...
    {
#if _DEBUG   
        DbgPrint("ftd_ctl_new_lg: ddi_get_soft_state(%08x,%08x)",ftd_lg_state, lg_minor_num);
#endif 
        OUT_FCT(ftd_ctl_new_lg)
        return(ENOTTY);
    }
...
ftd_ctl_ioctl(dev_t dev, int cmd, int arg, int flag)
{
    ...
    default:
#if _DEBUG
        DbgPrint("ftd_ctl_ioctl: Unknown IOC.\n");
#endif 
        err = ENOTTY;
        break;
    }
...
ftd_lg_send_lg_message(ftd_lg_t *lgp, int cmd, int arg, int flag)
{
    ...
    if ((buf = ftd_bab_alloc_memory(lgp->mgr, size64)) != NULL) 
    {
        ...
        // this just might crash if the message is invalid...
        if (sb.addr)
        {
            DbgPrint("send_lg_message: %s\n",sb.addr);
        }
#endif
        ...
        {
            DbgPrint("ftd_lg_send_lg_message:Could not commit memory\n");
        }
#endif
    } 
    else 
    {
        if (lgp->mgr->flags & FTD_BAB_DONT_USE)
        {
            RELEASE_MUTEX(lgp->mutex, context)
            OUT_FCT(ftd_lg_send_lg_message)
            return EAGAIN;
...
ftd_lg_oldest_entries(ftd_lg_t *lgp, int arg, int flag)
{
    ...
    //DbgPrint("LG?-Oldest entries (state=%10.10s) len64= %ld, offset=%ld, destlen=%ld",ftd_ctl_display_group_state(lgp->state),len64,offset64,destlen64 );
...
ftd_lookup_dev(ftd_lg_t *lgp, dev_t d)
{
    ...
    OUT_FCT(ftd_lookup_dev)
    return (ftd_dev_t *)NULL;  // was FTD_SUCCESS, which flunks compiler
}
...
ftd_lg_migrate(ftd_lg_t *lgp, int arg, int flag)
{
    ...
    int	            bytes;
    dev_t            lastdev;
    char                Buffer[50]; // Error message buffer
    PRE_ACQUIRE_MUTEX
    ...
    {
        //
        // Cannot migrate bytes which have this set
        //
        EvLogMessage(FTD_DRIVER_MIGRATE_BYTEERROR,_itoa(me.bytes,Buffer,10));
        OUT_FCT(ftd_lg_migrate)
        return EINVAL;
    }
    ...
    {
        //
        // Cannot migrate when there is no data
        //
        EvLogMessage(FTD_DRIVER_MIGRATE_NODATA,NULL);
        OUT_FCT(ftd_lg_migrate)
        return EINVAL;
    }
    ...
    while (temp && bytes > 0) 
    {
        ...
        {
            //
            // Trying to migrate too many entries
            //
            EvLogMessage(FTD_DRIVER_MIGRATE_TOOMANYENTRIES,_itoa(lgp->dev & ~FTD_LGFLAG,Buffer,10));
        ...
        {
            //
            // Trying to migrate too many sectors
            //
            EvLogMessage(FTD_DRIVER_MIGRATE_TOOMANYSECTORS,_itoa(lgp->dev & ~FTD_LGFLAG,Buffer,10));
        ...
        {
            //
            // Trying to migrate corrupted BAB entry
            //
            EvLogMessage(FTD_DRIVER_MIGRATE_CORRUPTBAB,_itoa(lgp->dev & ~FTD_LGFLAG,Buffer,10));
        ...
        {
            //
            // Trying to complete an uncompleted BAB entry
            //
            EvLogMessage(FTD_DRIVER_MIGRATE_UNCOMPLETEENTRY,_itoa(lgp->dev & ~FTD_LGFLAG,Buffer,10));
        }
        if (lgp->wlentries == 0) 
        {
            //
            // Migrated too many entries
            //
            EvLogMessage(FTD_DRIVER_MIGRATE_NOENTRIES,_itoa(lgp->dev & ~FTD_LGFLAG,Buffer,10));
        ...
        {
            //
            // Migrated too many sectors
            //
            EvLogMessage(FTD_DRIVER_MIGRATE_NOSECTORS,_itoa(lgp->dev & ~FTD_LGFLAG,Buffer,10));
        ...
        //DbgPrint("LG?-Migrate bytes= %ld, length= %ld",bytes,wl->length );
        ...
        //DbgPrint("LG?-Migrate** bytes= %ld, length= %ld, entries =%ld",bytes,wl->length,lgp->wlentries );
    ...
    {
        //
        // Too little data migrated
        //
        EvLogMessage(FTD_DRIVER_MIGRATE_LESSDATA,NULL);
    }
    else if (bytes < 0)
    {
        //
        // Too much data migrated
        //
        EvLogMessage(FTD_DRIVER_MIGRATE_MOREDATA,NULL);
    }
...
#ifndef NTFOUR    
// **************************************
//		BEGIN W2003/W2000 PNP CHANGES 
// **************************************

// The first two routines implement a synchronous pass-thru mechanism 
//		we need this to handle PNP events in a paging path.  
NTSTATUS ReplicatorSendToNextCompletion(	IN PDEVICE_OBJECT DeviceObject,
										IN PIRP Irp,
										IN PVOID Context) 
{ 
...
} // end ReplicatorSendToNextCompletion() 

NTSTATUS ReplicatorSendToNextDriverSync(	IN PDEVICE_OBJECT DeviceObject,
												IN PIRP Irp)
{
    ftd_dev_t			*deviceExtension;
    KEVENT				event;
    NTSTATUS			status;
    ...
    deviceExtension = (ftd_dev_t *) DeviceObject->DeviceExtension;
 
    IoCopyCurrentIrpStackLocationToNext(Irp);


    IoSetCompletionRoutine(Irp, ReplicatorSendToNextCompletion,
...
} // end ReplicatorSendToNextDriverSYNC()

/*******************************************************************
	ReplicatorDispatchPNP() 

  This is the PNP Event handler for Replicator driver

********************************************************************/
NTSTATUS
ReplicatorDispatchPnp(	IN PDEVICE_OBJECT DeviceObject,	IN PIRP Irp)
{
	...
	switch(irpSp->MinorFunction) 
	{
        ...
        case IRP_MN_DEVICE_USAGE_NOTIFICATION:
...
#ifdef TRACE_DISPATCH_PNP
			KdPrint(("ReplicatorDispatchPnP:: IRP_MN_DEVICE_USAGE_NOTIFICATION\n")); 
#endif
            irpStack = IoGetCurrentIrpStackLocation(Irp);

			// If we're not a volume, don't process this, just complete it. 
			if( !(deviceExtension->NodeIdentifier.NodeType == FTD_NODE_TYPE_ATTACHED_DEVICE) )
			{ 
	            IoCompleteRequest(Irp, IO_NO_INCREMENT);
			...
			// If we are a volume filter, then we need to handle this..
			//			If Paging I/O, then pass it down, otherwise handle the event
            ...
            /*
			  TBD: When you initialize the device extension, you need to define this 
						event doing something like: 
					KeInitializeEvent(&(pDevExt->PagingPathCountEvent), NotificationEvent, TRUE);
				
				Since we're a disk object, it's required by the I/O manager that we wait on 
				it while handling this IRP... keeps us from removing devices while paging ops
				are in progress.
			*/
			...
			/* 
				TBD: Ahhh, you also need to devine PagingPathCount element in the device
						extension for bookkeeping reasons... we use it again below. 
			*/
            ...
            // send the irp synchronously .. we do it this way because the documentation 
			//		says that we have to. 
			status = ReplicatorSendToNextDriverSync(DeviceObject, Irp);
            ...
            if (NT_SUCCESS(status)) {
                ...
                IoAdjustPagingPathCount(&deviceExtension->PagingPathCount,
        ...
        case IRP_MN_REMOVE_DEVICE:
			/* 
				IRP_MN_REMOVE_DEVICE: 
					...
					Put logic here to handle the surprise removal of a disk that 
					we might be attached to... if we don't do this, and the underlying
					drive goes away, then we'll get a blue screen (think about someone
					removing a drive from a JBOD). 
			*/

			// ** note fall thru here ** 
#ifdef TRACE_DISPATCH_PNP
			KdPrint(("ReplicatorDispatchPnP() :: IRP_MN_REMOVE_DEVICE\n")); 
#endif
...
#ifdef TRACE_DISPATCH_PNP
			KdPrint(("ReplicatorDispatchPnP() :: other IRP [0x%08x]\n",irpSp->MinorFunction)); 
#endif
			// What we're doing here is passing the IRP to the driver below us
			//		if the IRP is to a FDO attached to an underlying device. 
			//	If this is the PDO receiving the IRP, which means we're not 
			//		attached to an underlying device, then we simply complete it. 
			//	Note that if we're filtering and we don't pass this down, then bad
			//		things can happen beneath us. 
			if( deviceExtension->NodeIdentifier.NodeType  == FTD_NODE_TYPE_ATTACHED_DEVICE)
			{
			    IoSkipCurrentIrpStackLocation(Irp);
				status =  IoCallDriver(deviceExtension->TargetDeviceObject, Irp);
...
} // end ReplicatorDispatchPnp()

/******************************************************************
	ReplicatorDispatchPower() 

  Power management IRP handler - simply pass everything down. 

******************************************************************/
NTSTATUS
ReplicatorDispatchPower(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    ftd_dev_t  *deviceExtension;
...
#ifdef TRACE_POWER
	KdPrint(("ReplicatorDispatchPower()\n")); 
#endif
	if(deviceExtension->TargetDeviceObject == NULL) 
	{
	    Irp->IoStatus.Status = STATUS_INVALID_DEVICE_REQUEST;
...
} // end ReplicatorDispatchPower


/******************************************************************
	ReplicatorDispatchSysCtrl() 

  System Control management IRP handler - simply pass everything down. 

******************************************************************/
NTSTATUS 
ReplicatorDispatchSysCtrl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    PIO_STACK_LOCATION			irpSp = IoGetCurrentIrpStackLocation(Irp);
    ftd_dev_t                   *deviceExtension;
...
#ifdef _TRACE_SYS_CTRL
	KdPrint(("ReplicatorDispatchSysCtrl() :: IRP [0x%08x]\n",irpSp->MinorFunction)); 
#endif
    ...
    //
    // We don't implement WMI, so 
    // let's just pass it down to 
    // the next guy.
    //
    IoSkipCurrentIrpStackLocation(Irp);
