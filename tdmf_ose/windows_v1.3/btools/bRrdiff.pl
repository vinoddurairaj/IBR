...
open (STDOUT, ">rdiff.txt");
open (STDERR, "\&STDOUT");

print "\n\nSTART :   bRdiff.pl :  script \n\n";
print "\ncvs : determine source changes present  :  no source change  :  no build\n\n";
...
@list= qx ! cvs -Q rdiff -s $branch_rdiff -r $tag0 tdmf_ose/windows_v1.3 !;
...
######################################
######################################
#  SAVE  :  do we require cvs status $filename  :  get the version
#
#	then grab the log entry  :  if NO, then delete
######################################
######################################

#for ($i=0 ; $keepparsing[$i] ; $i++) {

   #print "\nFOR : index : keepparsing[$i] : $keepparsing[$i]\n";
...
#              if ("keepparsing[$i]" =~ m/buildcommitstring/ ) {
#                 splice ( @keepparsing, $i, 1);
#                 $i=$i - 1;
#                print "\n\ni after subtract - 1 = :  $i\n";   
#             } # if close bracket
...
open (FH1, ">sourcechangelist.txt");

print FH1 "@keepparsing\n";

close (FH1);

open (FH2, ">btoolschangelist.txt");
...
if ($force) {
    print "\nbSMrdiff.pl :  force option supplied\n";
