    ...
    foreach $line (@lines) {
        ...
        if (defined($Values[0])) {
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NT" ) ) {
                ...
                $Globals{"Platform"}  = "ALL";
                ProcInstallSection( $Values[0].".NT" );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NTX86" ) ) {
                ...
                ProcInstallSection( $Values[0].".NTX86" );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NTIA64" ) ) {
                ...
                ProcInstallSection( $Values[0].".NTIA64" );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NTAMD64" ) ) {
                ...
                ProcInstallSection( $Values[0].".NTAMD64" );
            }
...
sub CallChkSD
{
   ...
   if (!defined($Globals{"Chksd"})) {
      ...
      #
      if (system ("i386\\chksd.exe", "-test") != 0) {
         # can't execute chksd, log failure
         AddErrorRef(1319,1);
         $Globals{"Chksd"} = FALSE;
      }
      else {
   ...
   if ( $Globals{"Chksd"} == TRUE ) {
        if ( system("i386\\chksd.exe", $linetxt) != 0) {
        ...
        foreach $line (@lines) {
            ...
            if (uc $RegRoot eq "HKLM") {
                if ( $SubKey =~ /SYSTEM\\CURRENTCONTROLSET\\SERVICES\\.+?\\(.*)/i) {
                    AddErrorRef(1305, $CurrentINF->getIndex($Section,$count) )
                        unless (defined $1 and $1 =~ /^Parameters(\\.*)?$/i);
                }
            }
            ...
            if (defined($Flags) && ($Flags !~ /^[\s\t]*$/)) {
                ...
                if ($Flags !~ /^\d*$/) {
                    ...
                    AddErrorRef(1066,$CurrentINF->getIndex($Section, $count),"flags ($Flags)",$Section)
                        if (($Flags | 0xFFFF003f) != 0xFFFF003f);
...
sub VerifyServiceInstallSection {
    ...
    foreach $line (@lines) {
       ...
       unless (defined($SIDirectives{uc($Directive)}))  {
        ...
        if ($SIDirectives{uc($Directive)} eq "VERIFY") {
            ...
            AddErrorRef(1078, $CurrentINF->getIndex($Section, $count)) if (length($Value) > 1024);
        } elsif ($SIDirectives{uc($Directive)} ne "UNDEF") {
            AddErrorRef(1006, $CurrentINF->getIndex($Section, $count), $Directive);
        } else {
            $SIDirectives{uc($Directive)} = $Value;
            $SILine{uc($Directive)}       = $CurrentINF->getIndex($Section, $count);
        }

        if (uc $Directive eq "SERVICEBINARY") {
            if ($line !~ /=\s*\"?\%(-?\d*)\%/) { #"
                AddWarnRef(2005, $CurrentINF->getIndex($Section,$count));
            } else {
                AddWarnRef(2005, $CurrentINF->getIndex($Section,$count)) if ($1 eq "-1");
            }
        }
    ...
    if ($SIDirectives{"SERVICETYPE"} ne "UNDEF") {
        ...
        if (! is_numeric($SIDirectives{"SERVICETYPE"}) ) {
            # Error out if we're not numeric now.
            AddErrorRef(1066,$SILine{"SERVICETYPE"},"ServiceType ($SIDirectives{SERVICETYPE})", $Section);
        } else {
        ...
        AddErrorRef(1058, $SecStart, "ServiceType", $Section);
    }
    ...
    if ($SIDirectives{"ERRORCONTROL"} ne "UNDEF") {
        ...
        AddErrorRef(10666,$SILine{"ERRORCONTROL"},"ErrorControl ($SIDirectives{ERRORCONTROL})",$Section)
    ...
    if ($SIDirectives{"SERVICEBINARY"} eq "UNDEF") {
        ...
        if (   ($Globals{"Platform"} ne "IA64")
            ...
            # Verify a different binary wasn't given for same service name
            if (defined $Globals{"AllSIBins"}{uc $ServiceName}) {
                AddErrorRef(1014,$SILine{"SERVICEBINARY"},$ServiceName,$Globals{"AllSIBins"}{uc $ServiceName})
                    unless (uc $Globals{"AllSIBins"}{uc $ServiceName} eq uc $SIDirectives{"SERVICEBINARY"});
            } else {
                $Globals{"AllSIBins"}{uc $ServiceName} = $SIDirectives{"SERVICEBINARY"}
            }
    ...
    if ($SIDirectives{"STARTTYPE"} ne "UNDEF") {
        ...
        if ($SIDirectives{"STARTTYPE"} =~ /^\d*$/) {
            ...
            AddErrorRef(1066,$SILine{"STARTTYPE"},"StartType ($SIDirectives{STARTTYPE})",$Section);
        }
    } else {
        AddErrorRef(1058, $SecStart, "StartType", $Section);
    }
        ...
        if ($RegSection =~ /^@/) {
            ...
            if (! $#sdfiles < 0) {
                AddErrorRef(1110, $CurrentINF->getIndex($Section, $Offset), $RegSection)
                    if ((grep(/$RegSection/i,@sdfiles)) < 1);
            }
            AddErrorRef(1093, $CurrentINF->getIndex($Section, $Offset))
    ...
    if (defined $CheckedSections{uc($Section)})  {
        ...
        foreach $line (@lines) {
            ...
            # Check if IRQCONFIG key.
                ...
                if ($Value =~  /^[SL]?[:]?([0-9\s,]*)$/) {
                    @IRQList=GetList($1,$CurrentINF->getIndex($Section, $count),1);
                    $cnt=@IRQList;
                    for($i = 0; $i < $cnt; $i++) {
                        AddErrorRef(1102,$CurrentINF->getIndex($Section, $count))
...
sub NEEDS {
    ...
    if ( $CurrentINF->sectionDefined($NeedsSection) ) {
...
sub GetError {
     ...
     my(%ErrorTable) = (
        ...
        1319 => ["Error executing chksd.exe.  Please verify chksd.exe is in the i386 subdirectory of the directory that contains chkinf.bat"],
