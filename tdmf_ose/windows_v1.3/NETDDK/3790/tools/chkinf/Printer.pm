...
package PRINTER;
    ...
    #---------------------------------------------------#
    my($Version) = "5.2.3715.0";    # Version of this file
...
sub ExpandModelSection
{
    ...
    foreach $line (@lines)
    {
        ...
        foreach $PnpID (@Values)
        {
            ...
            $BaseMfrModel =~ tr/ /_/; # replace spaces with underscores
            ...
            if (   ($ReliableChk)
                || ($LessReliableChk))
            {
                my $FakeId = CalcFakePnpID($BaseMfrModel);
                {
                    if ($ReliableChk) # high confidence this is a real error
                    {
                        # 8560 => ["Manufactured (ie: fake) PnpID '%s' is incorrect, it should be '%s'"],
...
sub CalcFakePnpID
{
    ...
    return sprintf("%s%04X", substr($MfrModel, 0, 20), ($CS & 0x0FFFF));
}
